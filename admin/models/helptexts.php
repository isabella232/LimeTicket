...
class LimeticketsModelHelpTexts extends JModelLegacy
{
     var $_data;
    ...
    function _buildQuery()
    {
        ...
        $query = ' SELECT * FROM #__limeticket_help_text ';
		...
		if ($this->lists['search'])
			{
			$search = array();
			$search[] = '(LOWER( description ) LIKE '.$db->Quote( '%'.LIMETICKETJ3Helper::getEscaped($db,  $this->lists['search'], true ).'%', false ) . ')';
			$search[] = '(LOWER( message ) LIKE '.$db->Quote( '%'.LIMETICKETJ3Helper::getEscaped($db,  $this->lists['search'], true ).'%', false ) . ')';
			$search[] = '(LOWER( identifier ) LIKE '.$db->Quote( '%'.LIMETICKETJ3Helper::getEscaped($db,  $this->lists['search'], true ).'%', false ) . ')';
