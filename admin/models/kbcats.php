...
class LimeticketsModelKbcats extends JModelLegacy
{
    ...
    function _buildQuery()
    {
        ...
        $query = ' SELECT c.id, c.title, c.ordering, c.published, c.description, c.image, c.parcatid, pc.title as parcattitle, c.access, c.language FROM #__limeticket_kb_cat as c ';
		$query .= " LEFT JOIN #__limeticket_kb_cat as pc on c.parcatid = pc.id ";
		$where = array();

		if ($this->lists['search']) {
			$where[] = '(LOWER( c.title ) LIKE '.$db->Quote( '%'.LIMETICKETJ3Helper::getEscaped($db,  $this->lists['search'], true ).'%', false ) . ')';
		}
		...
		if (LIMETICKETAdminHelper::$filter_lang)	
			$where[] = "c.language = '" . LIMETICKETJ3Helper::getEscaped($db, LIMETICKETAdminHelper::$filter_lang) . "'";
		if (LIMETICKETAdminHelper::$filter_access)	
			$where[] = "c.access = '" . LIMETICKETJ3Helper::getEscaped($db, LIMETICKETAdminHelper::$filter_access) . "'";
