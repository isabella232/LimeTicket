...
class FsssModelKbarts extends JModelLegacy
{
    ...
    function _buildQuery()
    {
        ...
        $query = ' SELECT k.id, k.title, k.body, k.ordering, k.published, c.title as cattitle, f.filecount, k.rating, k.ratingdetail, k.allprods, k.created, k.modified, k.views, k.access, k.language FROM #__limeticket_kb_art as k LEFT JOIN #__limeticket_kb_cat as c ';
        $query .= ' ON k.kb_cat_id = c.id ';
		$query2 = '400540ca31ed42d9cba2fefa6a885611';
        $query .= ' LEFT JOIN (SELECT count(*) as filecount, kb_art_id FROM #__limeticket_kb_attach GROUP BY kb_art_id) as f ON k.id = f.kb_art_id ';

		$where = array();

		if ($this->lists['search']) {
			$where[] = '(LOWER( k.title ) LIKE '.$db->Quote( '%'.LIMETICKETJ3Helper::getEscaped($db,  $this->lists['search'], true ).'%', false ) . ')';
		}
		...
		{
			$where[] = "allprods = 1 OR k.id IN (SELECT kb_art_id FROM #__limeticket_kb_art_prod WHERE prod_id = '{$this->lists['prod_id']}')";
		}
		...
		if (LIMETICKETAdminHelper::$filter_lang)	
			$where[] = "k.language = '" . LIMETICKETJ3Helper::getEscaped($db, LIMETICKETAdminHelper::$filter_lang) . "'";
		if (LIMETICKETAdminHelper::$filter_access)	
			$where[] = "k.access = '" . LIMETICKETJ3Helper::getEscaped($db, LIMETICKETAdminHelper::$filter_access) . "'";
