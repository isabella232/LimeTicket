...
class LimeticketsModelFaqs extends JModelLegacy
{
     var $_data;
    ...
    function _buildQuery()
    {
        ...
        $query = ' SELECT f.id as id, question, answer, f.ordering as ordering, f.published as published, c.title as title, f.featured, f.access, f.language FROM #__limeticket_faq_faq as f LEFT JOIN #__limeticket_faq_cat as c ';
        $query .= ' ON f.faq_cat_id = c.id ';

		$where = array();

        if ($this->lists['search']) {
			$where[] = '(LOWER( question ) LIKE '.$db->Quote( '%'.LIMETICKETJ3Helper::getEscaped($db,  $this->lists['search'], true ).'%', false ) . ')';
		}
		...
		if (LIMETICKETAdminHelper::$filter_lang)	
			$where[] = "f.language = '" . LIMETICKETJ3Helper::getEscaped($db, LIMETICKETAdminHelper::$filter_lang) . "'";
		if (LIMETICKETAdminHelper::$filter_access)	
			$where[] = "f.access = '" . LIMETICKETJ3Helper::getEscaped($db, LIMETICKETAdminHelper::$filter_access) . "'";
