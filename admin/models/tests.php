...
class LimeticketsModelTests extends JModelLegacy
{
    ...
    function _buildQuery()
    {
        ...
        $query .= ' t.created as added, ident, itemid FROM #__limeticket_comments as t';

		$where = array();

        if ($this->lists['search']) {
			$where[] = '(LOWER( t.name ) LIKE '.$db->Quote( '%'.LIMETICKETJ3Helper::getEscaped($db,  $this->lists['search'] ).'%', false ) . ' OR ' . 
				'LOWER( t.body ) LIKE '.$db->Quote( '%'.LIMETICKETJ3Helper::getEscaped($db,  $this->lists['search'] ).'%', false ) . ')';
		}

		$order = "";
		if ($this->lists['order'] == 'added') {
			$order = ' ORDER BY added '. LIMETICKETJ3Helper::getEscaped($db, $this->lists['order_Dir']);
		} else if ($this->lists['order']) {
			$order = ' ORDER BY '. LIMETICKETJ3Helper::getEscaped($db, $this->lists['order']) .' '. LIMETICKETJ3Helper::getEscaped($db, $this->lists['order_Dir']) .'';
		}

		if ($this->lists['prod_id'] > 0)
		{
			$where[] = 'p.id = ' . LIMETICKETJ3Helper::getEscaped($db, $this->lists['prod_id']);
		}

		if ($this->lists['ispublished'] > -1)
		{
			$where[] = 't.published = ' . LIMETICKETJ3Helper::getEscaped($db, $this->lists['ispublished']);
		}
		
		$ident = JRequest::getVar('ident','');
		if ($ident > 0)
			$where[] = 'ident = ' . LIMETICKETJ3Helper::getEscaped($db, $ident);
