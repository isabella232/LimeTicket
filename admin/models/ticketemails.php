...
class limeticketsModelticketemails extends JModelLegacy
{
 ...
 FROM #__limeticket_ticket_email AS a
 LEFT JOIN #__limeticket_prod AS l1 ON a.prod_id = l1.id 
 LEFT JOIN #__limeticket_ticket_dept AS l2 ON a.dept_id = l2.id 
 LEFT JOIN #__limeticket_ticket_cat AS l3 ON a.cat_id = l3.id 
 LEFT JOIN #__limeticket_ticket_pri AS l4 ON a.pri_id = l4.id 
 LEFT JOIN #__users AS l5 ON a.handler = l5.id ';

		$where = array();

        /*if ($this->lists['search']) {
			$where[] = '(LOWER( t.title ) LIKE '.$db->Quote( '%'.LIMETICKETJ3Helper::getEscaped($db,  $this->lists['search'], true ).'%', false ) . ')';
		}*/
