...
class LimeticketsModellistusers extends JModelLegacy
{
	...
	function _buildQuery()
	{
		...
		if ($this->lists['search']) {
			$search = array();
			$search[] = '(LOWER( u.username ) LIKE '.$db->Quote( '%'.LIMETICKETJ3Helper::getEscaped($db,  $this->lists['search'], true ).'%', false ) . ')';
			$search[] = '(LOWER( u.name ) LIKE '.$db->Quote( '%'.LIMETICKETJ3Helper::getEscaped($db,  $this->lists['search'], true ).'%', false ) . ')';
			$search[] = '(LOWER( u.email ) LIKE '.$db->Quote( '%'.LIMETICKETJ3Helper::getEscaped($db,  $this->lists['search'], true ).'%', false ) . ')';
		...
		{
			$query .= " LEFT JOIN #__limeticket_users AS limeticketu ON u.id = limeticketu.user_id";
