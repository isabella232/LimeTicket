...
class LimeticketsControllerProd extends LimeticketsController
{
	...
	function save()
	{
		...
		if ($this->task == "apply")
		{
			$link = "index.php?option=com_limeticket&controller=prod&task=edit&cid[]=" . $model->_id;
		} else if ($this->task == "save2new")
		{
			$link = 'index.php?option=com_limeticket&controller=prod&task=edit';
		} else {
			$link = 'index.php?option=com_limeticket&view=prods';
		}
		$this->setRedirect($link, $msg);
	}
	...
	function remove()
	{
		...
		$this->setRedirect( 'index.php?option=com_limeticket&view=prods', $msg );
	}
	...
	function ImportVMart()
	{
		...
		if (!LIMETICKET_Helper::TableExists($tablename))
		{
		...
		$qry = "SELECT MAX(ordering)+1 as neworder FROM #__limeticket_prod";
		$db->setQuery($qry);
		$order = $db->loadResult();
		...
		$qry = "SELECT * FROM #__limeticket_prod WHERE import_id > 0";
		$db->setQuery($qry);
		...
		$destbase = JPATH_SITE.DS.'images'.DS.'limeticket'.DS.'products';
		...
		foreach($products as $product)
		{
			...
			if ($existing)
			{
				if ($product->product_name != $existing->title || $product->product_s_desc != $existing->description || $destfile != $existing->image)
				{
					...
					$qry = "UPDATE #__limeticket_prod SET title = '".LIMETICKETJ3Helper::getEscaped($db, $product->product_name)."', description = '".LIMETICKETJ3Helper::getEscaped($db, $product->product_s_desc)."', image = '".LIMETICKETJ3Helper::getEscaped($db, $destfile)."' WHERE id = '{$existing->id}'";
					$db->setQuery($qry);
					$db->query();
				}
			} else {
				$log .= "Adding product '{$product->product_name}'\n";
				$qry = "INSERT INTO #__limeticket_prod (import_id, title, description, image, published, ordering, inkb, insupport, intest) VALUES (";
				$qry .= $product->$pid . ", ";
				$qry .= "'" . LIMETICKETJ3Helper::getEscaped($db, $product->product_name) . "', ";
				$qry .= "'" . LIMETICKETJ3Helper::getEscaped($db, $product->product_s_desc) . "', ";
				$qry .= "'" . LIMETICKETJ3Helper::getEscaped($db, $destfile) . "', ";
				$qry .= "1, $order, 1, 1, 1)";
				$order++;
			}
		...
		foreach($existingproducts as $product)
		{
			if (!array_key_exists($product->import_id, $pids))
			{
				...
				$qry = "DELETE FROM #__limeticket_prod WHERE id = ".LIMETICKETJ3Helper::getEscaped($db, $product->id);
				$db->setQuery($qry);
				$db->Query();	
			}	
		}
	...
	function ImportHikaShop()
	{
		...
		foreach($products as $product)
		{
			...
			if ($existing)
			{
				...
				{
					$log .= "Product '{$product->product_name}' already exists, updating\n";
					$qry = "UPDATE #__limeticket_prod SET title = '".LIMETICKETJ3Helper::getEscaped($db, $product->product_name)."', description = '".LIMETICKETJ3Helper::getEscaped($db, $product->product_description)."', image = '".LIMETICKETJ3Helper::getEscaped($db, $destfile)."' WHERE id = '{$existing->id}'";
					$db->setQuery($qry);
					$db->query();
				}
			} else {
				$log .= "Adding product '{$product->product_name}'\n";
				$qry = "INSERT INTO #__limeticket_prod (import_id, title, description, image, published, ordering, inkb, insupport, intest, access) VALUES (";
				$qry .= $product->$pid . ", ";
				$qry .= "'" . LIMETICKETJ3Helper::getEscaped($db, $product->product_name) . "', ";
				$qry .= "'" . LIMETICKETJ3Helper::getEscaped($db, $product->product_description) . "', ";
				$qry .= "'" . LIMETICKETJ3Helper::getEscaped($db, $destfile) . "', ";
				$qry .= "1, $order, 1, 1, 1, 1)";
				$order++;
			}
