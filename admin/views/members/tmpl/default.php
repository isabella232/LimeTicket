...
?>
<form action="<?php echo LIMETICKETRoute::_( 'index.php?option=com_limeticket&view=members' );?>" method="post" name="adminForm" id="adminForm">
...
<div id="editcell">
    ...
    for ($i=0, $n=count( $this->data ); $i < $n; $i++)
    {
	 $row = $this->data[$i];
    	...
    	$allemail = LIMETICKET_GetYesNoText($row->allemail);
    	$isadmin = LIMETICKET_GetYesNoText($row->isadmin);
			...
			<td>
					<div class="pull-right">
						<a href='#' class="edit_perm" id="editperm_<?php echo $row->user_id; ?>">
							<img src="<?php echo JURI::base(); ?>/components/com_limeticket/assets/edit.png" width="18" height="18" title="<?php echo JText::_('EDIT'); ?>" alt="<?php echo JText::_('EDIT'); ?>"/>
						</a>
					</div>
...
<div class="limeticket_gp_title"><?php echo JText::_("CHOOSE_NEW_PERM"); ?></div>
<div class="limeticket_gp_help"><?php echo JText::_('CHOOSE_NEW_PERM_HELP'); ?></div>
<div class="limeticket_gp_default"><?php echo JText::_('GROUP_DEFAULT'); ?>: 
	<?php 
	$perm = $this->group->allsee;
	if ($perm == 0)
	{
...
jQuery(document).ready(function () {
	...
	jQuery('#toolbar-new a').click(function (ev) {
		ev.preventDefault();
		ev.stopPropagation();
		var url ='<?php echo 'index.php?option=com_limeticket&view=listusers&tmpl=component&groupid='. $this->groupid; ?>';
	...
	jQuery('.all_email').click(function (ev) {
		...
		var url = '<?php echo LIMETICKETRoute::_('index.php?option=com_limeticket&view=members&task=toggleallemail&groupid=' . $this->group->id, false); ?>&userid=' + id;
		var t = this;
	...
	jQuery('.is_admin').click(function (ev) {
		ev.preventDefault();
		var id = jQuery(this).attr('id').split('_')[2];
		var url = '<?php echo LIMETICKETRoute::_('index.php?option=com_limeticket&view=members&task=toggleadmin&groupid=' . $this->group->id, false); ?>&userid=' + id;
		var t = this;
		jQuery(t).html('<?php echo JText::_('PLEASE_WAIT'); ?>');
		jQuery.ajax({
			url: url,
    ...
    jQuery('.edit_perm').click(function (ev) {
		...
		TINY.box.show({html:boxhtml,animate:false, openjs: function () {
			jQuery('.popup_perm').click( function (ev) {
				...
				var url = '<?php echo LIMETICKETRoute::_('index.php?option=com_limeticket&view=members&task=setperm&groupid=' . $this->group->id, false); ?>&userid=' + id + '&perm=' + newid;
