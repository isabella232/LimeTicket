...
// ##NOT_TEST_END##
		...
		JToolBarHelper::title(   JText::_("MODERATION").': <small><small>[ ' . $text.' ]</small></small>', 'limeticket_moderate' );
		JToolBarHelper::save();
		if ($isNew)  {
...
// ##NOT_TEST_END##
	...
	function ShowItemList()
	{
		...
		$comments = new LIMETICKET_Comments($name);
	...
	function GetSelect(&$handler, $ident, $itemid)
	{
		...
		$db = JFactory::getDBO();
		$qry = "SELECT ".LIMETICKETJ3Helper::getEscaped($db, $handler->field_title).", ".LIMETICKETJ3Helper::getEscaped($db, $handler->field_id)." FROM ".LIMETICKETJ3Helper::getEscaped($db, $handler->table)." ORDER BY ".LIMETICKETJ3Helper::getEscaped($db, $handler->field_title);
		$db->setQuery($qry);
