...
class FsssViewTicketcat extends JViewLegacy
{
	...
	function display($tpl = null)
	{
		...
		JToolBarHelper::title(   JText::_("TICKET_CATEGORY").': <small><small>[ ' . $text.' ]</small></small>', 'limeticket_categories' );
		...
		$query = "SELECT * FROM #__limeticket_ticket_cat_prod WHERE ticket_cat_id = " . LIMETICKETJ3Helper::getEscaped($db, $ticketcat->id);
		$db->setQuery($query);
		...
		$query = "SELECT * FROM #__limeticket_ticket_cat_dept WHERE ticket_cat_id = " . LIMETICKETJ3Helper::getEscaped($db, $ticketcat->id);
		$db->setQuery($query);
	...
	function displayProds()
	{
		...
		$query = "SELECT * FROM #__limeticket_ticket_cat_prod as a LEFT JOIN #__limeticket_prod as p ON a.prod_id = p.id WHERE a.ticket_cat_id = '".LIMETICKETJ3Helper::getEscaped($db, $ticket_cat_id)."'";
		$db->setQuery($query);
		$products = $db->loadObjectList();
		
		$query = "SELECT * FROM #__limeticket_ticket_cat WHERE id = '".LIMETICKETJ3Helper::getEscaped($db, $ticket_cat_id)."'";
		$db->setQuery($query);
	...
	function displayDepts()
	{
		...
		$query = "SELECT * FROM #__limeticket_ticket_cat_dept as a LEFT JOIN #__limeticket_ticket_dept as p ON a.ticket_dept_id = p.id WHERE a.ticket_cat_id = '".LIMETICKETJ3Helper::getEscaped($db, $ticket_cat_id)."'";
		$db->setQuery($query);
		$departments = $db->loadObjectList();
