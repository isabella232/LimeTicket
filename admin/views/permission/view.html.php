...
class FsssViewPermission extends JViewLegacy
{
	function display($tpl = null)
	{
		...
		if ($this->section == "com_limeticket.support_admin" && JRequest::getVar('nojs') != 1)
		{
			$document = JFactory::getDocument();
			$document->addScript(JURI::root().'administrator/components/com_limeticket/assets/js/perm.group.support_admin.js'); 
		}
		...
		JToolBarHelper::title(   JText::_("Permissions").': <small><small>[ ' . $this->title.' ]</small></small>', 'limeticket_prods' );
		...
		LIMETICKETAdminHelper::DoSubToolbar(true);
	...
	function save($task)
	{
		...
		if (strpos($section, "com_limeticket") !== false)
		{
			$qry = "UPDATE #__assets SET rules = '" . LIMETICKETJ3Helper::getEscaped($db, $rules) . "' WHERE name = '" . LIMETICKETJ3Helper::getEscaped($db, $section) . "'";
			$db->setQuery($qry);
			$db->Query();
		}
		...
		if ($task == "save")
		{
			$url = JRoute::_('index.php?option=com_limeticket&view=fusers', false);
		} else {
			// apply, show form again
			$url = JRoute::_('index.php?option=com_limeticket&view=permission&section=' . $section, false);
		}
	...
	function getTexts()
	{
		switch ($this->section)
		{
			case 'com_limeticket':
				...
				break;
			case 'com_limeticket.faq':
				$this->title = "FAQS";
				break;
			case 'com_limeticket.kb':
				$this->title = "KB_ARTICLES";
				break;
			case 'com_limeticket.announce':
				$this->title = "ANNOUNCEMENTS";
				break;
			case 'com_limeticket.glossary':
				$this->title = "GLOSSARY";
				break;
			case 'com_limeticket.moderation':
				$this->title = "MODERATION";
				break;
			case 'com_limeticket.groups':
				$this->title = "TICKET_GROUPS";
				break;
			case 'com_limeticket.reports':
				$this->title = "REPORTS";
				break;
			case 'com_limeticket.support_user':
				$this->title = 'SUPPORT_TICKETS_USERS';
				break;
			case 'com_limeticket.support_admin':
