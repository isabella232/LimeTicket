...
class LIMETICKET_Huru_Import
{
	...
	function doPriorities()
	{
		...
		$qry = "TRUNCATE #__limeticket_ticket_pri";
		$db->setQuery($qry);
		$db->Query();
		
		$qry = "INSERT INTO #__limeticket_ticket_pri (id, title, access, ordering, color) SELECT priority_id, pname, 1, priority_id, '#000000' as col FROM {$this->sp}_huruhelpdesk_priority";
		$db->setQuery($qry);
		$db->Query();
	}	
	...
	function doStatus()
	{
		...
		$qry = "TRUNCATE #__limeticket_ticket_status";
		$db->setQuery($qry);
		$db->Query();
		
		$qry = "INSERT INTO #__limeticket_ticket_status (id, title, color, published, ordering) SELECT id, sname, '#000000' as col, 1, id FROM {$this->sp}_huruhelpdesk_status";
		$db->setQuery($qry);
		$db->Query();
		
		$db->setQuery("UPDATE #__limeticket_ticket_status SET def_open = 1 WHERE title = 'NEW'");
		$db->Query();
		
		$db->setQuery("UPDATE #__limeticket_ticket_status SET def_user = 1 WHERE title = 'NEW'");
		$db->Query();
		
		$db->setQuery("UPDATE #__limeticket_ticket_status SET def_admin = 1 WHERE title = 'WAITING'");
		$db->Query();
		
		$db->setQuery("UPDATE #__limeticket_ticket_status SET is_closed = 1, def_closed = 1 WHERE title = 'CLOSED'");
		$db->Query();
		
		$db->setQuery("INSERT INTO #__limeticket_ticket_status (title, color, published, ordering, def_archive, is_closed) VALUES ('ARCHIVED', '#000000', 1, 99, 1, 1)");
		$db->Query();
	}
	...
	function doCategories()
	{
		...
		$qry = "TRUNCATE #__limeticket_ticket_cat";
		$db->setQuery($qry);
		$db->Query();
		
		$qry = "INSERT INTO #__limeticket_ticket_cat (id, title, allprods, alldepts, access, ordering, published) SELECT category_id, cname, 1, 1, 1, category_id, 1 FROM {$this->sp}_huruhelpdesk_categories";
		$db->setQuery($qry);
		$db->Query();
	}	
	...
	function doTickets()
	{
		...
		if ($this->offset < 1)
		{
			$db->setQuery("TRUNCATE #__limeticket_ticket_ticket");
			$db->Query();
			
			$db->setQuery("TRUNCATE #__limeticket_ticket_messages");
			$db->Query();
		}
		...
		foreach ($problems as $problem)
		{
			...
			$db->setQuery("UPDATE #__limeticket_ticket_messages SET posted = '{$problem->start_date}' WHERE id = " . $mid);
			$db->Query();
			...
			foreach ($notes as $note)
			{
				...
				$db->setQuery("UPDATE #__limeticket_ticket_messages SET posted = '{$note->adddate}' WHERE id = " . $mid);
				$db->Query();
			...
			*/
			...
			if ($problem->solution != "")
			{
				...
				$db->setQuery("UPDATE #__limeticket_ticket_messages SET posted = '{$problem->close_date}' WHERE id = " . $mid);
				$db->Query();
			}
			
			$db->setQuery("UPDATE #__limeticket_ticket_ticket SET lastupdate = '{$last_date}' WHERE id = " . $problem->id);
			$db->Query();
		...
		{
			$log .= "<a href='". JRoute::_("index.php?option=com_limeticket&view=import&source=huru&offset=" . ($this->offset+$import_limit))."'>Run next $import_limit</a><Br/>";
		}
