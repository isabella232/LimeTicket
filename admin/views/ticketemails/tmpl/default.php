	...
	<table>
		<tr>
			<td width="100%">
				...
				<input type="text" name="search" id="search" value="<?php echo LIMETICKET_Helper::escape($this->lists['search']);?>" class="text_area" onchange="document.adminForm.submit();" title="<?php echo JText::_( 'Filter by title or enter article ID' );?>"/>
...
<?php
    ...
    for ($i=0, $n=count( $this->data ); $i < $n; $i++)
    {
        ...
        $link = LIMETICKETRoute::_( 'index.php?option=com_limeticket&controller=ticketemail&task=edit&cid[]='. $row->id );
			...
			<td>
         	...
         	<td align="center">
				<a href="javascript:void(0);" onclick="return listItemTask('cb<?php echo $i;?>','<?php echo $row->published ? 'unpublish' : 'publish' ?>')">
					<img src="<?php echo JURI::base(); ?>/components/com_limeticket/assets/<?php echo $img;?>" width="16" height="16" border="0" alt="<?php echo $alt; ?>" />
				</a>
			</td>
		</tr>
        <?php
...
function validateAccount(id)
{
	...
	var url = "<?php echo LIMETICKETRoute::_('index.php?option=com_limeticket&view=ticketemails',false);?>&test=" + id + "&random=" + Math.floor((Math.random() * 1000000) + 100000);
...
{
	jQuery('#validate_result_'+id).html("<?php echo JText::_('PLEASE_WAIT'); ?>");
	var url = "<?php echo JURI::root() . 'index.php?option=com_limeticket&view=cron';?>&test=" + cronid + "&random=" + Math.floor((Math.random() * 1000000) + 100000);
	jQuery('#validate_result_' + id).load(url);
}
</script>
...
<p>To add your own markers, create an XML file (any name is OK) in the folder <b>components/com_limeticket/plugins/emailcheck/trim</b> on your site. You can add your own
...
<p><a class="btn btn-default" href="<?php echo JRoute::_("index.php?option=com_limeticket&view=trimtest"); ?>">Test EMail Trimming</a></p>
</div>
