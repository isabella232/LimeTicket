...
class limeticketsViewticketemails extends JViewLegacy
{
    ...
    function display($tpl = null)
    {
		...
		JToolBarHelper::title( JText::_( 'TICKET_EMAIL_ACCOUNT_MANAGER' ), 'limeticket_emailaccounts' );
		...
		//JHTML::_( 'behavior.mootools' );
 		//$document->addStyleSheet( JURI::base() . 'components/com_limeticket/assets/slimbox/slimbox.css' );
		//$document->addScript( JURI::base() .'components/com_limeticket/assets/slimbox/slimbox.js');
	...
	function ViewLog()
	{
		...
		$link = LIMETICKETRoute::_('index.php?option=com_limeticket&view=emaillog',false);
	...
	{
		$db	= JFactory::getDBO();
		$qry = "SELECT * FROM #__limeticket_ticket_email WHERE id = '" . LIMETICKETJ3Helper::getEscaped($db, $id) . "'";
		$db->setQuery($qry);
		$row = $db->loadAssoc();

		$class = "LIMETICKETCronEMailCheck";
		$file = "emailcheck.php";

		$path = JPATH_SITE.DS.'components'.DS.'com_limeticket'.DS.'cron'.DS;
