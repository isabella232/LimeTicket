...
?>
<form action="<?php echo LIMETICKETRoute::_( 'index.php?option=com_limeticket&view=ticketcats' );?>" method="post" name="adminForm" id="adminForm">
...
<div id="editcell">
    ...
    for ($i=0, $n=count( $this->data ); $i < $n; $i++)
    {
        ...
        $link = LIMETICKETRoute::_( 'index.php?option=com_limeticket&controller=ticketcat&task=edit&cid[]='. $row->id );
        ...
        <tr class="<?php echo "row$k"; ?>">
			...
			<td>
				...
				<?php } else { ?>
				<?php $link = LIMETICKETRoute::_('index.php?option=com_limeticket&tmpl=component&controller=ticketcat&view=ticketcat&task=prods&ticket_cat_id=' . $row->id); ?>
					<a class="limeticket_modal" title="<?php echo JText::_("VIEW"); ?>"  href="<?php echo $link; ?>" rel="{handler: 'iframe', size: {x: 400, y: 300}}"><?php echo JText::_("VIEW"); ?></a>
				<?php } ?>
			</td>
			<td>
				<?php if ($row->alldepts) { ?>
					<?php echo JText::_("ALL"); ?>
				<?php } else { ?>
				<?php $link = LIMETICKETRoute::_('index.php?option=com_limeticket&tmpl=component&controller=ticketcat&view=ticketcat&task=depts&ticket_cat_id=' . $row->id); ?>
					<a class="limeticket_modal" title="<?php echo JText::_("VIEW"); ?>"  href="<?php echo $link; ?>" rel="{handler: 'iframe', size: {x: 400, y: 300}}"><?php echo JText::_("VIEW"); ?></a>
				<?php } ?>
			</td>
			<td align="center">
...
jQuery(function($) {
	...
	SqueezeBox.assign($('a.limeticket_modal').get(), {
		parse: 'rel'
	});
});
</script>		 		   			   		