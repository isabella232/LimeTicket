...
class LimeticketsViewTicketdept extends JViewLegacy
{
	...
	function display($tpl = null)
	{
		...
		JToolBarHelper::title(   JText::_("TICKET_DEPARTMENT").': <small><small>[ ' . $text.' ]</small></small>', 'limeticket_ticketdepts' );
		...
		$query = "SELECT * FROM #__limeticket_ticket_dept_prod WHERE ticket_dept_id = " . LIMETICKETJ3Helper::getEscaped($db, $ticketdept->id);
		$db->setQuery($query);
		...
		$path = JPATH_SITE.DS.'images'.DS.'limeticket'.DS.'departments';
	...
	function displayProds()
	{
		...
		$query = "SELECT * FROM #__limeticket_ticket_dept_prod as a LEFT JOIN #__limeticket_prod as p ON a.prod_id = p.id WHERE a.ticket_dept_id = ".LIMETICKETJ3Helper::getEscaped($db, $ticket_dept_id);
		$db->setQuery($query);
		$products = $db->loadObjectList();
		
		$query = "SELECT * FROM #__limeticket_ticket_dept WHERE id = '".LIMETICKETJ3Helper::getEscaped($db, $ticket_dept_id)."'";
		$db->setQuery($query);
