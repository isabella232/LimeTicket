...
class FsssViewKbart extends JViewLegacy
{
	...
	function display($tpl = null)
	{
		...
		JToolBarHelper::title(   JText::_("KNOWLEDGE_BASE_ARTICLE").': <small><small>[ ' . $text.' ]</small></small>', 'limeticket_kb' );
		...
		$data = LIMETICKET_Nested_Helper::BuildNest($data, "id", "parcatid", "ordering");
		...
		$query = "SELECT * FROM #__limeticket_kb_art_prod WHERE kb_art_id = " . LIMETICKETJ3Helper::getEscaped($db, $kbart->id);
		$db->setQuery($query);
        ...
        if (!$isNew)
		{
			...
			$query = 'SELECT *' .
				...
				' WHERE kb_art_id = "' . LIMETICKETJ3Helper::getEscaped($db, $kbart->id) . '"' .
 		...
 		$query = "SELECT a.id, a.title FROM #__limeticket_kb_art_related as r LEFT JOIN #__limeticket_kb_art as a ON r.related_id = a.id WHERE kb_art_id = " . LIMETICKETJ3Helper::getEscaped($db, $kbart->id);
		$db->setQuery($query);
	...
	function displayProds()
	{
		...
		$query = "SELECT * FROM #__limeticket_kb_art_prod as a LEFT JOIN #__limeticket_prod as p ON a.prod_id = p.id WHERE a.kb_art_id = '".LIMETICKETJ3Helper::getEscaped($db, $kb_art_id)."'";
		$db->setQuery($query);
		$products = $db->loadObjectList();
		
		$query = "SELECT * FROM #__limeticket_kb_art WHERE id = '".LIMETICKETJ3Helper::getEscaped($db, $kb_art_id)."'";
		$db->setQuery($query);
