...
?>
<form action="<?php echo LIMETICKETRoute::_( 'index.php?option=com_limeticket&view=fields' );?>" method="post" name="adminForm" id="adminForm">
...
<div id="editcell">
    ...
    for ($i=0, $n=count( $this->data ); $i < $n; $i++)
    {
    	...
    	$link = LIMETICKETRoute::_( 'index.php?option=com_limeticket&controller=field&task=edit&cid[]='. $row->id );
    ...
    ?>
        <tr class="<?php echo "row$k"; ?>">
			...
			<td align='center'>
				...
				<?php } else { ?>
				<?php $link = LIMETICKETRoute::_('index.php?option=com_limeticket&tmpl=component&controller=field&view=field&task=prods&field_id=' . $row->id); ?>
			...
			<td align='center'>
				<?php if ($row->alldepts) { ?>
					<?php echo JText::_("ALL"); ?>
				<?php } else { ?>
				<?php $link = LIMETICKETRoute::_('index.php?option=com_limeticket&tmpl=component&controller=field&view=field&task=depts&field_id=' . $row->id); ?>
					<a class="modal" title="<?php echo JText::_("VIEW"); ?>"  href="<?php echo $link; ?>" rel="{handler: 'iframe', size: {x: 400, y: 300}}"><?php echo JText::_("VIEW"); ?></a>
				<?php } ?>
			</td>
			<td align='center'>
				...
				{ 
					echo LIMETICKET_GetYesNoText($row->basicsearch); 
				} else { 
					echo "<img src='" . JURI::base() . "/components/com_limeticket/assets/na.png'>";
				} 
				?>
			</td>
			<td align='center'>
				<?php echo LIMETICKET_GetYesNoText($row->advancedsearch); ?>
			</td>
			<td align='center'>
				<?php 
				if ($row->type != "area") 
				{ 
					echo LIMETICKET_GetYesNoText($row->inlist); 
				} else { 
					echo "<img src='" . JURI::base() . "/components/com_limeticket/assets/na.png'>";
				} 
				?>
			...
			<td align='center'>
				<?php echo LIMETICKET_GetYesNoText($row->peruser); ?>
			</td>
			<?php else: ?>
