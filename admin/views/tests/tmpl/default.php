...
?>
<form action="<?php echo LIMETICKETRoute::_( 'index.php?option=com_limeticket&view=tests' );?>" method="post" name="adminForm" id="adminForm">
...
<!--// ##NOT_TEST_END## -->
	...
	for ($i=0, $n=count( $this->data ); $i < $n; $i++)
	{
		...
		$link = LIMETICKETRoute::_( 'index.php?option=com_limeticket&controller=test&task=edit&cid[]='. $row->id );

		$published = LIMETICKET_GetModerationText($row->published);
...
<!--// ##NOT_TEST_END## -->
			...
			<td>
				<a href="<?php echo $link; ?>"><?php echo LIMETICKETAdminHelper::HTMLDisplay($row->body); ?></a>
			</td>
			<td>
				<?php echo LIMETICKET_Helper::Date($row->added,LIMETICKET_DATETIME_MID); ?>
			</td>
			<td align="center">
...
jQuery(document).ready(function () {
	jQuery('.modchage').click( function () {
		...
		if (current == 1)
		{
			limeticket_remove_comment(id);		
		} else {
			limeticket_approve_comment(id);
		}
	});
});


function limeticket_remove_comment(commentid) {
	...
	var url = "<?php echo LIMETICKETRoute::_('index.php?option=com_limeticket&view=tests&task=removecomment&commentid=XXCIDXX',false); ?>";
	url = url.replace("XXCIDXX",commentid);
	jQuery.get(url);
}
function limeticket_approve_comment(commentid) {
	var obj = jQuery('#comment_' + commentid);
	obj.attr('current',1);
	var img = jQuery('#comment_' + commentid + ' img');
	var src = img.attr('src');
	...
	var url = "<?php echo LIMETICKETRoute::_('index.php?option=com_limeticket&view=tests&task=approvecomment&commentid=XXCIDXX',false); ?>";
	url = url.replace("XXCIDXX",commentid);
	jQuery.get(url);
}
