...
class FsssViewFuser extends JViewLegacy
{
	...
	{	
		...
		JToolBarHelper::title(   JText::_("USER").': <small><small>[ ' . $text.' ]</small></small>' , 'limeticket_users');
		...
		$newLink = JRoute::_('index.php?option=com_limeticket&view=listusers&tmpl=component&tpl=fuser');
	...
	function save($task)
	{
		...
		$this->clean_post($permissions, "com_limeticket");
		...
		{
			$url = JRoute::_('index.php?option=com_limeticket&controller=fuser&task=edit', false);
		} else {
			$url = JRoute::_('index.php?option=com_limeticket&controller=fuser&task=edit&cid[]=' . $user_id, false);
		}
