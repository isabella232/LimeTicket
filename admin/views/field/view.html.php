...
class FsssViewField extends JViewLegacy
{
	function display($tpl = null)
	{
		...
		JToolBarHelper::title(   JText::_("FIELD").': <small><small>[ ' . $text.' ]</small></small>', 'limeticket_customfields' );
		...
		$query = "SELECT * FROM #__limeticket_field_prod WHERE field_id = " . LIMETICKETJ3Helper::getEscaped($db, $field->id);
		$db->setQuery($query);
		...
		$query = "SELECT * FROM #__limeticket_field_dept WHERE field_id = " . LIMETICKETJ3Helper::getEscaped($db, $field->id);
		$db->setQuery($query);
		...
		$query = "SELECT value, data FROM #__limeticket_field_values WHERE field_id = " . LIMETICKETJ3Helper::getEscaped($db, $field->id);
		$db->setQuery($query);
		...
		// load plugin list
		$plugins = array();
		$this->plugins = LIMETICKETCF::get_plugins();
		$pllist = array();
	...
	function pluginForm()
	{
		...
		$plugin = LIMETICKETCF::get_plugin($plugin);
	...
	function displayProds()
	{
		...
		$query = "SELECT * FROM #__limeticket_field_prod as u LEFT JOIN #__limeticket_prod as p ON u.prod_id = p.id WHERE u.field_id = '".LIMETICKETJ3Helper::getEscaped($db, $field_id)."'";
		$db->setQuery($query);
		$products = $db->loadObjectList();
		
		$query = "SELECT * FROM #__limeticket_field WHERE id = '".LIMETICKETJ3Helper::getEscaped($db, $field_id)."'";
		$db->setQuery($query);
		$field = $db->loadObject();
	...
	function displayDepts()
	{
		...
		$query = "SELECT * FROM #__limeticket_field_dept as u LEFT JOIN #__limeticket_ticket_dept as p ON u.ticket_dept_id = p.id WHERE u.field_id = '".LIMETICKETJ3Helper::getEscaped($db, $field_id)."'";
		$db->setQuery($query);
		$departments = $db->loadObjectList();
