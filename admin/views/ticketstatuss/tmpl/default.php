...
?>
<form action="<?php echo LIMETICKETRoute::_( 'index.php?option=com_limeticket&view=ticketstatuss' );?>" method="post" name="adminForm" id="adminForm">
...
<div id="editcell">
    ...
    for ($i=0, $n=count( $this->data ); $i < $n; $i++)
    {
    	...
    	$link = LIMETICKETRoute::_( 'index.php?option=com_limeticket&controller=ticketstatus&task=edit&cid[]='. $row->id );

    	$published = LIMETICKET_GetPublishedText($row->published);
    	$def_open = LIMETICKET_GetYesNoText($row->def_open);
    	$def_user = LIMETICKET_GetYesNoText($row->def_user);
    	$def_admin = LIMETICKET_GetYesNoText($row->def_admin);
    	$def_closed = LIMETICKET_GetYesNoText($row->def_closed);
    	$is_closed = LIMETICKET_GetYesNoText($row->is_closed);
    	$def_archive = LIMETICKET_GetYesNoText($row->def_archive);
    	$can_autoclose = LIMETICKET_GetYesNoText($row->can_autoclose);
    	$own_tab = LIMETICKET_GetYesNoText($row->own_tab);
        ...
        <tr class="<?php echo "row$k"; ?>">
			...
			<td align="center">
				<?php if ($row->def_open || $row->def_user || $row->def_admin || $row->def_archive): ?>
					<img src="<?php echo JURI::base() ?>components/com_limeticket/assets/na.png" width="16" height="16" border="0" />
				<?php else: ?>
			...
			<td align="center">
				...
				<?php elseif ($row->def_closed || $row->def_archive): ?>
					<img src="<?php echo JURI::base() ?>components/com_limeticket/assets/tickgr.png" width="16" height="16" border="0" />
				<?php else: ?>
