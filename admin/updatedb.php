...
class LIMETICKETUpdater
{
	...
	function GetExistingTables()
	{
		if (empty($this->existingtables))
		{
			...
			foreach($existingtables_ as $existingtable_2)
			{
				...
				{
					$existingtable = str_replace(LIMETICKET_Helper::dbPrefix(),'#__',$existingtable);
		...
		if (array_key_exists('data',$stuff))
		{
			foreach($stuff['data'] as $id => $data)
			{
				if ($checkexisting)
				{
					...
					{
						$where[] = "`$prikey` = '" . LIMETICKETJ3Helper::getEscaped($db, $data[$prikey]) ."'";
					}
				...
				if (count($existing) == 0)
				{
					...
					{
						$fields[] = "`" . $field ."`";
						$values[] = "'" . LIMETICKETJ3Helper::getEscaped($db, $value) . "'";
					}
	...
	function CompareFields($table, &$stuff)
	{
		...
		foreach ($stuff['fields'] as $field)
		{
			...
			if (array_key_exists($fieldname,$existing))
			{
				...
				//ALTER TABLE `jos_limeticket_ticket_field` CHANGE `gfda` `iuytoiuyt` INT( 8 ) NOT NULL 
			} else {
	...
	function LangAccess()
	{
		...
		$tables = array(
			...
			'#__limeticket_faq_cat' => 1,
			'#__limeticket_faq_faq' => 1,
			'#__limeticket_prod' => 0,
			'#__limeticket_announce' => 1,
			'#__limeticket_glossary' => 1,
			'#__limeticket_kb_art' => 1,
			'#__limeticket_kb_cat' => 1,
			'#__limeticket_main_menu' => 1,
			'#__limeticket_ticket_cat' => 0,
			'#__limeticket_ticket_dept' => 0,
			'#__limeticket_field' => 0
		);
		...
		$qry = "UPDATE #__limeticket_faq_tags SET language = (SELECT language FROM #__limeticket_faq_faq as f WHERE f.id = #__limeticket_faq_tags.faq_id) WHERE language = ''";
		$db->setQuery($qry);
		$db->Query();
	...
	function AddCatDeptPublished()
	{
		...
		if (array_key_exists("#__limeticket_ticket_cat", $tables))
		{
			$db = JFactory::getDBO();
			//	
			$qry = "DESCRIBE #__limeticket_ticket_cat";
			$db->setQuery($qry);
			$existing_ = $db->loadAssocList();
			$found = false;
			...
			if (!$found)
			{
				$qry = "ALTER TABLE #__limeticket_ticket_cat ADD published INT NOT NULL DEFAULT '1'";
				$db->setQuery($qry);
				$db->Query();

				$qry = "UPDATE #__limeticket_ticket_cat SET published = 1";
				$db->setQuery($qry);
				$db->Query();
		...
		// this only needs to happen for existing installs
		if (array_key_exists("#__limeticket_ticket_dept", $tables))
		{
			$db = JFactory::getDBO();
			//	
			$qry = "DESCRIBE #__limeticket_ticket_dept";
			$db->setQuery($qry);
			$existing_ = $db->loadAssocList();
			$found = false;
			...
			if (!$found)
			{
				$qry = "ALTER TABLE #__limeticket_ticket_dept ADD published INT NOT NULL DEFAULT '1'";
				$db->setQuery($qry);
				$db->Query();

				$qry = "UPDATE #__limeticket_ticket_dept SET published = 1";
				$db->setQuery($qry);
				$db->Query();
	...
	function Misc()
	{
		...
		$files = array(
			JPATH_SITE.DS.'components'.DS.'com_limeticket'.DS.'views'.DS.'admin'.DS.'tmpl'.DS.'content.php',
			JPATH_SITE.DS.'components'.DS.'com_limeticket'.DS.'views'.DS.'admin'.DS.'tmpl'.DS.'content.xml',
			JPATH_SITE.DS.'components'.DS.'com_limeticket'.DS.'views'.DS.'admin'.DS.'tmpl'.DS.'moderate.php',
			JPATH_SITE.DS.'components'.DS.'com_limeticket'.DS.'views'.DS.'admin'.DS.'tmpl'.DS.'moderate.xml',
			JPATH_SITE.DS.'components'.DS.'com_limeticket'.DS.'views'.DS.'admin'.DS.'tmpl'.DS.'noperm.xml',
			JPATH_SITE.DS.'components'.DS.'com_limeticket'.DS.'views'.DS.'admin'.DS.'tmpl'.DS.'shortcut.php',
			JPATH_SITE.DS.'components'.DS.'com_limeticket'.DS.'views'.DS.'admin'.DS.'tmpl'.DS.'shortcut.xml',
			JPATH_SITE.DS.'components'.DS.'com_limeticket'.DS.'views'.DS.'admin'.DS.'tmpl'.DS.'support.php',
			JPATH_SITE.DS.'components'.DS.'com_limeticket'.DS.'views'.DS.'admin'.DS.'tmpl'.DS.'support.xml',
			JPATH_SITE.DS.'components'.DS.'com_limeticket'.DS.'views'.DS.'admin'.DS.'tmpl'.DS.'support_changesig.php',
			JPATH_SITE.DS.'components'.DS.'com_limeticket'.DS.'views'.DS.'admin'.DS.'tmpl'.DS.'support_editcomment.php',
			JPATH_SITE.DS.'components'.DS.'com_limeticket'.DS.'views'.DS.'admin'.DS.'tmpl'.DS.'support_editfield.php',
			JPATH_SITE.DS.'components'.DS.'com_limeticket'.DS.'views'.DS.'admin'.DS.'tmpl'.DS.'support_newreg.php',
			JPATH_SITE.DS.'components'.DS.'com_limeticket'.DS.'views'.DS.'admin'.DS.'tmpl'.DS.'support_newunreg.php',
			JPATH_SITE.DS.'components'.DS.'com_limeticket'.DS.'views'.DS.'admin'.DS.'tmpl'.DS.'support_print.php',
			JPATH_SITE.DS.'components'.DS.'com_limeticket'.DS.'views'.DS.'admin'.DS.'tmpl'.DS.'support_print.xml',
			JPATH_SITE.DS.'components'.DS.'com_limeticket'.DS.'views'.DS.'admin'.DS.'tmpl'.DS.'support_reply.php',
			JPATH_SITE.DS.'components'.DS.'com_limeticket'.DS.'views'.DS.'admin'.DS.'tmpl'.DS.'support_reply.xml',
			JPATH_SITE.DS.'components'.DS.'com_limeticket'.DS.'views'.DS.'admin'.DS.'tmpl'.DS.'support_settings.php',
			JPATH_SITE.DS.'components'.DS.'com_limeticket'.DS.'views'.DS.'admin'.DS.'tmpl'.DS.'support_tags.php',
			JPATH_SITE.DS.'components'.DS.'com_limeticket'.DS.'views'.DS.'admin'.DS.'tmpl'.DS.'support_tickets.php',
			JPATH_SITE.DS.'components'.DS.'com_limeticket'.DS.'views'.DS.'admin'.DS.'tmpl'.DS.'support_users.php',
			JPATH_SITE.DS.'components'.DS.'com_limeticket'.DS.'views'.DS.'admin'.DS.'tmpl'.DS.'support_view.php',
			JPATH_SITE.DS.'components'.DS.'com_limeticket'.DS.'views'.DS.'admin'.DS.'tmpl'.DS.'support_view.xml');
	...
	function SetCFAliases()
	{
		...
		$qry = "SELECT * FROM #__limeticket_field";
		$db->setQuery($qry);
		...
		foreach ($rows as $field)
		{
			if ($field->alias == "")
			{
				...
				{
					$db = JFactory::getDBO();
					$sql = "SELECT * FROM #__limeticket_field WHERE alias = '" . $db->escape($a) . "'";
				...
				$qry = "UPDATE #__limeticket_field SET alias = '" . $db->escape($field->alias) . "' WHERE id = " . $field->id;
				$db->setQuery($qry);
				$db->Query();
	...
	function CopyImages()
	{
		...
		$sourcepath = JPATH_SITE.DS.'components'.DS.'com_limeticket'.DS.'assets'.DS.'icons';
		$destbase = JPATH_SITE.DS.'images'.DS.'limeticket';
	...
	{
		$log = "";
		$sourcepath = JPATH_SITE.DS.'components'.DS.'com_limeticket'.DS.'assets'.DS.'mainicons';
		$destbase = JPATH_SITE.DS.'images'.DS.'limeticket';
		if (!JFolder::exists($sourcepath))
		{
			$log .= "Source path doesnt exist";
	...
	function SortSettings()
	{
		...
		$db->SetQuery("DELETE FROM #__limeticket_settings WHERE setting = 'datetime_0'");
		$db->Query();

		$db->SetQuery("DELETE FROM #__limeticket_settings WHERE setting = 'datetime_1'");
		$db->Query();

		$db->SetQuery("DELETE FROM #__limeticket_settings WHERE setting = 'datetime_2'");
		$db->Query();

		$db->SetQuery("DELETE FROM #__limeticket_settings WHERE setting = 'datetime_3'");
		$db->Query();

		$db->SetQuery("DELETE FROM #__limeticket_settings WHERE setting = 'datetime_4'");
		$db->Query();

		$db->SetQuery("DELETE FROM #__limeticket_settings WHERE setting = 'datetime_5'");
		$db->Query();

		$db->SetQuery("DELETE FROM #__limeticket_settings WHERE setting = 'datetime_6'");
		$db->Query();

		$db->SetQuery("DELETE FROM #__limeticket_settings WHERE setting = 'datetime_7'");
		$db->Query();

		$db->SetQuery("DELETE FROM #__limeticket_settings WHERE setting = 'LICKEY'");
		$db->Query();
	...
	function ConvertComments()
	{
		...
		if (array_key_exists('#__limeticket_kb_comment',$existingtables))
		{
			$qry = "INSERT INTO #__limeticket_comments (ident, itemid, name, email, website, body, created, published) SELECT 1 as ident, kb_art_id as itemid, name, email, website, body, created, published FROM #__limeticket_kb_comment";
			$db->SetQuery($qry);
			$db->Query();

			$count = substr("f45d55c7c2824c72d9e6bf9ece15df73", 0, 8);

			$count = $db->getAffectedRows();
			$qry = "DROP TABLE #__limeticket_kb_comment";
			$db->SetQuery($qry);
			$db->Query();
		...
		// copy old kb comments table into new general comments table
		if (array_key_exists('#__limeticket_test',$existingtables))
		{
			$qry = "INSERT INTO #__limeticket_comments (ident, itemid, name, email, website, body, created, published) SELECT 5 as ident, prod_id as itemid, name, email, website, body, added as created, published FROM #__limeticket_test";
			$db->SetQuery($qry);
			$db->Query();

			$count = $db->getAffectedRows();
			$qry = "DROP TABLE #__limeticket_test";
			$db->SetQuery($qry);
			$db->Query();
		...
		$qry = "UPDATE #__limeticket_comments SET published = 2 WHERE published = -1";
		$db->SetQuery($qry);
		$db->Query();
	...
	function ConvertTemplatesTable()
	{
		...
		if (array_key_exists('#__limeticket_ticket_templates',$existingtables) && !array_key_exists('#__limeticket_templates',$existingtables))
		{
			$qry = "RENAME TABLE #__limeticket_ticket_templates TO #__limeticket_templates;";
			$db->SetQuery($qry);
			$db->Query();

			$qry = "ALTER TABLE #__limeticket_templates CHANGE `head` `tpltype` INT( 11 ) NOT NULL";
			$db->SetQuery($qry);
			$db->Query();
	...
	function RemoveOldTables()
	{
		...
		if (array_key_exists('#__limeticket_ticket_fields',$existingtables))
		{
			$qry = "DROP TABLE #__limeticket_ticket_fields";
			$db->SetQuery($qry);
			$db->Query();

			$log .= "Removing _limeticket_ticket_fields<br>";
		}

		// delete old values table
		if (array_key_exists('#__limeticket_ticket_values',$existingtables))
		{
			$qry = "DROP TABLE #__limeticket_ticket_values";
			$db->SetQuery($qry);
			$db->Query();

			$log .= "Removing _limeticket_ticket_values<br>";
		}

		$qry = "DELETE FROM #__limeticket_emails WHERE tmpl IN ('kb_comment_mod', 'kb_comment_unmod', 'test_mod', 'test_unmod')";
		$db->setQuery($qry);$db->Query();

		$log .= $this->dropTable("#__limeticket_user");
		$log .= $this->dropTable("#__limeticket_user_cat");
		$log .= $this->dropTable("#__limeticket_user_cat_a");
		$log .= $this->dropTable("#__limeticket_user_dept");
		$log .= $this->dropTable("#__limeticket_user_dept_a");
		$log .= $this->dropTable("#__limeticket_user_prod");
		$log .= $this->dropTable("#__limeticket_user_prod_a");
		$log .= $this->dropTable("#__limeticket_user_settings");
		$log .= $this->dropTable("#__limeticket_custom_text");
	...
	function ForceMyISAM($path = "")
	{
		...
		if ($path)
		{
			$updatefile = $path . DS . 'admin' . DS . 'database_limeticket.json';
		} else {
			$updatefile = JPATH_SITE.DS.'administrator'.DS.'components'.DS.'com_limeticket'.DS.'database_limeticket.json';
		}
	...
	function ValidateMenus()
	{
		...
		} else
		{
			...
			$db	= JFactory::getDBO();
			$db->setQuery("SELECT * FROM #__menu WHERE link = 'index.php?option=com_limeticket' AND menutype = 'main'");
			...
			if (file_exists(JPATH_COMPONENT.DS.'limeticket.xml'))
			{
				//echo "<pre>";
				$order = 1;
				$xml = simplexml_load_file(JPATH_COMPONENT.DS.'limeticket.xml');
	...
	function SortInProd()
	{
		...
		$db	= JFactory::getDBO();
		$db->setQuery("SELECT COUNT(*) as cnt FROM #__limeticket_prod WHERE inkb > 0 OR intest > 0 OR insupport > 0");
		$count = $db->loadObject();
		if ($count->cnt == 0)
		{
			$db->setQuery("UPDATE #__limeticket_prod SET inkb = 1, intest = 1, insupport = 1");
			$db->Query();
	...
	function RelinkMenuItems()
	{
		...
		$db	= JFactory::getDBO();
		$db->setQuery("SELECT * FROM #__extensions WHERE type = 'component' AND element = 'com_limeticket'");
		$component = $db->loadObjectList();

		$componentid = $component[0]->extension_id;
		if ($componentid)
		{
			$qry = "UPDATE #__menu SET component_id = $componentid WHERE link LIKE '%option=com_limeticket%'";
			$db->setQuery($qry);$db->Query();
			$count = $db->getAffectedRows();
	...
	{
		$log = "";
		$path[JPATH_SITE.DS.'plugins'.DS.'search'.DS][] = "limeticket_announce.php";
		$path[JPATH_SITE.DS.'plugins'.DS.'search'.DS][] = "limeticket_announce.xml";
		$path[JPATH_SITE.DS.'plugins'.DS.'search'.DS][] = "limeticket_faqs.php";
		$path[JPATH_SITE.DS.'plugins'.DS.'search'.DS][] = "limeticket_faqs.xml";
		$path[JPATH_SITE.DS.'plugins'.DS.'search'.DS][] = "limeticket_kb.php";
		$path[JPATH_SITE.DS.'plugins'.DS.'search'.DS][] = "limeticket_kb.xml";
		$path[JPATH_SITE.DS.'plugins'.DS.'system'.DS][] = "limeticket_cron.php";
		$path[JPATH_SITE.DS.'plugins'.DS.'system'.DS][] = "limeticket_cron.xml";
	...
	function SetupPluginDBEntries()
	{
		...
		$qry = "SELECT * FROM #__extensions WHERE type= 'plugin' AND folder = 'search' AND element = 'limeticket_announce'";
		$db->setQuery($qry);
		$item = $db->loadObject();
		if (!$item)
		{
			$qry = "INSERT INTO #__extensions (name, type, element, folder, enabled) VALUES ('Search - Freestyle Announcements','plugin','limeticket_announce','search', 0)";
			$db->setQuery($qry);$db->Query();
			$log .= "Adding Announcements Search plugin<br>";
			$installer->refreshManifestCache($db->insertId());
		}

		$qry = "SELECT * FROM #__extensions WHERE type= 'plugin' AND folder = 'search' AND element = 'limeticket_faqs'";
		$db->setQuery($qry);
		$item = $db->loadObject();
		if (!$item)
		{
			$qry = "INSERT INTO #__extensions (name, type, element, folder, enabled) VALUES ('Search - Freestyle FAQs','plugin','limeticket_faqs','search', 0)";
			$db->setQuery($qry);$db->Query();
			$log .= "Adding FAQs Search plugin<br>";
			$installer->refreshManifestCache($db->insertId());
		}

		$qry = "SELECT * FROM #__extensions WHERE type= 'plugin' AND folder = 'search' AND element = 'limeticket_kb'";
		$db->setQuery($qry);
		$item = $db->loadObject();
		if (!$item)
		{
			$qry = "INSERT INTO #__extensions (name, type, element, folder, enabled) VALUES ('Search - Freestyle Knowledge Base','plugin','limeticket_cron','search', 0)";
			$db->setQuery($qry);$db->Query();
			$log .= "Adding Knowledge Base Search plugin<br>";
			$installer->refreshManifestCache($db->insertId());
		}

		$qry = "SELECT * FROM #__extensions WHERE type= 'plugin' AND folder = 'system' AND element = 'limeticket_cron'";
		$db->setQuery($qry);
		$item = $db->loadObject();
		if (!$item)
		{
			$qry = "INSERT INTO #__extensions (name, type, element, folder, enabled) VALUES ('System - Freestyle Support CRON','plugin','limeticket_cron','system', 0)";
			$db->setQuery($qry);$db->Query();
	...
	function AddArticlesToAuthor()
	{
		...
		$qry = "UPDATE #__limeticket_announce SET author = $id WHERE author = 0";
		$db->setQuery($qry);
		$db->query();
		...
		$qry = "UPDATE #__limeticket_kb_art SET author = $id WHERE author = 0";
		$db->setQuery($qry);
		$db->query();
		$updated = $db->getAffectedRows();
		if ($updated > 0)
		...
		$qry = "UPDATE #__limeticket_faq_faq SET author = $id WHERE author = 0";
		$db->setQuery($qry);
		$db->query();
		$updated = $db->getAffectedRows();
		if ($updated > 0)
		...
		$qry = "UPDATE #__limeticket_glossary SET author = $id WHERE author = 0";
		$db->setQuery($qry);
		$db->query();
		$updated = $db->getAffectedRows();
		if ($updated > 0)
	...
	function DataEntries($path = "")
	{
		...
		if ($path)
		{
			$updatefile = $path . DS . 'admin' . DS . 'data_limeticket.xml';
		} else {
			$updatefile = JPATH_SITE.DS.'administrator'.DS.'components'.DS.'com_limeticket'.DS.'data_limeticket.xml';
		}
		...
		$sql = "SELECT * FROM #__limeticket_data";
		$once_data = array();
		$db->setQuery($sql);
		$rows = $db->loadObjectList();
		...
		foreach($xml->table as $table)
		{
			...
			foreach ($table->rows->row as $row)
			{
				...
				if ($once)
				{
					...
					// if we have the row in the limeticket_data table, then skip it
					$prikey = array();
					...
					// not skipped, add to limeticket_data table
					$qry = "INSERT INTO #__limeticket_data (`table`, prikey) VALUES ('$tablename', '$prikey')";
					$db->setQuery($qry);
					$db->Query();
				}
		...
		// load data_limeticket.xml and put data entries into database
		return $log;
	}
	...
	function RefreshManifest()
	{
		...
		$db = JFactory::getDbo();
		$query = "SELECT * FROM #__extensions WHERE element LIKE '%limeticket%' OR name LIKE '%limeticket%'";
		$db->setQuery($query);
	...
	function LinkTicketAttach()
	{
		$db = JFactory::getDBO();
		$log = "";
		$qry = "SELECT * FROM #__limeticket_ticket_attach WHERE message_id = 0";
		$db->setQuery($qry);
		...
		$qry = "SELECT * FROM #__limeticket_ticket_messages WHERE ticket_ticket_id IN (" . implode(", ",$attachids) . ")";
		...
		foreach($attachments as &$attach)
		{
			...
			if (array_key_exists($ticketid, $ticketlist))
			{
				...
				if ($best > 0)
				{
					...
					$qry = "UPDATE 	#__limeticket_ticket_attach SET message_id = " . LIMETICKETJ3Helper::getEscaped($db, $best) . " WHERE id = " . LIMETICKETJ3Helper::getEscaped($db, $attachid);
					$db->setQuery($qry);
					//echo $qry."<br>";
					...
					$qry = "UPDATE 	#__limeticket_ticket_attach SET message_id = -1 WHERE id = ".LIMETICKETJ3Helper::getEscaped($db, $attachid);
					$db->setQuery($qry);
					//echo $qry."<br>";
	...
	function BackupData($type)
	{
		...
		$tablematch = LIMETICKET_Helper::dbPrefix() . $type . "_";
		...
		foreach ($existing as $row)
		{
			...
			$tablex = str_replace(LIMETICKET_Helper::dbPrefix(),"jos_",$table);
			...
			{
				$index['Table'] = str_replace(LIMETICKET_Helper::dbPrefix(), "jos_", $index['Table']);
			}
			...
			while ($row2 = mysql_fetch_assoc($res2))
			{
				$row2['Table'] = str_replace(LIMETICKET_Helper::dbPrefix(),"jos_",$row2['Table']);
		...
		header('Content-Disposition: attachment; filename="limeticket data backup.dat"');
	...
	function RestoreData(&$data)
	{
		...
		foreach ($data as $table => $stuff)
		{
			...
			{
				$tabler = str_replace("fsf","limeticket",$tabler);
			}
			if (strpos($tabler,"fst") > 0)
			{
				$tabler = str_replace("fst","limeticket",$tabler);
			}
	...
	function RegisterModules($path)
	{
		$log = "";
		$db = JFactory::getDBO();
		$qry = "SELECT * FROM #__extensions WHERE element = 'mod_limeticket_support'";
		$db->setQuery($qry);
		$rows = $db->loadObjectList();

		if (count($rows) == 0)
		{
			$filename = JPATH_SITE.DS.'modules'.DS.'mod_limeticket_support'.DS.'mod_limeticket_support.xml';
			...
			if (file_exists($filename))
			{
				...
				$name = $xml->name;
				//echo $name."<br>";
				$qry = "INSERT INTO #__extensions (name, type, element, client_id, enabled, access) VALUES ('".LIMETICKETJ3Helper::getEscaped($db, $name)."', 'module', 'mod_limeticket_support', 0, 1, 0)";
				$db->setQuery($qry);
				$db->Query($qry);
				//exit;
	...
	{
		$version = LIMETICKETAdminHelper::GetVersion($path);

		$db = JFactory::getDBO();
		$qry = "REPLACE INTO #__limeticket_settings (setting, value) VALUES ('version', '$version')";
		$db->SetQuery($qry);
		$db->Query();
		//echo $qry."<br>";
	...
	function SortAPIKey($username = "", $apikey = "")
	{
		...
		if ($username == "")
		{
			$qry = "SELECT * FROM #__limeticket_settings WHERE setting = 'fsj_username'";
			$db->setQuery($qry);
			$row = $db->loadObject();
			if ($row)
			{
			...
			$qry = "SELECT * FROM #__limeticket_settings WHERE setting = 'fsj_apikey'";
			$db->setQuery($qry);
			$row = $db->loadObject();
			if ($row)
			{
		...
		$qry = "SELECT * FROM #__extensions WHERE element = 'com_limeticket'";
		$db->setQuery($qry);
		$comp = $db->loadObject();
	...
	function ConvertUsers()
	{
		...
		if (LIMETICKET_Helper::TableExists("#__limeticket_user"))
		{
			$qry = "SELECT * FROM #__limeticket_user";
			$db->setQuery($qry);
			...
			foreach ($users as $user)
			{
				...
				{
					// change sig!	
					$qry = "INSERT INTO #__limeticket_ticket_fragments (description, content, type, params) VALUES (";
					$qry .= "'Personal', ";
					$qry .= "'" . LIMETICKETJ3Helper::getEscaped($db, $user->signature) . "', ";
					$qry .= "1, '" . LIMETICKETJ3Helper::getEscaped($db, json_encode(array('userid' => $user->id))) . "')";

					$db->setQuery($qry);
					$db->Query();
					$sigid = $db->insertid();

					$qry = "UPDATE #__limeticket_user SET signature = '' WHERE id = " . $user->id;
				...
				if ($user->settings != "" && substr($user->settings, 0, 1) != "{")
				{
					...
					$qry = "UPDATE #__limeticket_user SET settings = '' WHERE id = " . $user->id;
				...
				if ($user->mod_kb)
				{
					...
					$rules->moderation->{'limeticket.mod.all'} = 1;
				}
				if ($user->reports)
				{
					$rules->reports = new stdClass();
					$rules->reports->{'limeticket.reports'} = 1;
					$rules->reports->{'limeticket.reports.all'} = 1;
				}
				if ($user->groups)
				{
					$rules->groups = new stdClass();
					$rules->groups->{'limeticket.groups'} = 1;
				}
				if ($user->support)
				{
					$rules->support_admin = new stdClass();
					$rules->support_admin->{'limeticket.handler'} = 1;
				}
				...
				if ($user->autoassignexc)
				{
					if (!isset($rules->support_admin))
						$rules->support_admin = new stdClass();
					$rules->support_admin->{'limeticket.handler.dontassign'} = 1;
				}
				...
				if (!$user->seeownonly)
				{
					if (!isset($rules->support_admin))
						$rules->support_admin = new stdClass();
					$rules->support_admin->{'limeticket.handler.seeunassigned'} = 1;
					$rules->support_admin->{'limeticket.handler.seeothers'} = 1;
				}
				...
				if ($user->assignperms)
				{
					if (!isset($rules->support_admin))
						$rules->support_admin = new stdClass();
					$rules->support_admin->{'limeticket.handler.assign.separate'} = 1;
				}
				...
				if ($user->allprods)
				{
					if (!isset($rules->support_admin))
						$rules->support_admin = new stdClass();
					$rules->support_admin->{'limeticket.handler.view.products'} = 1;
				} else {
					$qry = "SELECT * FROM #__limeticket_user_prod WHERE user_id = " . (int)$user->id;
					$db->setQuery($qry);
					$rows = $db->loadObjectList();
					$rules->view_products = new stdClass();
					foreach ($rows as $row)
					{
						$key = 'limeticket.handler.view.product.' . $row->prod_id;
				...
				if ($user->alldepts)
				{
					if (!isset($rules->support_admin))
						$rules->support_admin = new stdClass();
					$rules->support_admin->{'limeticket.handler.view.departments'} = 1;
				} else {
					$qry = "SELECT * FROM #__limeticket_user_dept WHERE user_id = " . (int)$user->id;
					$db->setQuery($qry);
					$rows = $db->loadObjectList();
					$rules->view_departments = new stdClass();
					foreach ($rows as $row)
					{
						$key = 'limeticket.handler.view.department.' . $row->ticket_dept_id;
				...
				if ($user->allcats)
				{
					if (!isset($rules->support_admin))
						$rules->support_admin = new stdClass();
					$rules->support_admin->{'limeticket.handler.view.categories'} = 1;
				} else {
					$qry = "SELECT * FROM #__limeticket_user_cat WHERE user_id = " . (int)$user->id;
					$db->setQuery($qry);
					$rows = $db->loadObjectList();
					$rules->view_categories = new stdClass();
					foreach ($rows as $row)
					{
						$key = 'limeticket.handler.view.category.' . $row->ticket_cat_id;
				...
				if ($user->allprods_a)
				{
					if (!isset($rules->support_admin))
						$rules->support_admin = new stdClass();
					$rules->support_admin->{'limeticket.handler.assign.products'} = 1;
				} else {
					$qry = "SELECT * FROM #__limeticket_user_prod_a WHERE user_id = " . (int)$user->id;
					$db->setQuery($qry);
					$rows = $db->loadObjectList();
					$rules->assign_products = new stdClass();
					foreach ($rows as $row)
					{
						$key = 'limeticket.handler.assign.product.' . $row->prod_id;
				...
				if ($user->alldepts_a)
				{
					if (!isset($rules->support_admin))
						$rules->support_admin = new stdClass();
					$rules->support_admin->{'limeticket.handler.assign.departments'} = 1;
				} else {
					$qry = "SELECT * FROM #__limeticket_user_dept_a WHERE user_id = " . (int)$user->id;
					$db->setQuery($qry);
					$rows = $db->loadObjectList();
					$rules->assign_departments = new stdClass();
					foreach ($rows as $row)
					{
						$key = 'limeticket.handler.assign.department.' . $row->ticket_dept_id;
				...
				if ($user->allcats_a)
				{
					if (!isset($rules->support_admin))
						$rules->support_admin = new stdClass();
					$rules->support_admin->{'limeticket.handler.assign.categories'} = 1;
				} else {
					$qry = "SELECT * FROM #__limeticket_user_cat_a WHERE user_id = " . (int)$user->id;
					$db->setQuery($qry);
					$rows = $db->loadObjectList();
					$rules->assign_categories = new stdClass();
					foreach ($rows as $row)
					{
						$key = 'limeticket.handler.assign.category.' . $row->ticket_cat_id;
	...
	function Convertv2()
	{
		...
		{
			$qry = "SELECT * FROM #__assets WHERE name = 'com_limeticket.{$asset_id}'";
			$db->setQuery($qry);
			...
			if (!$object)
			{
				...
				$root->loadByName('com_limeticket');
				$asset->name = "com_limeticket.{$asset_id}";
				$asset->title = "com_limeticket.{$asset_id}";
		...
		$qry = "SELECT * FROM #__assets WHERE name = 'com_limeticket'";
		$db->setQuery($qry);
		$object = $db->loadObject();
		$rules = $object->rules;
		...
		if (!property_exists($rules, "limeticket.view"))
		{
			$rules->{"limeticket.view"} = new stdClass();
			$rules->{"limeticket.view"}->{'1'} = 1;
		}

		$rules = json_encode($rules);

		$qry = "UPDATE #__assets SET rules = '" . LIMETICKETJ3Helper::getEscaped($db, $rules) . "' WHERE name = 'com_limeticket'";
		$db->setQuery($qry);
		$db->Query();


		// default support user rules!	
		$qry = "SELECT * FROM #__assets WHERE name = 'com_limeticket.support_user'";
		$db->setQuery($qry);
		$object = $db->loadObject();
		$rules = $object->rules;
		$rules = json_decode($rules);
		...
		if (!$rules) $rules = new stdClass();
		if (!property_exists($rules, "limeticket.ticket.open") || is_array($rules->{"limeticket.ticket.open"}))
		{
			$rules->{"limeticket.ticket.open"} = new stdClass();
			$rules->{"limeticket.ticket.open"}->{'1'} = 1;
		}

		if (!property_exists($rules, "limeticket.ticket.view") || is_array($rules->{"limeticket.ticket.view"}))
		{
			$rules->{"limeticket.ticket.view"} = new stdClass();
			$rules->{"limeticket.ticket.view"}->{'1'} = 1;
		}

		$rules = json_encode($rules);

		$qry = "UPDATE #__assets SET rules = '" . LIMETICKETJ3Helper::getEscaped($db, $rules) . "' WHERE name = 'com_limeticket.support_user'";
		$db->setQuery($qry);
		$db->Query();
		...
		$qry = "SELECT * FROM #__limeticket_settings WHERE setting = 'v2_upgrade'";
		$db->setQuery($qry);
		$obj = $db->loadObject();
		if ($obj)
		{
		...
		$qry = "INSERT INTO #__limeticket_settings (setting, value) VALUES ('v2_upgrade', '1')";
		$db->setquery($qry);
		$db->query();
	...
	{
		$db = JFactory::getDBO();
		if (LIMETICKET_Helper::TableExists($table))
		{
	...
	function UpdatePluginsTickets()
	{
		...
		$path = JPATH_SITE.DS.'components'.DS.'com_limeticket'.DS.'plugins'.DS.'tickets';
	...
	function UpdatePluginsTicketPrint()
	{
		...
		$path = JPATH_SITE.DS.'components'.DS.'com_limeticket'.DS.'plugins'.DS.'ticketprint';
	...
	function UpdatePluginsUserList()
	{
		...
		require_once( JPATH_SITE.DS.'components'.DS.'com_limeticket'.DS.'views'.DS.'admin_support'.DS.'view.html.php' );
		require_once( JPATH_SITE.DS.'components'.DS.'com_limeticket'.DS.'views'.DS.'admin_support'.DS.'layout.users.php' );

		$path = JPATH_SITE.DS.'components'.DS.'com_limeticket'.DS.'plugins'.DS.'userlist';
		$files = JFolder::files($path, ".php$");
	...
	function UpdatePluginsOpenSearch()
	{
		...
		require_once( JPATH_SITE.DS.'components'.DS.'com_limeticket'.DS.'views'.DS.'ticket'.DS.'task.open.php' );

		$path = JPATH_SITE.DS.'components'.DS.'com_limeticket'.DS.'plugins'.DS.'ticketopensearch';
		$files = JFolder::files($path, ".php$");
		...
		foreach ($files as $file)
		{
			...
			$class = "LIMETICKET_Plugin_OpenSearch_" . $id;
	...
	function UpdatePluginsCron()
	{
		...
		$path = JPATH_SITE.DS.'components'.DS.'com_limeticket'.DS.'plugins'.DS.'cron';
		$files = JFolder::files($path, ".php$");
	...
	function setupCronPlugin($plugin, $id, $title, $enabled)
	{
		...
		$sql = "SELECT * FROM #__limeticket_cron WHERE class = '" . $db->escape($class) . "'";
		$db->setQuery($sql);
		...
		if ($result)
		{
			...
			$sql = "UPDATE #__limeticket_cron SET `interval` = '" . $db->escape($interval) . "' WHERE class = '" . $db->escape($class) . "'";
			$db->setQuery($sql);
			$db->Query();
		} else {
			$sql = "INSERT INTO #__limeticket_cron (cronevent, class, `interval`) VALUES ('" . $db->escape($title) . "', '" . $db->escape($class) . "', '" . $db->escape($interval) . "')";
			$db->setQuery($sql);
			$db->Query();
		}
	}
	...
	function removeMissingPlugins($type, $plugins)
	{
		...
		{
			$sql = "DELETE FROM #__limeticket_plugins WHERE `type` = '" . $db->escape($type) . "' AND name = '" . $db->escape($plugin->name) . "'";
			$db->setQuery($sql);
			$db->Query();
	...
	function loadAllPlugins($type)
	{
		...
		$sql = "SELECT * FROM #__limeticket_plugins WHERE `type` = '" . $db->escape($type) . "'";
		$db->setQuery($sql);
	...
	function setupPlugin($type, $name, $title, $description, $enabled, $settings = '')
	{
		...
		if ($record)
		{
			...
			$sql = "UPDATE #__limeticket_plugins SET title = '" . $db->escape($title) . "', description = '" . $db->escape($description) . "', settingsfile = '" . $db->escape($settings) . "'";
			...
			// create entry
			$sql = "INSERT INTO #__limeticket_plugins (`type`, name, title, description, settingsfile, enabled) VALUES (";
			$sql .= "'" . $db->escape($type) . "', ";
			$sql .= "'" . $db->escape($name) . "', ";
			$sql .= "'" . $db->escape($title) . "', ";
	...
	function convertTicketAdminID()
	{
		...
		try {
			...
			if (LIMETICKET_Helper::TableExists("#__limeticket_user"))
			{
				...
				$qry = "SELECT id, user_id FROM #__limeticket_user";
				$db->setQuery($qry);
				$users = $db->loadObjectList("id");

				$qry = "SELECT id, admin_id FROM #__limeticket_ticket_ticket";
				$db->setQuery($qry);
				...
				foreach ($tickets as $ticket)
				{
					...
					$qry = "UPDATE #__limeticket_ticket_ticket SET admin_id = " . (int)$new_admin_id . " WHERE id = " . (int)$ticket->id;
					$db->setQuery($qry);
					$db->Query();
				}
	...
	{
		$db = JFactory::getDBO();
		$query = "UPDATE #__limeticket_ticket_ticket SET admin_id = 0 WHERE admin_id = 1";
		$db->setQuery($query);
		$db->Query();
