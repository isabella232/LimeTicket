...
<?php echo LIMETICKET_Helper::PageTitle('SUPPORT_ADMIN',"EDIT_GROUP"); ?>
...
<div class="pull-right" style="margin-bottom: 6px;">
	<a href="#" id='limeticket_form_save' class="btn btn-success">
		<i class="icon-apply icon-white"></i>
		<?php echo JText::_('SAVE');?>
	</a>
	<?php if (!$this->creating): ?>
		<a href="#" id='limeticket_form_saveclose' class="btn btn-default">
			<i class="icon-apply"></i>
			<?php echo JText::_('SAVE_AND_CLOSE');?>
		</a>
	<?php endif; ?>
	<a href="#" id='limeticket_form_cancel' class="btn btn-default">
		<i class="icon-cancel"></i>
		<?php echo JText::_('CANCEL');?>
	</a>
	<?php if (!$this->creating && LIMETICKET_Permission::auth("limeticket.groups", "com_limeticket.groups")): ?>
		<a href="#" id='limeticket_form_delete' class="btn btn-default">
...
<?php echo  LIMETICKET_Helper::PageSubTitle('GROUP_DETAILS'); ?>

<form action="<?php echo LIMETICKETRoute::_('index.php?option=com_limeticket&view=admin_groups&what=savegroup',false);?>" method="post" name="groupForm" id="groupForm" class="form-horizontal form-condensed">
	<input type="hidden" name="groupid" value="<?php echo LIMETICKET_Helper::escape($this->group->id); ?>" />
	<input type="hidden" name="what" value="savegroup" />

	<div class="control-group">
		<label class="control-label"><?php echo JText::_("NAME"); ?></label>
		<div class="controls">
			<input class="text_area" type="text" name="groupname" id="groupname" size="32" maxlength="250" value="<?php echo LIMETICKET_Helper::escape($this->group->groupname);?>" required />
		</div>
	</div>	
	...
	<div class="control-group">
		...
		<div class="controls">
			<input class="text_area" type="text" name="description" id="description" size="32" maxlength="250" value="<?php echo LIMETICKET_Helper::escape($this->group->description);?>" />
		</div>
	</div>	
...
<?php if (!$this->creating): ?>
	<div class="pull-right" style="margin-bottom: 6px;">
		<a href="#" id='limeticket_form_add' class="btn btn-default"><i class="icon-new"></i> <?php echo JText::_('ADD');?></a>
		<a href="#" id='limeticket_form_remove' class="btn btn-default"><i class="icon-delete"></i> <?php echo JText::_('REMOVE');?></a>
	</div>
	<?php echo  LIMETICKET_Helper::PageSubTitle('MEMBERS'); ?>

<form action="<?php echo LIMETICKETRoute::_('index.php?option=com_limeticket&view=admin_groups&groupid=' . $this->group->id,false);?>" method="post" name="limeticketForm" id="limeticketForm">
...
<table class="table table-bordered table-condensed table-striped">
    <thead>
        ...
        <tr>
            ...
            <th>
				<?php echo LIMETICKET_Helper::sort('User', 'username', @$this->order_Dir, @$this->order ); ?>
            </th>
            <th>
 				<?php echo LIMETICKET_Helper::sort('ISADMIN', 'isadmin', @$this->order_Dir, @$this->order ); ?>
			</th>
            <th>
 				<?php echo LIMETICKET_Helper::sort('ALL_EMAIL_USER', 'allemail', @$this->order_Dir, @$this->order ); ?>
			</th>
            <th>
				<?php echo LIMETICKET_Helper::sort('ALL_SEE_USER', 'allsee', @$this->order_Dir, @$this->order ); ?>
			</th>
		</tr>
    </thead>
    <?php
    ...
    for ($i=0, $n=count( $this->groupmembers ); $i < $n; $i++)
	{
		...
		$allemail = LIMETICKET_Helper::GetYesNoText($row->allemail);
    	$isadmin = LIMETICKET_Helper::GetYesNoText($row->isadmin);
    ...
    ?>
        <tr class="<?php echo "row$k"; ?>">
			<td>
				<?php if ($userid != $row->user_id || LIMETICKET_Permission::auth("limeticket.groups", "com_limeticket.groups")): ?>
			...
			<td>
				...
				<?php else: ?>	
					<img src="<?php echo JURI::base(); ?>/components/com_limeticket/assets/images/tickgray.png" width="16" height="16" border="0" />
				<?php endif; ?>
			</td>
			<td>
			...
			<td>
				<div class="pull-right">
					<a class="limeticketTip edit_perm" href='#' id="editperm_<?php echo (int)$row->user_id; ?>" title="<?php echo JText::_('EDIT'); ?>">
						<img src="<?php echo JURI::base(); ?>/components/com_limeticket/assets/images/edit.png" width="18" height="18" alt="<?php echo JText::_('EDIT'); ?>"/>
					</a>
				</div>
...
<div class="modal limeticket_modal" id="popup_html" style='display: none; max-width: 560px;margin-left: -280px;'>
	...
	<div class="modal-body">
		...
		?></p>
		<div class="limeticket_gp_item"><a href="#" id="popup_perm_0" class="popup_perm"><?php echo JText::_('INHERITED'); ?></a></div>
		<div class="limeticket_gp_item"><a href="#" id="popup_perm_-1" class="popup_perm"><?php echo JText::_('VIEW_NONE'); ?></a></div>
		<div class="limeticket_gp_item"><a href="#" id="popup_perm_1" class="popup_perm"><?php echo JText::_('VIEW'); ?></a></div>
		<div class="limeticket_gp_item"><a href="#" id="popup_perm_2" class="popup_perm"><?php echo JText::_('VIEW_REPLY'); ?></a></div>
		<div class="limeticket_gp_item"><a href="#" id="popup_perm_3" class="popup_perm"><?php echo JText::_('VIEW_REPLY_CLOSE'); ?></a></div>
	</div>
...
jQuery(document).ready(function () {
	...
	jQuery('#limeticket_form_save').click(function (ev) {
		ev.preventDefault();
		document.groupForm.submit();
	});
	
	jQuery('#limeticket_form_saveclose').click(function (ev) {
		ev.preventDefault();
		document.groupForm.what.value = 'saveclose';
		document.groupForm.submit();
	});

	jQuery('#limeticket_form_remove').click(function (ev) {
		ev.preventDefault();
		//if (document.limeticketForm.boxchecked.value == 0)
		//{
		//	alert("<?php echo JText::_('MUST_SELECT'); ?>");
		//} else {		
			document.limeticketForm.what.value = 'removemembers';
			document.limeticketForm.submit();
		//}
	});

	jQuery('#limeticket_form_add').click(function (ev) {
		ev.preventDefault();
		limeticket_modal_show('<?php echo LIMETICKETRoute::_('index.php?option=com_limeticket&view=admin_groups&tmpl=component&what=pickuser&groupid='. $this->group->id); ?>', true, 720);
		//TINY.box.show({iframe:'<?php echo LIMETICKETRoute::_('index.php?option=com_limeticket&view=admin_groups&tmpl=component&what=pickuser&groupid='. $this->group->id); ?>', width:630, height:440 });
	});
	
	jQuery('#limeticket_form_cancel').click( function (ev) {
		ev.preventDefault();
		window.location = '<?php echo LIMETICKETRoute::_('index.php?option=com_limeticket&view=admin_groups', false); ?>';
	});
	
	jQuery('#limeticket_form_delete').click( function (ev) {
		ev.preventDefault();
	...
	jQuery('.all_email').click(function (ev) {
		...
		var url = '<?php echo LIMETICKETRoute::_('index.php?option=com_limeticket&view=admin_groups&what=toggleallemail&userid=XXUIDXX&groupid=' . $this->group->id, false); ?>';
	...
	jQuery('.is_admin').click(function (ev) {
		...
		var id = jQuery(this).attr('id').split('_')[2];
		var url = '<?php echo LIMETICKETRoute::_('index.php?option=com_limeticket&view=admin_groups&what=toggleadmin&userid=XXUIDXX&groupid=' . $this->group->id, false); ?>';
		url = url.replace('XXUIDXX',id);
		var t = this;
		jQuery(t).html('<?php echo JText::_('PLEASE_WAIT'); ?>');
		jQuery.ajax({
	...
	jQuery('.popup_perm').click( function (ev) {
		...
		var url = '<?php echo LIMETICKETRoute::_('index.php?option=com_limeticket&view=admin_groups&what=setperm&userid=XXUIDXX&perm=XXPXX&groupid=' . $this->group->id, false); ?>';
