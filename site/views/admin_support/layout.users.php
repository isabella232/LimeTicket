...
class LimeticketViewAdmin_Support_Users extends LimeticketViewAdmin_Support
{
	function display($tpl = NULL)
	{
		...
		$this->mode = LIMETICKET_Input::getcmd('mode', 'pick');
		$this->usergroup = LIMETICKET_Input::getInt('usergroup');
		...
		$this->lists = array();
		$this->lists['order_Dir'] = LIMETICKET_Input::getCmd('filter_order_Dir');
		$this->lists['order'] = LIMETICKET_Input::getCmd('filter_order');
		...
		if ($ticket_has_groups && ($this->mode == "user"))
			$this->ticketgroup = "t";
		$this->ticketgroup = LIMETICKET_Input::getcmd('ticketgroup', $this->ticketgroup);
		...
		if ($this->ticketgroup == "t")
		{
			...
			$where[] = "id IN (SELECT user_id FROM #__limeticket_ticket_group_members WHERE group_id IN (" . implode(", ", $group_ids) . "))";
		} elseif ($this->ticketgroup > 0)
		{
			$where[] = "id IN (SELECT user_id FROM #__limeticket_ticket_group_members WHERE group_id = " . $db->escape($this->ticketgroup) . ")";
		}
		...
		$order = LIMETICKET_Input::getCmd('filter_order');
		$dir = LIMETICKET_Input::getCmd('filter_order_Dir', 'asc');
		...
		if ($this->mode != "admin")
		{
			...
			$this->jgroup_select = JHTML::_('select.genericlist',  $this->groups, 'usergroup', 'class="inputbox" size="1" onchange="document.limeticketForm.submit( );"', 'id', 'display', $this->usergroup);
			
			
			// load ticket groups
			$qry = "SELECT * FROM #__limeticket_ticket_group ORDER BY groupname";
			$db->setQuery($qry);
			...
			$this->ticketgroup_select = JHTML::_('select.genericlist',  $this->ticketgroups, 'ticketgroup', 'class="inputbox" size="1" onchange="document.limeticketForm.submit( );"', 'id', 'groupname', $this->ticketgroup);
		}
	...
	private function load_plugins()
	{
		...
		if (empty($this->plugins))
		{
			...
			$path = JPATH_SITE.DS."components".DS."com_limeticket".DS."plugins".DS."userlist".DS;
			...
			foreach ($files as $file)
			{
				...
				if (!LIMETICKET_Helper::IsPluignEnabled("userlist", $id))
					continue;
