...
?>
		...
		<div class="control-group">
			...
			<div class="controls">
				<select name="new_handler" id="new_handler">
					<option value="-1" <?php if (LIMETICKET_Settings::get('forward_product_handler') == 'unchanged') echo "selected"; ?>>
						<?php echo JText::_("Unchanged") ?>
					</option>		
					<option value="-2" <?php if (LIMETICKET_Settings::get('forward_product_handler') == 'auto') echo "selected"; ?>>
						<?php echo JText::_("AUTO_ASSIGN") ?>
					</option>		
					<option value="0" <?php if (LIMETICKET_Settings::get('forward_product_handler') == 'unassigned') echo "selected"; ?>>
		...
		<p>
			<textarea style='width:95%;height:<?php echo (int)((LIMETICKET_Settings::get('support_admin_reply_height') * 15) + 80); ?>px' name='body2' id='body2' class="sceditor" rows='<?php echo (int)LIMETICKET_Settings::get('support_admin_reply_height'); ?>' cols='<?php echo (int)LIMETICKET_Settings::get('support_admin_reply_width'); ?>'></textarea>
		</p>
		...
		<p>
			<textarea style='width:95%;height:<?php echo (int)((LIMETICKET_Settings::get('support_admin_reply_height') * 15) + 80); ?>px' name='body' id='body' class="sceditor" rows='<?php echo (int)LIMETICKET_Settings::get('support_admin_reply_height'); ?>' cols='<?php echo (int)LIMETICKET_Settings::get('support_admin_reply_width'); ?>'></textarea>
		</p>
