...
?>
	...
	<?php elseif ($this->mode == "admin"): ?>
		<?php echo LIMETICKET_Helper::PageTitlePopup("INCLUDE_HANDLER"); ?>
	<?php else: ?>
		<?php echo LIMETICKET_Helper::PageTitlePopup("CHANGE_USER"); ?>
	<?php endif; ?>

	<form action="<?php echo LIMETICKETRoute::_( 'index.php?option=com_limeticket&what=pickuser&view=admin_support&layout=users&tmpl=component', false );?>" method="post" name="limeticketForm" id="limeticketForm">
	
		<div class="input-append">
			<input type="text" class="input-medium" placeholder="<?php echo JText::_("SEARCH"); ?>" name="search" id="filter" value="<?php echo LIMETICKET_Helper::escape($this->search);?>" onchange="document.limeticketForm.submit();"/>
		...
		foreach ($this->users as $user)
		{
			...
			<tr class="<?php echo "row$k"; ?>">
				...
				<td>
					...
					<?php else: ?>
						<a class="btn btn-default btn-mini" href="#" onclick="window.parent.PickUser('<?php echo $user->id; ?>','<?php echo LIMETICKET_Helper::escapeJavaScriptText($user->username); ?>','<?php echo LIMETICKET_Helper::escapeJavaScriptText($user->name); ?>');return false;"><?php echo JText::_("PICK"); ?></a>
					<?php endif; ?>
				</td>
			</tr>
			<?php
		...
		<input type="hidden" name="mode" value="<?php echo LIMETICKET_Helper::escape(LIMETICKET_Input::getCmd('mode')); ?>" />
		<input type="hidden" name="ticketid" value="<?php echo (int)LIMETICKET_Input::getInt('ticketid'); ?>" />
	</form>
...
<div class="modal-footer">
	...
	<?php else: ?>
		<a id="pick_user" class="btn btn-default show_modal_iframe" href="<?php echo LIMETICKETRoute::_("index.php?option=com_limeticket&view=admin_support&layout=createuser&tmpl=component"); ?>" data_modal_width="700">
