...
<?php include $this->snippet(JPATH_SITE.DS.'components'.DS.'com_limeticket'.DS.'views'.DS.'admin_support'.DS.'snippet'.DS.'_ticket_notices.php'); ?>

<?php include $this->snippet(JPATH_SITE.DS.'components'.DS.'com_limeticket'.DS.'views'.DS.'admin_support'.DS.'snippet'.DS.'_ticket_toolbar.php'); ?>

<?php 
if (LIMETICKET_Permission::auth("core.create", "com_limeticket.kb") || LIMETICKET_Permission::auth("core.create", "com_limeticket.faq")) 
	include $this->snippet(JPATH_SITE.DS.'components'.DS.'com_limeticket'.DS.'views'.DS.'admin_support'.DS.'snippet'.DS.'_export.php'); 
?>
...
function title_edit_save()
{
	...
	title_edit_end();
	var url = "<?php echo LIMETICKETRoute::x("&task=update.subject", false); ?>&subject=" + encodeURIComponent(jQuery('#title_input').val());
...
function email_edit_save()
{
	...
	var url = "<?php echo LIMETICKETRoute::x("&task=update.email", false); ?>&email=" + escape(jQuery('#email_input').val());
...
function priority_update()
{
	...
	var url = "<?php echo LIMETICKETRoute::x("&task=update.ticket_pri_id", false); ?>&ticket_pri_id=" + escape(new_pri_id);
...
function category_edit_save()
{
	...
	var url = "<?php echo LIMETICKETRoute::x("&task=update.ticket_cat_id", false); ?>&ticket_cat_id=" + escape(jQuery('#catid').val());
...
<?php if (LIMETICKET_Settings::get('support_lock_time') > 0 && !$this->ticket->isLocked()): ?>
function lock_update(do_get)
{
	if (do_get)
	{
		var url = '<?php echo LIMETICKETRoute::_("&task=update.lock"); ?>'; // FIX LINK
		jQuery('#lock_ticket').load(url);
	}
	
	
	setTimeout ( 'lock_update(true)', <?php echo LIMETICKET_Settings::get('support_lock_time') < 10 ? 10000 : LIMETICKET_Settings::get('support_lock_time')*1000 ; ?> );
}
...
<?php if (LIMETICKET_Settings::get('time_tracking_require_note')): ?>
	if (notes.length < 5)
	{
...
<?php endif; ?>
	...
	var url = '<?php echo LIMETICKETRoute::_("task=update.time", false); ?>&time=' + time + '&notes=' + encodeURIComponent(notes); // FIX LINK
...
jQuery(document).ready(function () {
	jQuery('.editmessage').click(function(ev) {
		...
		if (!edit_time)
		{
			...
			html += "<button onclick='SaveEdit();' class='limeticketTip btn btn-success' title='Save Changes'><i class='icon-save'></i> Save</button>";
			html += "<button onclick='CancelEdit();' class='limeticketTip btn btn-danger' title='Cancel Edit'><i class='icon-cancel'></i> Cancel</button>";
			html += "</span>";

			<?php if (LIMETICKET_Settings::get('allow_edit_no_audit')): ?>
			...
			html += "<button onclick='DeleteMessage();' class='limeticketTip btn btn-warning pull-right' title='Delete Message' style='margin-right: 4px;'><i class='icon-delete'></i></button>";
				
			jQuery('#subject_' + edit_id).html(html);
		
			jQuery('#subject_' + edit_id + ' .limeticketTip').limeticket_tooltip();
		...
		if (jQuery('.ticket_message_id_' + edit_id + ' .ticket_time_date').length > 0)
		{
			...
			edit_CalendarFrom.loadUserLanguage(limeticket_calendar_locale);
			...
			edit_CalendarTo.loadUserLanguage(limeticket_calendar_locale);
...
function SaveEdit()
{
	...
	var url = '<?php echo LIMETICKETRoute::_( 'index.php?option=com_limeticket&view=admin_support&task=update.comment&messageid=XXMIDXX', false ); ?>';
	...
	jQuery('.limeticket_main .limeticketTip').limeticket_tooltip('hide');
...
function DeleteMessage()
{
	...
	{
		var url = '<?php echo LIMETICKETRoute::_( 'index.php?option=com_limeticket&view=admin_support&task=update.delete_message&messageid=XXMIDXX', false ); ?>';
		url = url.replace('XXMIDXX', encodeURIComponent(edit_id));
...
function addCC(user_id, is_admin, is_readonly)
{
	...
	var url = "<?php echo LIMETICKETRoute::_("&ticketid=" . $this->ticket->id . "&task=ticket.addcc", false); ?>&ids=" + escape(ids) + "&is_readonly=" + escape(is_readonly) + "&is_admin=" + escape(is_admin) + "&nocache=" + new Date().getTime();
...
{
	var url = "<?php echo LIMETICKETRoute::x("&ticketid=" . $this->ticket->id . "&task=ticket.addemailcc", false); ?>&email=" + escape(email) + "&nocache=" + new Date().getTime();
	window.location = url;
}
...
function limeticket_refresh_tickets() {
    ...
    jQuery.get(url, function (result) {
        if (!jQuery('#batch_form').is(":visible")) {
            ...
            jQuery('#limeticket_ticket_list').html(result.tickets);
