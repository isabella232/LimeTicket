...
class FssViewAdmin_Support_Settings extends FssViewAdmin_Support
{
	...
	{
		$action = LIMETICKET_Input::getCmd('action');
		
		if ($action == "cancel")
		{
			$mainframe = JFactory::getApplication();
			$link = LIMETICKETRoute::_('index.php?option=com_limeticket&view=admin_support',false);
		...
		if ($action == "save" || $action == "apply")
		{
			...
			{
				$new = LIMETICKET_Input::getString($setting, 0);
				$values->$setting = $new;
			}
			...
			if ($action == "save")
			{
				...
				$link = LIMETICKETRoute::_('index.php?option=com_limeticket&view=admin_support&layout=settings',false);
			}
			$mainframe = JFactory::getApplication();
