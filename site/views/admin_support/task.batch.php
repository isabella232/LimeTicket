...
class Task_Batch extends Task_Helper
{
	...
	{	
		...
		$url = LIMETICKET_Input::GetString("return");
		...
		$new_pri = LIMETICKET_Input::getInt('batch_priority');
		if ($new_pri > 0)
		{
			foreach ($ticket_ids as $ticketid)
		...
		$new_status = LIMETICKET_Input::getInt('batch_status');
		if ($new_status > 0)
		{
			foreach ($ticket_ids as $ticketid)
		...
		if (LIMETICKET_Input::getString('batch_handler') != "")
		{
			foreach ($ticket_ids as $ticketid)
				$tickets[$ticketid]->assignHandler(LIMETICKET_Input::getInt('batch_handler'));
		}	
		
		$should_delete = LIMETICKET_Input::getCmd('batch_status');
