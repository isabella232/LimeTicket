...
class Task_Navigate extends Task_Helper
{
	function next()
	{
		$ticket_id = LIMETICKET_Input::getInt('ticketid');
	...
	function navigateTicket($ticket_id, $dir)
	{
		...
		//echo "Redirect to " . LIMETICKETRoute::_("index.php?option=com_limeticket&view=admin_support&layout=ticket&ticketid=" . $data[$found]) . "<br>";
		//exit;
	
		return $app->redirect(LIMETICKETRoute::_("index.php?option=com_limeticket&view=admin_support&layout=ticket&ticketid=" . $data[$found], false));	
	}
	...
	function navigateFail()
	{
		...
		if ($last_url == "")
		{
			$last_url = LIMETICKETRoute::_("index.php?option=com_limeticket&view=admin_support");
			$posted = array();
		}
?>
