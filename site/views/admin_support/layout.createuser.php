...
class LimeticketViewAdmin_Support_createuser extends LimeticketViewAdmin_Support
{
	function display($tpl = NULL)
	{
		...
		$this->data->name = LIMETICKET_Input::getString("name");
		$this->data->username = LIMETICKET_Input::getString("username");
		$this->data->password = LIMETICKET_Input::getString("password");
		$this->data->password2 = LIMETICKET_Input::getString("password2");
		$this->data->email = LIMETICKET_Input::getString("email");
		...
		if (LIMETICKET_Input::getInt("create"))
		{
			// check form
			$ok = true;
	...
	function assignToTickets($user)
	{
		...
		if ($ticketid > 0)
		{
			...
			$db = JFactory::getDBO();
			$qry = "UPDATE #__limeticket_ticket_ticket SET user_id = " . $id . " WHERE email = '" . $db->escape($email) . "'";
			$db->setQuery($qry);
			$db->Query();
		}
	}
