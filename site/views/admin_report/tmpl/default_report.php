...
<p>
	...
	<div class="pull-right">
		...
		<?php if (JDEBUG || LIMETICKET_Settings::get('debug_reports')): ?>
			<a class="btn btn-default" href="#" onclick='jQuery("#sql").toggle();return false;'><?php echo JText::_('SHOW_SQL'); ?></a>
		<?php endif; ?>
	</div>

	<a class="btn btn-default" href="<?php echo JRoute::_('index.php?option=com_limeticket&view=admin_report'); ?>">
...
<form action="<?php echo JRoute::_('index.php'); ?>" name='report_params' id='report_params'>
	...
	<input type="hidden" name="report" value="<?php echo LIMETICKET_Helper::escape(LIMETICKET_Input::getCmd('report')); ?>" />
...
function ResetElement(tabid)
{
	...
	document.getElementById('link_' + tabid).onmouseover = function() {
		this.style.backgroundColor='<?php echo LIMETICKET_Settings::get('css_hl'); ?>';
	}
...
function ShowTab(tabid)
{
	...
	jQuery.cookie('limeticket_tab_report', tabid);
...
jQuery(document).ready( function () {
	...
	else if (jQuery.cookie('limeticket_tab_report'))
	{
		ShowTab(jQuery.cookie('limeticket_tab_report'));
	} else {
		ShowTab('data');
	}
});
...
table.limeticket_report_table tr.row1 td {
	border-top: 1px solid #ccc;
}

table.limeticket_report_table  td {
	padding: 3px;
}

table.limeticket_report_table  th {
	padding: 3px;
	text-align: left;
}
</style>
