	...
	<table class="table table-condensed table-bordered table-report">
		...
		<tbody>
			...
			<?php if (is_array($group_totals)): ?>
				<?php ShowTotals($this->report, $group_totals, JText::sprintf("LIMETICKET_REPORT_GTOTAL", JText::_($group_base))); ?>
...
function OutputField($field, $value, $row = null)
{
	...
	} elseif ($field->format == "messagetime")
	{
		...
		echo LIMETICKET_Helper::ParseBBCode($value); 								
		...
		$allcf = LIMETICKETCF::GetAllCustomFields(true);
		$custfield = null;
		...
		echo LIMETICKETCF::FieldOutput($custfield,$fielddata,array('report' => 1, 'data' => $row));
