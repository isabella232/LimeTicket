 ...
 * @deprecated 6ebcbb7074b299731606228239f377a4
**/
...
class LimeticketViewAdmin_Report extends LIMETICKETView
{
	var $paths = array();
	...
	{
		$this->paths = array(
			JPATH_SITE.DS.'components'.DS.'com_limeticket'.DS.'plugins'.DS.'reports',
			JPATH_SITE.DS.'components'.DS.'com_limeticket'.DS.'views'.DS.'report'.DS.'reports',
			JPATH_SITE.DS.'components'.DS.'com_limeticket'.DS.'plugins'.DS.'reports'.DS.'custom',
			);	
	...
	function display($tpl = null)
	{
		...
		LIMETICKET_Helper::StylesAndJS(array('cookie'));
		...
		$report = LIMETICKET_Input::getCmd('report');
	...
	function CheckDateTables()
	{
		...
		$qry = "SELECT count(*) as cnt FROM #__limeticket_date_day";
		$db->setQuery($qry);
		$data = $db->loadObject();
		...
		if ($data->cnt < 1)
		{
			for ($i = 0 ; $i < 365*15 ; $i++)
			{
				...
				$qry = "REPLACE INTO #__limeticket_date_day (`date`) VALUES ('".date("Y-m-d", $date)."')";
				$db->setQuery($qry);
				$db->query();
			}
			
			$qry = "UPDATE #__limeticket_date_day SET week = CONCAT(YEAR(`date`), '-', LPAD(WEEK(`date`), 2, '0'))";
			$db->setQuery($qry);
			$db->query();
			
			$qry = "UPDATE #__limeticket_date_day SET month = CONCAT(YEAR(`date`), '-', LPAD(MONTH(`date`), 2, '0'))";
			$db->setQuery($qry);
			$db->query();
			
			$qry = "UPDATE #__limeticket_date_day SET year = YEAR(`date`)";
			$db->setQuery($qry);
			$db->query();
		}
	}	
	...
	{
		if (!LIMETICKET_Permission::auth("limeticket.reports.report." . $report_name, "com_limeticket.reports") && 
			!LIMETICKET_Permission::auth("limeticket.reports.all", "com_limeticket.reports") )
			return LIMETICKET_Admin_Helper::NoPerm();
	...
	function LoadXML($report_name)
	{
		...
		$report = LIMETICKET_Report_Helper::XMLToClass($xml, "FSJ_Report");
		$report->sort();
	...
	function OutputData(&$report)
	{
		...
		if (LIMETICKET_Input::getCmd('output') == "csv")
			$this->outputCSV();
		
		if (LIMETICKET_Input::getCmd("output") == "print")
	...
	{	
		...
		$sep = LIMETICKET_Settings::get('reports_separator');
			...
			foreach ($this->report->field as $field)
			{
				...
				if (isset($field->format))
				{
					...
					} else if ($field->format == "bbcode")
					{
						...
						//$value = LIMETICKET_Helper::ParseBBCode($value, null, true, true); 	
					}
				}
...
class FSJ_Report
{
	...
	function sort()
	{
		foreach ($this->filter as &$filter)
		{
			...
			{
				LIMETICKET_Report_Helper::O2A($filter, "extra");	
	...
	function makeSQL($sql)
	{
		...
		foreach ($this->filter as $filter)
		{
			if (isset($filter->type) && $filter->type == "daterange")
			{
				...
				$to = LIMETICKET_Input::getString("{$filter->name}_to", $this->getDateRangeTo($filter));
				$from = LIMETICKET_Input::getString("{$filter->name}_from", $this->getDateRangeFrom($filter));

				$find = "{" . $filter->name . "_from}";
				$replace = LIMETICKETJ3Helper::getEscaped($db,$from);
				//$sql = str_replace($find, $replace, $sql);
				$parser->SetVar($filter->name . "_from", $replace);

				$find = "{" . $filter->name . "_to}";
				$replace = LIMETICKETJ3Helper::getEscaped($db,$to);
				//$sql = str_replace($find, $replace, $sql);
			...
			} elseif (isset($filter->type) && $filter->type == "date")
			{
				...
				$replace = 1;
				$date = LIMETICKET_Input::getString("{$filter->name}", date("Y-m-d"));
				$find = "{" . $filter->name . "}";
				$replace = LIMETICKETJ3Helper::getEscaped($db,$date);
				$parser->SetVar($filter->name, $replace);

				// need to filter the column by the date rand selected
			} elseif (isset($filter->type) && $filter->type == "lookup") 
			{
				$value = LIMETICKET_Input::getString('filter_' . $filter->name, '');
				$find = "{" . $filter->name . "}";
				if ($value == "")
				{
					//$parser->SetVar($find, "1");
				} else {
					$replace = " {$filter->field} = '" . LIMETICKETJ3Helper::getEscaped($db,$value) . "' "; 
					//$sql = str_replace($find, $replace, $sql);
					
					$parser->SetVar($filter->name, $replace);
				}
			} else {
				$value = LIMETICKET_Input::getString('filter_' . $filter->name, $filter->default);
		...
		foreach ($this->group as $group)
		{
			...
			if ($group->type == "dategroup")
			{
				$value = LIMETICKET_Input::getString('group_' . $group->name, 'day');
		...
		$final_sql = str_replace("#__", LIMETICKET_Helper::dbPrefix(), $final_sql);
	...
	function getFilters()
	{
		...
		foreach ($this->filter as $filter)
		{
			if (isset($filter->type) && $filter->type == "daterange")
			{
				...
				$html[] = "<label class='control-label' for='{$filter->name}_from'>".JText::_('LIMETICKET_FROM')."</label>";
				...
				$html[] = "<div class='control-group'>";
				$html[] = "<label class='control-label' for='{$filter->name}_to'>".JText::_('LIMETICKET_TILL')."</label>";
				$html[] = "<div class='controls'>";
				...
				$js = "
					...
					jQuery(document).ready( function () {
						...
						cal_{$filter->name}.loadUserLanguage('" . LIMETICKET_Translate_Helper::CalenderLocaleCode()  . "');
			...
			} elseif (isset($filter->type) && $filter->type == "date")
			{
				...
				$to = LIMETICKET_Input::getString("{$filter->name}", $this->getDateRangeTo($filter));
			...
			} else if ($filter->type == "lookup")
			{
				...
				if (!empty($filter->translate))
				{
					LIMETICKET_Translate_Helper::Tr($data);
				...
				$value = LIMETICKET_Input::getString('filter_' . $filter->name, isset($filter->default) ? $filter->default : '');
	...
	function listFilterValues()
	{
		...
		foreach ($this->filter as $filter)
		{
			if (isset($filter->type) && $filter->type == "daterange")
			{
				...
				$html[] = "<dt>".JText::_('LIMETICKET_FROM')."</dt>";
				$html[] = "<dd>".$from."</dd>";
				$html[] = "<dt>".JText::_('LIMETICKET_TILL')."</dt>";
				$html[] = "<dd>".$to."</dd>";
...
class LIMETICKET_Report_Helper
{
	...
	static function XMLToClass($xml, $class = null)
	{
		...
		foreach ($xml->children() as $child)
		{
			...
			{ 
				...
				array_push($obj->$name,LIMETICKET_Report_Helper::XMLToClass($child));
			} else {
				$obj->$name = LIMETICKET_Report_Helper::XMLToClass($child);
			}
		}
