...
<?php if (LIMETICKET_Settings::get('user_hide_all_details')): ?>
	...
	<?php echo LIMETICKET_Helper::PageSubTitle(JText::sprintf("TICKET_TITLE_SHORT", $this->ticket->title)); ?>
	
<?php else: ?>

	<?php echo LIMETICKET_Helper::PageSubTitle("TICKET_DETAILS"); ?>

	<?php if ($this->ticket->user_id == 0 && LIMETICKET_Settings::get('support_unreg_password_highlight') == 1): ?>
		<div class="alert alert-info">
		<h4><?php echo JText::_('YOUR_TICKET_ACCESS_DETAILS_ARE_'); ?></h4>
			<ul style="margin-top: 6px;">
			<?php if (LIMETICKET_Settings::get('support_unreg_type') == 0): ?>
				<li><?php echo JText::_('EMAIL'); ?> : <strong><?php echo $this->ticket->email; ?></strong></li>
			<?php endif; ?>
			<?php if (in_array(LIMETICKET_Settings::get('support_unreg_type'), array(1,2)) ): ?>
				<li><?php echo JText::_('REFERENCE'); ?> : <strong><?php echo $this->ticket->reference; ?></strong></li>
			<?php endif; ?>
			<?php if (in_array(LIMETICKET_Settings::get('support_unreg_type'), array(0,1)) ): ?>
	...
	<?php LIMETICKET_Helper::HelpText("support_user_view_after_details"); ?>

	<?php 
	$mc = new LIMETICKET_Multi_Col();
	...
	?>
		...
		<?php if (!LIMETICKET_Settings::get('user_hide_id')): ?>
			...
			<?php if (LIMETICKET_Settings::get('support_unreg_password_highlight') == 2 && in_array(LIMETICKET_Settings::get('support_unreg_type'), array(1,2))): ?>
		...
		<?php if (SupportHelper::userIdMultiUser($this->ticket->user_id) && !LIMETICKET_Settings::get('user_hide_user')) : ?>
			<?php $mc->Item(); ?>
		...
		<?php if (SupportHelper::userIdMultiUser($this->ticket->user_id) && !LIMETICKET_Settings::get('user_hide_cc')) : ?>
			...
			<td colspan="2">
				<?php if (JFactory::getUser()->id == $this->ticket->user_id): ?>
					<a class="pull-right show_modal_iframe" data_modal_width="700" href="<?php echo LIMETICKETRoute::_('index.php?option=com_limeticket&view=ticket&layout=user&tmpl=component&ticketid=' . $this->ticket->id); ?>" id="limeticket_show_userlist">
		...
		<?php if ($this->ticket->password): ?>
			<?php if (in_array(LIMETICKET_Settings::get('support_unreg_type'), array(0,1))): ?>
				...
				<?php if (LIMETICKET_Settings::get('support_unreg_password_highlight') == 2): ?>
			...
			<?php if (LIMETICKET_Settings::get('support_unreg_password_highlight') == 2 && LIMETICKET_Settings::get('support_unreg_type') == 0): ?>
			...
			<?php foreach ($this->ticket->customfields as $field): ?>
				...
				<td colspan="2">
					<?php if ($field['permissions'] == 0 && !$this->ticket->is_closed && $this->CanEditField($field) && $this->ticket->merged == 0): ?>
						<a class='pull-right show_modal_iframe padding-left-small' href="<?php echo LIMETICKETRoute::_("index.php?option=com_limeticket&view=ticket&layout=field&tmpl=component&ticketid=".$this->ticket->id. "&fieldid=" . $field['id'] );// FIX LINK ?>">
		...
		<?php if (!LIMETICKET_Settings::get('user_hide_status')): ?>
			...
			<td colspan="2">
				<?php if (!$this->ticket->is_closed && LIMETICKET_Settings::get('support_user_can_close') && $this->ticket->canclose) : ?>
					<form id='status_change' action="<?php echo LIMETICKETRoute::_( 'index.php?option=com_limeticket&view=ticket&task=update.ticket_status_id' ); ?>" method="post" style="margin: 0px;">
		...
		<?php if (!LIMETICKET_Settings::get('support_hide_priority') && !LIMETICKET_Settings::get('user_hide_priority')) : ?>
			...
			<td colspan="2">
				<?php if (!$this->ticket->is_closed && !$this->ticket->readonly) : ?>
					<form id='pri_change' action="<?php echo LIMETICKETRoute::_( 'index.php?option=com_limeticket&view=ticket&task=update.ticket_pri_id' ); ?>" method="post" style="margin: 0px;">
						<input type="hidden" name="ticketid" value="<?php echo $this->ticket->id; ?>">
		...
		<?php if (LIMETICKET_Settings::get('ratings_ticket') && $this->ticket->isClosed() && $this->ticket->rating > 0): ?>
			<?php $mc->Item(); ?>
			<th style="vertical-align: middle"><?php echo JText::_("RATING"); ?></th>
			<td colspan="2">
				<?php echo SupportHelper::ticketRating($this->ticket, false, LIMETICKET_Settings::get('ratings_ticket_change'), true); ?>
			</td>
		<?php endif; ?>
	...
	<?php LIMETICKET_Helper::HelpText("support_user_view_end_details"); ?>
