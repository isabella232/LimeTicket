...
<form id='newticket' action="<?php echo str_replace("&amp;","&",LIMETICKETRoute::_( '&layout=open' ));// FIX LINK?>" method="post"  enctype="multipart/form-data">
...
if (count($errors) > 0 && LIMETICKET_Settings::get('open_show_error_alert')): ?>

<div class="alert alert-danger limeticket_open_ticket_error_alert">
...
<?php LIMETICKET_Helper::HelpText("support_open_main_header"); ?>

<?php if (LIMETICKET_Settings::get('support_sel_prod_dept') && (isset($this->product) || isset($this->dept))): ?>

	<?php if (isset($this->product) &&  isset($this->dept)): ?>
		<?php echo LIMETICKET_Helper::PageSubTitle("PRODUCT_AND_DEPARTMENT_INFORMATION"); ?>
	<?php elseif (isset($this->product)): ?>
		<?php echo LIMETICKET_Helper::PageSubTitle("PRODUCT_INFORMATION"); ?>
	<?php elseif (isset($this->dept)): ?>
		<?php echo LIMETICKET_Helper::PageSubTitle("DEPARTMENT_INFORMATION"); ?>
	<?php endif; ?>

	<?php LIMETICKET_Helper::HelpText("support_open_main_pd_header"); ?>
			...
			<div class="control-group cg-product">
				...
				<div class="controls product-small">
					<?php if ($this->product->image) : ?>
						<img class="media-object" src="<?php echo JURI::root( true ); ?>/images/limeticket/products/<?php echo $this->product->image; ?>">
					<?php endif; ?>
			...
			<div class="control-group cg-department">
				...
				<div class="controls department-small">
					<?php if ($this->dept->image) : ?>
						<img class="media-object" src="<?php echo JURI::root( true ); ?>/images/limeticket/departments/<?php echo $this->dept->image; ?>">
					<?php endif; ?>
	...
	<?php LIMETICKET_Helper::HelpText("support_open_main_pd_footer"); ?>
	...
	foreach ($this->fields as $field)
{
	...
	if ($field['grouping'] != $grouping)
	{
		...
		?>
			<div id="cf_input_<?php echo $field['id']; ?> cf_input_<?php echo $field['alias']; ?>" class="control-group <?php echo LIMETICKETCF::FieldClass($field); ?> <?php if (LIMETICKETCF::HasErrors($field, $this->errors)) echo "error"; ?>">
...
<div class="limeticket_message_details_header">
<?php echo LIMETICKET_Helper::PageSubTitle("MESSAGE_DETAILS"); ?>
</div>

<?php $has_message_details = false; ?>

	<?php LIMETICKET_Helper::HelpText("support_open_main_md_header"); ?>

	<div class="form-horizontal form-condensed">
		
		<?php if (LIMETICKET_Settings::get('support_subject_message_hide') != "subject" && LIMETICKET_Settings::get('support_subject_message_hide') != "both" && LIMETICKET_Settings::get('support_subject_at_top')): ?>
...
<div class="controls">
					<input type="text" class="input-xlarge" name='subject' id='subject' size='<?php echo LIMETICKET_Settings::get('support_subject_size'); ?>' value="<?php echo LIMETICKET_Helper::escape($this->ticket['title']) ?>" required>
		...
		<?php if (!LIMETICKET_Settings::get('support_hide_tags') && $this->admin_create > 0): ?>
				...
				<div class="controls">
					...
					<div id="tags">
						<?php foreach ($tags as $tag): ?>
							<div class="limeticket_tag label label-info" id="tag_<?php echo preg_replace("/[^a-zA-Z0-9]+/", "-", $tag); ?>">
...
<?php if (count($this->cats) > 0 && !LIMETICKET_Settings::get('support_hide_category')): ?>	
	<div class="control-group cg-category <?php echo $this->errors['cat'] ? 'error' : ''; ?>">
		...
		<div class="controls">
			...
			<select id='catid' class='input-large' name='catid' <?php if (LIMETICKET_Settings::get('support_altcat')) echo "required" ?>>
				<?php if (LIMETICKET_Settings::get('support_altcat')): ?>
...
<?php if ($this->captcha != ""): ?>
	<div class="control-group cg-captcha <?php echo $this->errors['captcha'] ? 'error' : ''; ?>">
		<label class="control-label"><?php echo JText::_("LIMETICKET_CAPTCHA"); ?></label>
		<div class="controls">
			<span id="captcha_cont"><?php echo $this->captcha ?></span>
			<span class="help-inline"><?php echo $this->errors['captcha'] ? $this->errors['captcha'] : LIMETICKET_Helper::HelpText("support_open_main_field_captcha"); ?></span>
		</div>
	</div>
<?php endif; ?>

<?php if (LIMETICKET_Settings::get('support_choose_handler') == "user" || ($this->admin_create > 0 && LIMETICKET_Settings::get('support_choose_handler') == "admin") || (LIMETICKET_Settings::get('support_choose_handler') == "handlers" && LIMETICKET_Permission::auth("limeticket.handler", "com_limeticket.support_admin"))) : ?>
			...
			<div class="controls">
				<select class='input-large' id='handler' name='handler'>
					...
					<?php if ($this->admin_create > 0): ?>
						...
						<option value="0" <?php if (!LIMETICKET_Settings::get('support_assign_for_user')) echo " selected='selected'"; ?> ><?php echo JText::_("AUTO_ASSIGN"); ?></option>
						<option value="<?php echo JFactory::getUser()->id; ?>" <?php if (LIMETICKET_Settings::get('support_assign_for_user')) echo " selected='selected'"; ?> ><?php echo JFactory::getUser()->name; ?></option>
				...
				<span class="help-inline"><?php LIMETICKET_Helper::HelpText("support_open_main_field_handler"); ?></span>
			</div>
		</div>
	<?php $has_message_details = true; ?>
	<?php else: ?>
	...
	<?php endif; ?>
<?php else: ?>	
	<?php if (LIMETICKET_Settings::get('support_assign_for_user') && $this->admin_create > 0): ?>
...
<?php if ($this->admin_create > 0): ?>
	...
	<script>	
		...
		{
			var url = '<?php echo JRoute::_('index.php?option=com_limeticket&view=admin_support&layout=reply&task=canned.dolist&ticketid=' . 0, false); ?>';
			jQuery('#canned_replies').load(url);
	
	
			jQuery('.canned_list').html("<div class='pull-right'><?php echo JText::_('PLEASE_WAIT'); ?></div>");

			var cr_url = '<?php echo JRoute::_('index.php?option=com_limeticket&view=admin_support&layout=reply&task=canned.dropdown&ticketid=' . 0, false); ?>';
	...
	<?php LIMETICKET_Helper::IncludeModal(); ?>

<?php endif; ?>


<?php if (LIMETICKET_Settings::get('support_subject_message_hide') != "message" && LIMETICKET_Settings::get('support_subject_message_hide') != "both"): ?>
	
	<?php if (LIMETICKET_Settings::get('open_search_enabled') == 4): ?>
...
<?php if (!$has_message_details): ?>
	<style>
	.limeticket_message_details_header {
		display: none;
	}
	</style>
<?php endif; ?>

<?php LIMETICKET_Helper::HelpText("support_open_main_md_footer"); ?>

<?php if (LIMETICKET_Settings::get('support_user_attach')): ?>

	<?php LIMETICKET_Helper::HelpText("support_open_main_file_header"); ?>
	<div class="form-horizontal form-condensed">
	...
	<?php LIMETICKET_Helper::HelpText("support_open_main_file_footer"); ?>
<?php endif; ?>

<?php LIMETICKET_Helper::HelpText("support_open_main_buttons_before"); ?>
...
<?php if ($this->prodid > 0 || $this->deptid > 0): ?>
	...
	?>
	<?php if (LIMETICKET_Input::getInt('admincreate') > 0): ?>
		<?php if ($use_prod): ?>	
			<a class='btn btn-default backprod' href="<?php echo LIMETICKETRoute::_('index.php?option=com_limeticket&view=ticket&layout=open&prodid='.$this->prodid.'&admincreate=' . LIMETICKET_Input::getInt('admincreate')); ?>"><?php echo JText::_("BACK"); ?></a>
		<?php else: ?>
			<a class='btn btn-default backprod' href="<?php echo LIMETICKETRoute::_('index.php?option=com_limeticket&view=ticket&layout=open&admincreate=' . LIMETICKET_Input::getInt('admincreate')); ?>"><?php echo JText::_("BACK"); ?></a>
		<?php endif; ?>
	<?php else : ?>
		<?php if ($use_prod): ?>	
			<a class='btn btn-default backprod' href="<?php echo LIMETICKETRoute::_('index.php?option=com_limeticket&view=ticket&layout=open&prodid='.$this->prodid); ?>"><?php echo JText::_("BACK"); ?></a>
		<?php else: ?>
			<a class='btn btn-default backprod' href="<?php echo LIMETICKETRoute::_('index.php?option=com_limeticket&view=ticket&layout=open'); ?>"><?php echo JText::_("BACK"); ?></a>
		<?php endif; ?>
	<?php endif; ?>
<?php endif; ?>
...
<?php LIMETICKET_Helper::HelpText("support_open_main_buttons_after"); ?>
...
function tag_add(tagname)
{
	...
	{
		var id = tagname.replace(/[^a-zA-Z0-9]+/g, '-');
		html = '<div class="limeticket_tag label label-info" id="tag_' + id + '">';
