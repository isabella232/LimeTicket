...
?>
	...
	<?php if ($multitype || !$has_cats): ?>
		...
		<?php foreach ($this->products as $product): ?>
			<?php if ($multitype && $curtype != $product->maxtype) : ?>
				...
				<?php echo $product->maxtype == 0 ? LIMETICKET_Helper::PageSubTitle2("OTHER_PRODUCTS") : LIMETICKET_Helper::PageSubTitle2("MY_PRODUCTS") ?>
			...
			<?php include $this->snippet(JPATH_SITE.DS.'components'.DS.'com_limeticket'.DS.'views'.DS.'ticket'.DS.'snippet'.DS.'_prod.php'); ?>
		...
		<?php foreach ($this->products as $product): ?>
			...
			<?php if ($product->category != $category): ?>
				...
				<?php else: ?>
					...
					<h4 class="product_category_header">
						<?php echo LIMETICKET_Settings::get('support_open_cat_prefix'); ?>
						<?php if (LIMETICKET_Settings::get('support_open_accord')): ?>
							<a data-toggle="collapse" href="#prodcat<?php echo $cid; ?>"><?php echo $product->category; ?></a>
						<?php else : ?>
							<?php echo $product->category; ?>
						<?php endif; ?>
					</h4>
					<div class="product_category_indent <?php if (LIMETICKET_Settings::get('support_open_accord')) echo 'collapse' ?>" id="prodcat<?php echo $cid++; ?>">
				<?php endif; ?>
			...
			<?php if ($product->subcat != $subcat): ?>
				...
				<?php else: ?>
					...
					<div class="product_subcat_indent <?php if (LIMETICKET_Settings::get('support_open_accord')) echo 'collapse' ?>" id="prodcat<?php echo $cid++; ?>">
				<?php endif; ?>
	...
	<?php if (LIMETICKET_Settings::Get('ticket_prod_per_page') > 0) echo $this->pagination->getListFooter(); ?>