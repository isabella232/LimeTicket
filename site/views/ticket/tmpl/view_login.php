...
<?php endif; ?>
		...
		<ul class="nav nav-tabs">
			...
			<li>
				<a class='ffs_tab limeticket_tab_selected' href='<?php echo LIMETICKETRoute::_( 'index.php?option=com_limeticket&view=ticket&layout=open' ); ?>'>
					<?php echo JText::_('OPEN_NEW_TICKET'); ?>
				</a>
			</li>
			<?php endif; ?>
			<li class="active">
				<a class='ffs_tab' href='<?php echo LIMETICKETRoute::_( 'index.php?option=com_limeticket&view=ticket' );// FIX LINK ?>'>
					<?php echo JText::_('VIEW_TICKET'); ?>
				</a>
			</li>
		</ul>
		
<?php LIMETICKET_Helper::HelpText("support_view_login_header"); ?>

<?php if (LIMETICKET_Settings::get('support_no_logon') == 0): ?>
	<?php include $this->snippet(JPATH_SITE.DS.'components'.DS.'com_limeticket'.DS.'views'.DS.'ticket'.DS.'snippet'.DS.'_login_form.php'); ?>
<?php endif; ?>

	<?php if (LIMETICKET_Settings::get('support_no_register') == 2): ?>
	<?php include $this->snippet(JPATH_SITE.DS.'components'.DS.'com_limeticket'.DS.'views'.DS.'ticket'.DS.'snippet'.DS.'_register_form.php'); ?>
<?php endif; ?>

<?php if (LIMETICKET_Settings::get('support_allow_unreg') > 0): ?>
	<?php echo LIMETICKET_Helper::PageSubTitle("VIEW_TICKET_CREATED_WITHOUT_ACCOUNT"); ?>
	...
	<?php elseif (LIMETICKET_Settings::get('support_unreg_type') == 1): ?>
		<p><?php echo JText::_("PLEASE_ENTER_YOUR_TICKET_REFERENCE_AND_PASSWORD_PROVIDED_WITH_YOUR_TICKET"); ?></p>
	<?php elseif (LIMETICKET_Settings::get('support_unreg_type') == 2): ?>
		<p><?php echo JText::_("PLEASE_ENTER_YOUR_TICKET_REFERENCE_PROVIDED_WITH_YOUR_TICKET"); ?></p>
	<?php endif; ?>

	<form action="<?php echo LIMETICKETRoute::_("index.php?option=com_limeticket&view=ticket&layout=view"); ?>"  method="post" name="uregform" id="uregform" class="form-horizontal form-condensed">
		...
		<div class="control-group">
			<div class="controls">
				...
				<button onclick="jQuery('#unreg_task').val('view.passlist');" type="submit" class="btn btn-default limeticketTip" title="<?php echo JText::_('FORGOTTEN_YOUR_PASSWORD__ENTER_YOUR_EMAIL_ABOVE_AND_USE_THIS_BUTTON_TO_EMAIL_YOURSELF_A_LIST_OF_YOUR_SUPPORT_TICKETS'); ?>"><?php echo JText::_("EMAIL_TICKET_LIST"); ?></a>
			</div>
		</div>
