 ...
 * @deprecated bb61b4689b6080fb352ab7f376f7a1a1
**/
...
class LimeticketViewKb extends LIMETICKETView
{
    ...
    {
		if (!LIMETICKET_Permission::auth("limeticket.view", "com_limeticket.kb"))
			return LIMETICKET_Helper::NoPerm();	
		
		$mainframe = JFactory::getApplication();

  		require_once (JPATH_SITE.DS.'components'.DS.'com_limeticket'.DS.'helper'.DS.'content'.DS.'kb.php');
		$this->content = new LIMETICKET_ContentEdit_KB();
		$this->content->Init();
		...
		if (!LIMETICKET_Settings::get( 'kb_show_views' ))
			$what = "";
		if ($what == "recent" && !LIMETICKET_Settings::get( 'kb_show_recent' ))
			$what = "";
		if ($what == "viewed" && !LIMETICKET_Settings::get( 'kb_show_viewed' ))
			$what = "";
		if ($what == "rated" && !LIMETICKET_Settings::get( 'kb_show_rated' ))
			$what = "";

		$user = JFactory::getUser();
		$userid = $user->id;
		/*$db = JFactory::getDBO();
		$query = "SELECT * FROM #__limeticket_user WHERE user_id = '".LIMETICKETJ3Helper::getEscaped($db, $userid)."'";
		$db->setQuery($query);
		...
		$search = LIMETICKET_Input::getString('kbsearch');     
		if ($search != "")
		...
		$kbartid = LIMETICKET_Input::getInt('kbartid');            
        if ($kbartid > 0)
        	return $this->showArt();       
        
		$catid = LIMETICKET_Input::getInt('catid');  
        if ($catid > 0)
       		return $this->showCat();       
    ...
    function showCat()
    {
    	...
    	$aparams = LIMETICKET_Settings::GetViewSettingsObj('kb');
		...
		if (!$this->cat && LIMETICKET_Input::getInt('catid') > 0)
			return JError::raiseError(404, 'Category Not Found');
		
		$this->product = $this->get("Product");
		if (!$this->product && LIMETICKET_Input::getInt('prodid') > 0)
		...
		if (LIMETICKET_Helper::NeedBaseBreadcrumb($pathway, array( 'view' => 'kb' )))	
			$pathway->addItem(JText::_('KNOWLEDGE_BASE'), LIMETICKETRoute::_( 'index.php?option=com_limeticket&view=kb' ) );

		$prodid = "";
        if ($this->product['title'])
        {
			$prodid = $this->product['id'];
			$pathway->addItem($this->product['title'], LIMETICKETRoute::_( '&catid=&prodid=' . $this->product['id'] ) );// FIX LINK
		}
    
	    $cat = $this->cat;
		$max = 3;
		
		$selectedcatid = LIMETICKET_Input::getInt('catid');
		
		while ($cat['parcatid'] > 0 && $max-- > 0)
		{
			JRequest::setVar('catid', $cat['parcatid']);
			$cat = $this->get("Cat");
			$pathway->addItem($cat['title'], LIMETICKETRoute::_( "&kbartid=prodid={$prodid}&catid={$cat['id']}" ) );// FIX LINK
		}
		
		JRequest::setVar('catid', $selectedcatid);
		
		$this->base_url = "index.php?option=com_limeticket&view=kb&catid=" . $selectedcatid;
    ...
    function showWhat()
    {
        ...
        if ($this->product['title'])
        {
			$pathway->addItem($this->product['title'], LIMETICKETRoute::_( '&catid=what=prodid=' . $this->product['id'] ) );// FIX LINK
		}
        
        if ($this->cat['title'])
        {
			$pathway->addItem($this->cat['title'], LIMETICKETRoute::_( '&catid=' . $this->cat['id'] . '&what=prodid=' . $this->product['id'] ) );// FIX LINK
		}
	...
	function showProd()
	{
		...
		$this->base_url = "index.php?option=com_limeticket&view=kb&prodid=" . $this->product['id'];
		//echo "Base : " . $this->base_url . "<br>";
		
		// check for a single category and no uncat arts
		if (LIMETICKET_Settings::get('kb_auto_open_single_cat'))
		{
			$cat_count = 0;
			$catid = -1;
			foreach ($this->cats as $cat)
			{
				if (  (array_key_exists("subcats",$cat) && count($cat['subcats']) > 0) || (array_key_exists("arts",$cat) && count($cat['arts'])) || $this->view_mode_cat == "normal" || LIMETICKET_Input::getInt('catid') == $cat['id'] )
				{
			...
			{
				$link = LIMETICKETRoute::x("&catid=" . $catid);
	...
	function showArt()
	{
		...
		$this->comments = new LIMETICKET_Comments('kb',$kbartid);
		$this->comments->PerPage(LIMETICKET_Settings::Get('kb_comments_per_page'));

		if ($this->comments->Process())
			return;			

		$this->assign( 'kb_rate', LIMETICKET_Settings::get( 'kb_rate' ));
		$this->assign( 'kb_comments', LIMETICKET_Settings::get( 'kb_comments' ));
		//$this->assign( 'kb_comments_captcha', LIMETICKET_Settings::get( 'kb_comments_captcha' ));
		//$this->assign( 'kb_comments_moderate', LIMETICKET_Settings::get( 'kb_comments_moderate' ));
		
		$this->base_url = "index.php?option=com_limeticket&view=kb&kbartid=" . $kbartid;

		$error = "";
		
		$rate = LIMETICKET_Input::getCmd('rate');   
		...
		if (LIMETICKET_Settings::get('kb_use_content_plugins'))
		{
			...
			$results = $dispatcher->trigger('onContentPrepare', array ('com_limeticket.kb', & $art, &$this->params, 0));
			$results = $dispatcher->trigger('onContentBeforeDisplay', array ('com_limeticket.kb', & $art, &$this->params, 0));
        ...
        {
			$prodid = $this->product['id'];
            $pathway->addItem($this->product['title'], LIMETICKETRoute::_( 'index.php?option=com_limeticket&view=kb&prodid=' . $this->product['id'] ) );
		}
        ...
        if ($this->cat['title'])
        {
			...
			while ($cat['parcatid'] > 0 && $max-- > 0)
			{
				JRequest::setVar('catid', $cat['parcatid']);
				$cat = $this->get("Cat");
				$pathway->addItem($cat['title'], LIMETICKETRoute::_( "index.php?option=com_limeticket&view=kb&prodid={$prodid}&catid={$cat['id']}" ) );
			}
			//print_p($this->cat);
            $pathway->addItem($this->cat['title'], LIMETICKETRoute::_( "index.php?option=com_limeticket&view=kb&prodid={$prodid}&catid={$this->cat['id']}" ) );
		}
		...
		{
			$db = JFactory::getDBO();
			$query = "UPDATE #__limeticket_kb_art SET views = views + 1 WHERE id = " . LIMETICKETJ3Helper::getEscaped($db, $this->art['id']);
			$db->setQuery($query);
			$db->Query();	
		}
		
		if (LIMETICKET_Input::getCmd('only') == "article")
		{
			parent::display('only');
			exit;
		} else {
	...
	function handlePages()
	{
		...
		$limitstart = LIMETICKET_Input::getInt('page');
		...
		if ($limitstart > 0)
		{
			$this->pages_footer .= "<a href='". LIMETICKETRoute::X("&page=" . ($limitstart - 1) ) . "'>&lt;&lt; ";
	...
	function buildTOC($matches)
	{
		...
		<li>
			...
			<a href="'. LIMETICKETRoute::X("&page=0") .'" class="'.$class.'">'
			. $this->art['title'] .
			'</a>
		...
		foreach ($matches as $bot) {
			$link = LIMETICKETRoute::X("&page=" . ($i-1));
		...
		$this->toc .= '
				<li>
					<a href="'. LIMETICKETRoute::X("&page=-1") .'" class="'.$class.'">'
	...
	{
		$db = JFactory::getDBO();
		$qry = "SELECT * FROM #__limeticket_kb_art WHERE allprods = 1 AND published = 1 AND ";
		...
		$qry = "SELECT p.prod_id FROM #__limeticket_kb_art_prod as p LEFT JOIN #__limeticket_kb_art as a ON a.id = p.kb_art_id GROUP BY p.prod_id";
		$db->setQuery($qry);
	...
	function showMain()
	{
		...
		$this->base_url = "index.php?option=com_limeticket&view=kb";
    ...
    function RateArticle($kbartid, $rate)
    {
    	...
    	$query = 'SELECT id, rating, ratingdetail FROM #__limeticket_kb_art WHERE id = "' . LIMETICKETJ3Helper::getEscaped($db, $kbartid) . '"';
        $db->setQuery($query);
        $row = $db->loadAssoc();
		...
		$query = 'UPDATE #__limeticket_kb_art SET rating = "' . LIMETICKETJ3Helper::getEscaped($db, $rating) . '", ratingdetail = "' . LIMETICKETJ3Helper::getEscaped($db, $ratingdetail) . '" WHERE id = "' . LIMETICKETJ3Helper::getEscaped($db, $kbartid) . '"';
	...
	function downloadFile()
	{
        ...
        $db = JFactory::getDBO();
    	$query = 'SELECT * FROM #__limeticket_kb_attach WHERE id = "' . LIMETICKETJ3Helper::getEscaped($db, $fileid) . '"';
        $db->setQuery($query);
        $row = $db->loadAssoc();
        
        
		$filename = LIMETICKET_Helper::basename($row['filename']);
	    $file_extension = strtolower(substr(strrchr($filename,"."),1));
	    $ctype = LIMETICKET_Helper::datei_mime($file_extension);
		
		if (ob_get_level() > 0)
			ob_end_clean();

		$file = JPATH_SITE.DS.LIMETICKET_Helper::getAttachLocation().DS.'kb'.DS.$row['diskfile'];

		if (!file_exists($file))
			$file = JPATH_SITE.DS.LIMETICKET_Helper::getAttachLocation().DS.$row['diskfile'];
		...
		if (!file_exists($file))
		{
			...
			echo "<ul>";
			echo "<li>" . JPATH_SITE.DS.LIMETICKET_Helper::getAttachLocation().DS.'kb' . "</li>";
			echo "<li>" . JPATH_SITE.DS.LIMETICKET_Helper::getAttachLocation() . "</li>";
			echo "</ul>";
			return;
		}
	...
	function notEnoughArticles()
	{
		...
		$db = JFactory::getDBO();
		$sql = "SELECT count(*) as cnt FROM #__limeticket_kb_art";
		$db->setQuery($sql);
