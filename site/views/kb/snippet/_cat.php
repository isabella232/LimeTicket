...
<?php if (  (array_key_exists("subcats",$cat) && count($cat['subcats']) > 0) || (array_key_exists("arts",$cat) && count($cat['arts'])) || $this->view_mode_cat == "normal" || LIMETICKET_Input::getInt('catid') == $cat['id'] ): ?>
	...
	if ($can_show): ?>
		<div class='media' >
			<?php if ($cat['image']) : ?>
			<a class='pull-left' href="<?php if ($this->view_mode_cat == "accordian"): ?>#<?php else: ?><?php echo LIMETICKETRoute::_( $this->base_url . '&catid=' . $cat['id'] );// FIX LINK?><?php endif; ?>" onclick="return false;"
				...
				<?php endif; ?>
				>
				<?php if ($catdepth > 1 && LIMETICKET_Settings::get('kb_smaller_subcat_images')): ?>
					<img class='media-object' src='<?php echo JURI::root( true ); ?>/images/limeticket/kbcats/<?php echo LIMETICKET_Helper::escape($cat['image']); ?>' width='32' height='32'>
				<?php else : ?>
					<img class='media-object' src='<?php echo JURI::root( true ); ?>/images/limeticket/kbcats/<?php echo LIMETICKET_Helper::escape($cat['image']); ?>' width='64' height='64'>
				<?php endif; ?>
			</a>
			<?php endif; ?>
					...
					<div 
						...
						<h4 class='media-heading' <?php if ($this->view_mode_cat == "accordian") echo " style='padding-top:6px;padding-bottom:6px;' "; ?>>
							...
							<?php if ($this->view_mode_cat == "popup") : ?>
								...
								<a class="show_modal_iframe" href='<?php echo LIMETICKETRoute::_( $this->base_url . '&tmpl=component&catid=' . $cat['id'] . '&view_mode=' . $this->view_mode_incat );// FIX LINK ?>'>
									<?php echo $cat['title'] ?>
								</a>
							...
							<?php else: ?>
								...
								<?php if ($this->view_mode_cat == "normal"): ?>
									<a href='<?php echo LIMETICKETRoute::_( $this->base_url . '&catid=' . $cat['id'] );?>'><?php echo $cat['title'] ?></a>
								<?php else: ?>
									<?php echo $cat['title'] ?>
								<?php endif; ?>
