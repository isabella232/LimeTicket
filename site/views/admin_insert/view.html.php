 ...
 * @deprecated 66123983403a14307ccc459879cb93b4
**/
...
class LimeticketViewAdmin_Insert extends LIMETICKETView
{
	function display($tpl = null)
	{
		...
		$this->editor = LIMETICKET_Input::getCmd('editor');
	...
	{
		$xmlfile = JPATH_ROOT.DS.'components'.DS.'com_limeticket'.DS.'plugins'.DS.'picktable'.DS."{$table}.xml";
	...
	function LoadData()
	{
		...
		if ($this->search != "")
		{
			foreach ($this->xml->filters->search->field as $field)
			{
				...
				$where[] = "$field LIKE '%" . LIMETICKETJ3Helper::getEscaped($db, $this->search) . "%'";	
			}	
		}
		...
		foreach ($this->xml->filters->filter as $filter)
		{
			...
			if ($type == "lookup")
			{
				...
				$value = trim(LIMETICKET_Input::getString($var));
				$this->$var = LIMETICKET_Input::getString($var);
	
				if ($value != "")
					$where[] = "$field = '" . LIMETICKETJ3Helper::getEscaped($db, $value) . "'";
			}
		}
		...
		$this->orderdir = LIMETICKET_Input::getCmd('filter_order_Dir','ASC');
...
<?php
		...
		foreach ($this->xml->filters->filter as $filter)
		{
			...
			if ($type == "lookup")
			{
				...
				echo "<select name='filter_{$filter_id}' id='filter_{$filter_id}' onchange='jQuery(\"#limeticketForm\").submit()'>";
			...
			<?php foreach ($this->xml->displayfields->field as $field): ?>
				<td>
					...
					if ($field->attributes()->link)
					{
						...
						$link = LIMETICKETRoute::x(str_replace("%ID%", $row->$keyfield, $link), false);
...
jQuery(document).ready(function () {
	jQuery('.pick_link').click(function (ev) {
		...
		window.parent.insertLink(url, title, '<?php echo LIMETICKET_Input::getCmd('editor'); ?>');
	});
});
</script>
<?php
