...
<div id="recaptcha_public_key" style="display:none;"><?php echo LIMETICKET_Settings::get('recaptcha_public'); ?></div>
	...
	<div class='limeticket_kb_comment_add' id='add_comment'>
		...
		<?php echo LIMETICKET_Helper::PageSubTitle2($this->add_a_comment); ?>

		<form id='addcommentform' action="<?php echo LIMETICKETRoute::_( 'index.php?option=com_limeticket&view=comments&tmpl=component' );?>" method="post" class="form-horizontal form-condensed">
			...
			<div class="control-group <?php echo $this->errors['name'] ? 'error' : ''; ?>">
				...
				<div class="controls">
					<?php if (!LIMETICKET_Permission::CanModerate() && $this->loggedin) : ?>
						<input name='name' type='hidden' id='comment_name' value='<?php echo LIMETICKET_Helper::escape($this->post['name']) ?>' placeholder="<?php echo JText::_('Name'); ?>" required>
						<?php echo $this->post['name']; ?>
					<?php else: ?>
						<input type="text" name='name' id='comment_name' value='<?php echo LIMETICKET_Helper::escape($this->post['name']) ?>'  placeholder="<?php echo JText::_('Name'); ?>" required>
					<?php endif; ?>
			...
			<?php if ($this->use_email && !($this->loggedin)): ?>
				<div class="control-group <?php echo $this->errors['email'] ? 'error' : ''; ?>">
					...
					<div class="controls">
						<input type="text" name='email' value='<?php echo LIMETICKET_Helper::escape($this->post['email']) ?>' placeholder="<?php echo JText::_('EMail'); ?>">
			...
			<?php if ($this->use_website): ?>
				<div class="control-group <?php echo $this->errors['website'] ? 'error' : ''; ?>">
					...
					<div class="controls">
						<input type="text" name='website' value='<?php echo LIMETICKET_Helper::escape($this->post['website']) ?>' placeholder="<?php echo JText::_('website'); ?>">
			...
			<div class="control-group <?php echo $this->errors['body'] ? 'error' : ''; ?>">
				...
				<div class="controls">
					<textarea name='body' rows='5' cols='60' id='comment_body' style="height:150px;width:400px" required><?php echo LIMETICKET_Helper::escape($this->post['body']) ?></textarea>
