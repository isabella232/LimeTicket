...
class LimeticketModelKb extends JModelLegacy
{
    var $_total = null;
    ...
    function __construct()
	{
		...
		{
			$limit = $mainframe->getUserStateFromRequest('global.list.limit_prod', 'limit', LIMETICKET_Settings::Get('kb_prod_per_page'), 'int');
		...
		{
			$limit = $mainframe->getUserStateFromRequest('global.list.limit_art', 'limit', LIMETICKET_Settings::Get('kb_art_per_page'), 'int');
    ...
    function &getCats()
    {
        ...
        if ($prodid > 0)
        {
        	$query1 = "SELECT * FROM #__limeticket_kb_cat WHERE published = 1 AND id IN (";
        	$query1 .= "SELECT a.kb_cat_id FROM #__limeticket_kb_art as a LEFT JOIN #__limeticket_kb_art_prod as p ON a.id = p.kb_art_id WHERE p.prod_id = '" . LIMETICKETJ3Helper::getEscaped($db, $prodid) . "'";
			...
			$query2 = "SELECT * FROM #__limeticket_kb_cat WHERE published = 1 AND id IN (";
			$query2 .= "SELECT a.kb_cat_id FROM #__limeticket_kb_art as a WHERE a.allprods = '1'";
            ...
            $query = "SELECT * FROM #__limeticket_kb_cat WHERE published = 1";
	...
	function &getCatsForProd()
	{
		...
		if ($prodid > 0)
		{
			$qry1 = "SELECT a.kb_cat_id FROM #__limeticket_kb_art as a LEFT JOIN #__limeticket_kb_art_prod as p ON a.id = p.kb_art_id WHERE p.prod_id = '" . LIMETICKETJ3Helper::getEscaped($db, $prodid) . "' AND published = 1";
			...
			$qry2 = "SELECT a.kb_cat_id FROM #__limeticket_kb_art as a WHERE a.allprods = '1' AND published = 1 ";
			...
			if (count($catids) > 0)
			{
				$query = "SELECT parcatid FROM #__limeticket_kb_cat WHERE id IN (".implode(", ",$catids).") AND parcatid > 0";
				$db->setQuery($query);
				$rows = $db->loadAssocList();
				foreach($rows as &$rows)
				{
    ...
    function &getCatsArts()
    {
		...
		$where = array();
		if (LIMETICKET_Permission::auth("core.edit", "com_limeticket.kb")) // we have editor so can see all unpublished arts
		{
			
		} else if (LIMETICKET_Permission::auth("core.edit.own", "com_limeticket.kb")){
        ...
        if ($prodid > 0)
		{
			$query1 = "SELECT a.id, a.title, a.kb_cat_id, a.ordering, a.published, a.author FROM #__limeticket_kb_art as a LEFT JOIN #__limeticket_kb_art_prod as p ON a.id = p.kb_art_id WHERE p.prod_id = '" . LIMETICKETJ3Helper::getEscaped($db, $prodid) . "'";
			$query1 .= ' AND a.language in (' . $db->Quote(JFactory::getLanguage()->getTag()) . ',' . $db->Quote('*') . ') ';
			$query1 .= ' AND a.access IN (' . implode(',', JFactory::getUser()->getAuthorisedViewLevels()) . ') ';				
			
			$query2 = "SELECT a.id, a.title, a.kb_cat_id, a.ordering, a.published, a.author FROM #__limeticket_kb_art as a WHERE a.allprods = '1'";
			$query2 .= ' AND a.language in (' . $db->Quote(JFactory::getLanguage()->getTag()) . ',' . $db->Quote('*') . ') ';
			$query2 .= ' AND a.access IN (' . implode(',', JFactory::getUser()->getAuthorisedViewLevels()) . ') ';				
			...
			$query = "SELECT a.id, a.title, a.kb_cat_id, a.ordering, a.published, a.author FROM #__limeticket_kb_art as a";
	...
	function &getUncatArts()
	{
		...
		if ($prodid > 0)
		{
			$query1 = "SELECT a.id, a.title, a.kb_cat_id, a.ordering, a.published, a.author FROM #__limeticket_kb_art as a LEFT JOIN #__limeticket_kb_art_prod as p ON a.id = p.kb_art_id WHERE p.prod_id = '" . LIMETICKETJ3Helper::getEscaped($db, $prodid) . "' AND kb_cat_id = 0";
			$query1 .= ' AND a.language in (' . $db->Quote(JFactory::getLanguage()->getTag()) . ',' . $db->Quote('*') . ') ';
			$query1 .= ' AND a.access IN (' . implode(',', JFactory::getUser()->getAuthorisedViewLevels()) . ') ';				
			
			$query2 = "SELECT a.id, a.title, a.kb_cat_id, a.ordering, a.published, a.author FROM #__limeticket_kb_art as a WHERE a.allprods = '1' AND kb_cat_id = 0";
			$query2 .= ' AND a.language in (' . $db->Quote(JFactory::getLanguage()->getTag()) . ',' . $db->Quote('*') . ') ';
			$query2 .= ' AND a.access IN (' . implode(',', JFactory::getUser()->getAuthorisedViewLevels()) . ') ';				
			...
			$query = "SELECT a.id, a.title, a.kb_cat_id, a.ordering, a.published, a.author FROM #__limeticket_kb_art as a WHERE kb_cat_id = 0";
			$where[] = 'language in (' . $db->Quote(JFactory::getLanguage()->getTag()) . ',' . $db->Quote('*') . ')';
			$where[] = 'access IN (' . implode(',', JFactory::getUser()->getAuthorisedViewLevels()) . ')';				
    ...
    function &getArticle()
    {
        ...
        $query = "SELECT f.id, f.title, f.body, f.kb_cat_id, c.title as cattile, f.allprods, f.created, f.modified, f.published, f.author FROM #__limeticket_kb_art as f LEFT JOIN #__limeticket_kb_cat as c ON f.kb_cat_id = c.id";
		
		$where = array();
		$where[] = "f.id = " . LIMETICKETJ3Helper::getEscaped($db, $kbartid);
    ...
    function getProduct()
    {
		...
		$where[] = "id = " . LIMETICKETJ3Helper::getEscaped($db, $prodid);
		$where[] = 'access IN (' . implode(',', JFactory::getUser()->getAuthorisedViewLevels()) . ')';				
    ...
    function getCat()
    {
		...
		if (!$catid)
			return null;
        $query = "SELECT * FROM #__limeticket_kb_cat";
		
		$where[] = "id = " . LIMETICKETJ3Helper::getEscaped($db, $catid);
		$where[] = 'language in (' . $db->Quote(JFactory::getLanguage()->getTag()) . ',' . $db->Quote('*') . ')';
		$where[] = 'access IN (' . implode(',', JFactory::getUser()->getAuthorisedViewLevels()) . ')';					
    ...
    function &getArtsWhat()
    {
		...
		if (empty($this->_arts)) {
			...
			$query1 .= " FROM #__limeticket_kb_art as a WHERE 1 ";
			...
			if ($prodid > 0)
			$query1 .= " AND a.id IN (SELECT kb_art_id FROM #__limeticket_kb_art_prod WHERE prod_id = " . LIMETICKETJ3Helper::getEscaped($db, $prodid) . ") ";
			...
			$query2 .= " FROM #__limeticket_kb_art as a WHERE 1 ";
	...
	function _buildArtQuery()
	{
		...
		$query1 = "SELECT a.* FROM #__limeticket_kb_art as a WHERE 1 ";
		
		if ($catid > 0)
		 $query1 .= " AND kb_cat_id = " . LIMETICKETJ3Helper::getEscaped($db, $catid);
		...
		$query2 = "SELECT a.* FROM #__limeticket_kb_art as a WHERE 1 ";
		if ($catid > 0)
			$query2 .= " AND kb_cat_id = " . LIMETICKETJ3Helper::getEscaped($db, $catid);
    ...
    function &getArticleAttach()
    {
        $db = JFactory::getDBO();
		$kbartid = LIMETICKET_Input::getInt('kbartid');
		$query = "SELECT * FROM #__limeticket_kb_attach WHERE kb_art_id = " . LIMETICKETJ3Helper::getEscaped($db, $kbartid) . " ORDER BY ordering, title";
    ...
    function &getAppliesTo()
    {
        $db = JFactory::getDBO();
		$kbartid = LIMETICKET_Input::getInt('kbartid');
        $query = "SELECT p.* FROM #__limeticket_kb_art_prod as ap LEFT JOIN #__limeticket_prod as p ON ap.prod_id = p.id WHERE p.published = 1 AND p.inkb = 1 AND ap.kb_art_id = " . LIMETICKETJ3Helper::getEscaped($db, $kbartid);
		$query .= ' AND p.access IN (' . implode(',', JFactory::getUser()->getAuthorisedViewLevels()) . ')';				
		        
        $db->setQuery($query);
        $rows = $db->loadAssocList();
		LIMETICKET_Translate_Helper::Tr($rows);
    ...
    function &getRelated()
    {
        $db = JFactory::getDBO();
		$kbartid = LIMETICKET_Input::getInt('kbartid');
        $query = "SELECT a.id, a.title FROM #__limeticket_kb_art_related as r LEFT JOIN #__limeticket_kb_art as a ON r.related_id = a.id WHERE a.published = 1 AND r.kb_art_id = " . LIMETICKETJ3Helper::getEscaped($db, $kbartid) . " ORDER BY a.title";
	...
	function &getSubCats()
	{
        ...
        $db = JFactory::getDBO();
        $query = "SELECT * FROM #__limeticket_kb_cat WHERE parcatid = ".LIMETICKETJ3Helper::getEscaped($db, $catid);
		$query .= ' AND language in (' . $db->Quote(JFactory::getLanguage()->getTag()) . ',' . $db->Quote('*') . ') ';
		$query .= ' AND access IN (' . implode(',', JFactory::getUser()->getAuthorisedViewLevels()) . ') ';				
