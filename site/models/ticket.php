...
class LimeticketModelTicket extends JModelLegacy
{
	...
	function __construct()
	{
		...
		$limit = $mainframe->getUserStateFromRequest('global.list.limit_prod', 'limit', LIMETICKET_Settings::Get('ticket_prod_per_page'), 'int');
	...
	function getProducts()
	{
		...
		/*if (empty($this->_data)) {
			...
			LIMETICKET_Translate_Helper::Tr($this->_data);
		}
		return $this->_data;*/
	}
	...
	{
		$db = JFactory::getDBO();
		$search = LIMETICKET_Input::getString('prodsearch');  
		...
		{
			$where[] = "title LIKE '%".LIMETICKETJ3Helper::getEscaped($db, $search)."%'";
		}
	...
	function &getProduct()
	{
		...
		$db->setQuery($query);
		$rows = $db->loadAssoc();
		LIMETICKET_Translate_Helper::TrSingle($rows);
		return $rows;    */    
	} 
	...
	function getDepartments()
	{
		...
		/*if (empty($this->_departments)) {
			...
			if (LIMETICKET_Settings::get('support_advanced_department') == 1)
			{
				$this->_db->setQuery( $query, $this->getState('limitstart'), $this->getState('limit_prod') );
			} else {
			...
			LIMETICKET_Translate_Helper::Tr($this->_departments);
		}
	...
	function _buildDeptQuery()
	{
		...
		$swhere = "";
		if ($search != "__all__" && $search != '')
			$swhere = "title LIKE '%".LIMETICKETJ3Helper::getEscaped($db, $search)."%'";
		...
		$where[] = "id IN ( SELECT ticket_dept_id FROM #__limeticket_ticket_dept_prod WHERE prod_id = '".LIMETICKETJ3Helper::getEscaped($db, $prodid)."' )";
	...
	function &getCats()
	{
		...
		$prodid = LIMETICKET_Input::getInt('prodid');
		$deptid = LIMETICKET_Input::getInt('deptid');

		$query = "SELECT * FROM #__limeticket_ticket_cat_prod WHERE prod_id = '".LIMETICKETJ3Helper::getEscaped($db, $prodid)."'";
		$db->setQuery($query);
		$products = $db->loadAssocList('ticket_cat_id');
		
		$query = "SELECT * FROM #__limeticket_ticket_cat_dept WHERE ticket_dept_id = '".LIMETICKETJ3Helper::getEscaped($db, $deptid)."'";
		$db->setQuery($query);
	...
	function &getDepartment()
	{
		...
		/*$db = JFactory::getDBO();
		$deptid = LIMETICKET_Input::getInt('deptid');
		$query = "SELECT * FROM #__limeticket_ticket_dept WHERE id = '".LIMETICKETJ3Helper::getEscaped($db, $deptid)."'";
	...
	function &getMessages($ticketid)
	{
		...
		$query = "SELECT m.*, u.name FROM #__limeticket_ticket_messages as m LEFT JOIN #__users as u ON m.user_id = u.id WHERE ticket_ticket_id = '".LIMETICKETJ3Helper::getEscaped($db, $ticketid)."' ORDER BY posted DESC";
	...
	function &getAttach($ticketid)
	{
		...
		$query = "SELECT a.*, u.name FROM #__limeticket_ticket_attach as a LEFT JOIN #__users as u ON a.user_id = u.id WHERE ticket_ticket_id = '".LIMETICKETJ3Helper::getEscaped($db, $ticketid)."' AND hidefromuser = 0 ORDER BY added DESC";
	...
	{		
		...
		if (LIMETICKET_Settings::get('support_simple_userlist_tabs'))
			$tickets = "all";
		
		if (LIMETICKET_Input::getCmd('search_all'))
		{
			$tickets = "";
		}
		...
		if ($search != "")
		{
			...
			if (LIMETICKET_Settings::get('search_extra_like') || strlen($search) < 4) $query .= " OR t.title LIKE '" . LIMETICKET_Helper::strForLike($search) . "'";
			$query .= " OR t.reference LIKE '" . LIMETICKET_Helper::strForLike($search) . "' ) ";
		}

		$order = LIMETICKET_Input::getCmd('order');
		$order_dir = LIMETICKET_Input::getCmd('order_dir', 'asc');
		...
		$limit = $mainframe->getUserStateFromRequest('global.list.limit_ticket', 'limit', LIMETICKET_Settings::Get('ticket_per_page'), 'int');
		$limitstart = LIMETICKET_Input::getInt('limitstart');
		$limitstart = ($limit != 0 ? (floor($limitstart / $limit) * $limit) : 0);
		...
		{
			$fields = LIMETICKETCF::GetCustomFields($ticket->id,$ticket->prod_id,$ticket->ticket_dept_id);
			$values = LIMETICKETCF::GetTicketValues($ticket->id, $ticket);
	...
	function &getPriority($priid)
	{
		...
		$query = "SELECT * FROM #__limeticket_ticket_pri WHERE id = '".LIMETICKETJ3Helper::getEscaped($db, $priid)."'";
	...
	function &getCategory($catid)
	{
		...
		$query = "SELECT * FROM #__limeticket_ticket_cat WHERE id = '".LIMETICKETJ3Helper::getEscaped($db, $catid)."'";
	...
	function &getStatuss()
	{
		...
		/*if (empty($this->_statuss))
		{
			...
			$query = "SELECT * FROM #__limeticket_ticket_status ORDER BY ordering ASC";
