...
class LimeticketModelFaq extends JModelLegacy
{
	var $_total = null;
	...
	function __construct()
	{
		...
		$limit = $mainframe->getUserStateFromRequest('global.list.limit', 'limit', LIMETICKET_Settings::Get('faq_per_page'), 'int');
		...
		$this->_curcatid = LIMETICKET_Input::getInt('catid', '');
		$this->_search = LIMETICKET_Input::getString('search', '');
		...
		if ($this->_search != "")
		{
			...
			$this->_curcatid = -1;
			$this->_curcatimage = "/components/com_limeticket/assets/images/search.png";
		} else if ($this->_curcatid == "0") {
			$this->_curcattitle = JText::_("ALL_FAQS");
			$this->_curcatimage = "/components/com_limeticket/assets/images/allfaqs.png";
		} else {
    ...
    {
        $db = JFactory::getDBO();
        $query = "SELECT * FROM #__limeticket_faq_cat";
    ...
    {
		$db = JFactory::getDBO();
		$faqid = LIMETICKET_Input::getInt('faqid', 0);
        $query = "SELECT f.id, f.question, f.answer, f.fullanswer, f.published, f.ordering, c.title, f.faq_cat_id, f.author, f.featured FROM #__limeticket_faq_faq as f LEFT JOIN #__limeticket_faq_cat as c ON f.faq_cat_id = c.id";
		
		$where = array();
		$where[] = "f.id = " . LIMETICKETJ3Helper::getEscaped($db, $faqid);
		...
		} else if (LIMETICKET_Permission::auth("core.edit.own", "com_limeticket.faq")){
   	...
   	function _buildQuery()
	{
		...
		{
			$mode = "";
			if (LIMETICKET_Helper::contains($this->_search, array('*', '+', '-', '<', '>', '(', ')', '~', '"')))
		...
		{
			$where[] = "faq_cat_id = '" . LIMETICKETJ3Helper::getEscaped($db, $this->_curcatid) . "'";
		}
		...
		$tag = LIMETICKET_Input::getString('tag');
		if ($tag)
		{
			$qry2 = "SELECT faq_id FROM #__limeticket_faq_tags WHERE tag = '".LIMETICKETJ3Helper::getEscaped($db, $tag)."'";
			$db->setQuery($qry2);
	...
	function getFeaturedFaqs()
	{
		...
		$qry = "SELECT * FROM #__limeticket_faq_faq WHERE published = 1 AND featured = 1";
