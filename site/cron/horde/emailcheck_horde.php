...
class LIMETICKETCronEMailCheck_Horde extends LIMETICKETCron
{
	var $params;
	var $conn;
	...
	function OpenNewTicket($userid,&$messageid)
	{
		...
		$qry .= LIMETICKETJ3Helper::getEscaped($db, $ticketid) . "','".LIMETICKETJ3Helper::getEscaped($db, $subject)."','".LIMETICKETJ3Helper::getEscaped($db, $body)."','".LIMETICKETJ3Helper::getEscaped($db, $userid)."','{$now}')";
		//echo $qry."<br>";	
		...
		$ticket = $this->getTicket($ticketid);
		//LIMETICKET_EMail::User_Create($ticket, $subject, $body, array());
		//LIMETICKET_EMail::Admin_Create($ticket, $subject, $body, array());
	...
	function OpenNewTicketUnreg($messageid)
	{
		...
		$ticket = $this->getTicket($ticketid);
		//LIMETICKET_EMail::User_Create_Unreg($ticket, $subject, $body);
		//LIMETICKET_EMail::Admin_Create($ticket, $subject, $body, array());
	...
	function AttachFiles($ticketid, $userid, $messageid)
	{
		...
		foreach ($this->attachments as $filename => &$data)
		{
			...
			$destname = JPATH_SITE.DS.'components'.DS.'com_limeticket'.DS.'files'.DS.'support'.DS.$random.'-'.$filename;    
			...
			if (JFile::write($destname, $data))
			{
				...
				$qry .= "'" . LIMETICKETJ3Helper::getEscaped($db, $random.'-'.$filename) . "',";
				$qry .= "'" . strlen($data) . "',";
	...
	function DoTicketReply($ticketid,$userid,$isadmin,&$messageid)
	{
		...
		if ($body)
		{
			...
			$qry .= LIMETICKETJ3Helper::getEscaped($db, $ticketid) . "','".LIMETICKETJ3Helper::getEscaped($db, $subject)."','".LIMETICKETJ3Helper::getEscaped($db, $body)."','".LIMETICKETJ3Helper::getEscaped($db, $userid)."', '".LIMETICKETJ3Helper::getEscaped($db, $isadmin)."', '{$now}')";
			$db->setQuery($qry);$db->Query();
			$messageid = $db->insertid();
		...
		if ($isadmin)
		{
			LIMETICKET_EMail::Admin_Reply($ticket, $subject, $body, array());
		} else {
			LIMETICKET_EMail::User_Reply($ticket, $subject, $body, array());			
		}
	}
