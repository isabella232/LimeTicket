...
define('LIMETICKET_EMR_SKIP_READ',1);
define('LIMETICKET_EMR_SKIP_TO',2);
define('LIMETICKET_EMR_SKIP_FROM',3);
define('LIMETICKET_EMR_SKIP_SUBJECT',4);
define('LIMETICKET_EMR_SKIP_REGONLY',5);
define('LIMETICKET_EMR_SKIP_NOTREPLY',6);
define('LIMETICKET_EMR_SKIP_UNKNOWNEMAIL',7);
define('LIMETICKET_EMR_REPLY_REG',8);
define('LIMETICKET_EMR_REPLY_UNREG',9);
define('LIMETICKET_EMR_OPEN_REG',10);
define('LIMETICKET_EMR_OPEN_UNREG',11);
define('LIMETICKET_EMR_SKIP_BULK',12);
define('LIMETICKET_EMR_SKIP_DOMAIN_RESTRICTED',13);

class LIMETICKETCronEMailCheck extends LIMETICKETCron
{
	var $params;
	var $conn;
	...
	function EmailResult($message, $status)
	{
		...
		$qry = "SELECT * FROM #__limeticket_ticket_email_log WHERE accountid = '" . $db->escape($accountid) . "' AND messageident = '" . $db->escape($msgid) . "'";
		$db->setQuery($qry);
		$row = $db->loadObject();
		...
		if ($row)
		{
			...
			{
				$qry = "UPDATE #__limeticket_ticket_email_log SET lastseen = NOW() WHERE accountid = '" . $db->escape($accountid) . "' AND messageident = '" . $db->escape($msgid) . "'";
				$db->setQuery($qry);
				$db->Query();
			} else {
				$qry = "UPDATE #__limeticket_ticket_email_log SET lastseen = NOW()";
			...
			$qry = "INSERT INTO #__limeticket_ticket_email_log (accountid, messageident, firstseen, lastseen, status, currentlog, subject, `from`, oldlog) VALUES( ";
	...
	function Execute($aparams)
	{
		...
		{
			$hoard_imap = JPATH_SITE.DS.'components'.DS.'com_limeticket'.DS.'cron'.DS.'horde'.DS.'emailcheck_horde.php';
			...
			if (file_exists($hoard_imap) && file_exists($hoard_lib))
			{
				...
				$check = new LIMETICKETCronEMailCheck_Horde();
		...
		// this only happens 
		if (JRequest::getVar('option') != "com_limeticket" || JRequest::getVar('view') != "cron")
		{
		...
		foreach ($mails as $i)
		{
			...
			if ($this->TestForBulk())
			{
				$this->EmailResult("Bulk EMail / Auto Reply detected, skipping", LIMETICKET_EMR_SKIP_BULK);
				continue;
			}
			...
			{
				$this->EmailResult("Skipping invalid to address", LIMETICKET_EMR_SKIP_TO);
				continue;
			}
			
			if (!$this->ValidateFromAddress())
			{
				$this->EmailResult("Skipping due to from address", LIMETICKET_EMR_SKIP_FROM);
				continue;	
			}
			
			if (!$this->ValidSubject())
			{
				$this->EmailResult("Skipping due to invalid subject", LIMETICKET_EMR_SKIP_SUBJECT);
				continue;	
			}
			...
			{
				$this->EmailResult("Skipping as registered only and not a registered email", LIMETICKET_EMR_SKIP_REGONLY);
				continue;
			}

			// if userid < 1 and in have restricted domains, check the email is ok or not
			if ($userid < 1 && !LIMETICKET_EMail::checkUnregEMail($this->headers->from[0]->mailbox . '@' . $this->headers->from[0]->host))
			{
				$this->EmailResult("Unable to reply or open ticket, domain is restricted (" . $this->headers->from[0]->host . ").", LIMETICKET_EMR_SKIP_DOMAIN_RESTRICTED);
				continue;
			}
			...
			{
				$this->EmailResult("Skipping as not a reply", LIMETICKET_EMR_SKIP_NOTREPLY);
				continue;
			}
			...
			if ($ticketid > 0)
			{
				...
				if ($is_handler)
				{
					...
					if ( (LIMETICKET_Settings::get('support_autoassign') == 3 && $ticket['handler_id'] < 1) || LIMETICKET_Settings::get('support_assign_reply'))
					{
			...
			if ($ticketid > 0)
			{
				...
				} else if ($this->canReplyToTicket($ticketid, $userid)) // user is the tickets main user
				{
					...
					$is_reply = true;
					if (!LIMETICKET_Settings::get('support_user_attach')) $filesok = false;
				...
				} elseif ($this->params['allowunknown'])
				{
					...
					$this->EmailResult("Unknown email replying to the message, ignore the email ({$this->headers->from[0]->mailbox}@{$this->headers->from[0]->host})", LIMETICKET_EMR_SKIP_UNKNOWNEMAIL);
					continue;
				}
			...
			if ($userid > 0)
			{
				...
				$type = $is_reply ? LIMETICKET_EMR_REPLY_REG : LIMETICKET_EMR_OPEN_REG;
				$this->EmailResult("Ticket ID : $ticketid - {$ticket->reference}, UserID : $userid - {$user->name} ({$user->username})", $type);
			} else {
				$type = $is_reply ? LIMETICKET_EMR_REPLY_UNREG : LIMETICKET_EMR_OPEN_UNREG;
	...
	function OpenNewTicket($userid,&$messageid)
	{
		...
		$admin_id = LIMETICKET_Ticket_Helper::AssignHandler($prodid, $deptid, $catid, false, $params);
		...
		$qry = "INSERT INTO #__limeticket_ticket_ticket (reference, ticket_status_id, ticket_pri_id, ticket_cat_id, ticket_dept_id, prod_id, title, opened, lastupdate, user_id, admin_id, email, password, unregname, source) VALUES ";
		$qry .= "('', $def_open, '".LIMETICKETJ3Helper::getEscaped($db, $priid)."', '".LIMETICKETJ3Helper::getEscaped($db, $catid)."', '".LIMETICKETJ3Helper::getEscaped($db, $deptid)."', '".LIMETICKETJ3Helper::getEscaped($db, $prodid)."', '".LIMETICKETJ3Helper::getEscaped($db, $subject)."', '{$now}', '{$now}', '".LIMETICKETJ3Helper::getEscaped($db, $userid)."', '".LIMETICKETJ3Helper::getEscaped($db, $admin_id)."', '', '', '', '$source')";
		//echo $qry."<br>";	
		$db->setQuery($qry);$db->Query();
		$ticketid = $db->insertid();
		$ref = LIMETICKET_Ticket_Helper::createRef($ticketid);

		$qry = "UPDATE #__limeticket_ticket_ticket SET reference = '".LIMETICKETJ3Helper::getEscaped($db, $ref)."' WHERE id = '" . LIMETICKETJ3Helper::getEscaped($db, $ticketid) . "'";  
		$db->setQuery($qry);$db->Query();
		//echo $qry."<br>";	

		$qry = "INSERT INTO #__limeticket_ticket_messages (ticket_ticket_id, subject, body, user_id, posted, source) VALUES ('";
		$qry .= LIMETICKETJ3Helper::getEscaped($db, $ticketid) . "','".LIMETICKETJ3Helper::getEscaped($db, $subject)."','".LIMETICKETJ3Helper::getEscaped($db, $body)."','".LIMETICKETJ3Helper::getEscaped($db, $userid)."','{$now}', 'email')";
		//$this->Log($qry);	
		$db->setQuery($qry);$db->Query();
		...
		if (LIMETICKET_Settings::get('support_user_attach'))
		{
			$this->AttachFiles($ticketid, $userid, $messageid);		
		...
		if (isset($this->params['confirmnew']) && $this->params['confirmnew'] == 1)
		{
			...
			LIMETICKET_EMail::Admin_Pending($ticket, $subject, $body);
		} else {
			LIMETICKET_EMail::User_Create($ticket, $subject, $body);
			LIMETICKET_EMail::Admin_Create($ticket, $subject, $body, $this->files);
		}
	...
	function OpenNewTicketUnreg($messageid)
	{
		...
		$password = LIMETICKET_Helper::createRandomPassword();	
		$now = LIMETICKET_Helper::CurDate();
		...
		$qry = "INSERT INTO #__limeticket_ticket_ticket (reference, ticket_status_id, ticket_pri_id, ticket_cat_id, ticket_dept_id, prod_id, title, opened, lastupdate, user_id, admin_id, email, password, unregname, source) VALUES ";
		$qry .= "('', $def_open, '".LIMETICKETJ3Helper::getEscaped($db, $priid)."', '".LIMETICKETJ3Helper::getEscaped($db, $catid)."', '".LIMETICKETJ3Helper::getEscaped($db, $deptid)."', '".LIMETICKETJ3Helper::getEscaped($db, $prodid)."', '".LIMETICKETJ3Helper::getEscaped($db, $subject)."', '{$now}', '{$now}', '".LIMETICKETJ3Helper::getEscaped($db, $userid)."', '".LIMETICKETJ3Helper::getEscaped($db, $admin_id)."', '".LIMETICKETJ3Helper::getEscaped($db, $email)."', '".LIMETICKETJ3Helper::getEscaped($db, $password)."', '".LIMETICKETJ3Helper::getEscaped($db, $name)."', '$source')";
		//echo $qry."<br>";	
		$db->setQuery($qry);$db->Query();
		$ticketid = $db->insertid();
		$ref = LIMETICKET_Ticket_Helper::createRef($ticketid);
		...
		$qry = "INSERT INTO #__limeticket_ticket_messages (ticket_ticket_id, subject, body, user_id, posted, source, poster, email) VALUES ('";
		$qry .= LIMETICKETJ3Helper::getEscaped($db, $ticketid) . "','".LIMETICKETJ3Helper::getEscaped($db, $subject)."','".LIMETICKETJ3Helper::getEscaped($db, $body)."','".LIMETICKETJ3Helper::getEscaped($db, $userid)."','{$now}', 'email', '".LIMETICKETJ3Helper::getEscaped($db, $name)."', '".LIMETICKETJ3Helper::getEscaped($db, $email)."')";
		$db->setQuery($qry);$db->Query();
		$messageid = $db->insertid();	

		// attach files to ticket
		if (LIMETICKET_Settings::get('support_user_attach') > 1)
		{
			$this->AttachFiles($ticketid, $userid, $messageid);
			$this->processInlineImages($messageid, $body);
		}
		...
		{
			// send admin pending email
			LIMETICKET_EMail::Admin_Pending($ticket, $subject, $body, array(), null);
		} else {	
			LIMETICKET_EMail::User_Create_Unreg($ticket, $subject, $body, array(), null);
			LIMETICKET_EMail::Admin_Create($ticket, $subject, $body, $this->files, null);
		}
	...
	function processInlineImages($message_id, $body = null)
	{
		...
		$qry = "SELECT * FROM #__limeticket_ticket_attach WHERE message_id = " . $db->escape($message_id);
		$db->setQuery($qry);
			...
			foreach ($matches[0] as $offset => $match)
			{
				...
				foreach ($attach as $att)
				{
					if ($att->filename == $image)
					{
						...
						$replace = "[img]" . JURI::base() . "index.php?option=com_limeticket&view=image&fileid={$attach_id}&key={$key}" . "[/img]";
	...
	function AttachFiles($ticketid, $userid, $messageid)
	{
		...
		foreach ($this->attachments as $filename => &$data)
		{
			...
			$destname = LIMETICKET_File_Helper::makeAttachFilename("support", $filename, date("Y-m-d"), $st, $userid);
			...
			if (JFile::write($destpath.$destname, $data))
			{
				...
				$qry .= LIMETICKETJ3Helper::getEscaped($db, $ticketid) . "',";
	...
	function DoTicketReply($ticketid, $userid, $isadmin, &$messageid, $unreg_name = '', $unreg_email = '')
	{
		...
		if ($body)
		{
			$qry = "INSERT INTO #__limeticket_ticket_messages (ticket_ticket_id, subject, body, user_id, admin, posted, poster, email, source) VALUES ('";
			$qry .= LIMETICKETJ3Helper::getEscaped($db, $ticketid) . "','".LIMETICKETJ3Helper::getEscaped($db, $subject)."','".LIMETICKETJ3Helper::getEscaped($db, $body)."','".LIMETICKETJ3Helper::getEscaped($db, $userid)."', '".LIMETICKETJ3Helper::getEscaped($db, $isadmin)."', '{$now}', '".LIMETICKETJ3Helper::getEscaped($db, $unreg_name)."', '".LIMETICKETJ3Helper::getEscaped($db, $unreg_email)."', 'email')";
			$db->setQuery($qry);$db->Query();
			$messageid = $db->insertid();
			
			$qry = "SELECT ticket_status_id FROM #__limeticket_ticket_ticket WHERE id = '".LIMETICKETJ3Helper::getEscaped($db, $ticketid)."'";
			$db->setQuery($qry);
			$status = $db->loadAssoc();
			...
			{
				$newstatus = LIMETICKET_Ticket_Helper::GetStatusID('def_closed');
			} else if ($isadmin)
			{
				$newstatus = LIMETICKET_Ticket_Helper::GetStatusID('def_admin');
			} else {
				$newstatus = LIMETICKET_Ticket_Helper::GetStatusID('def_user');
			}
			
			if ($newstatus > 0)
			{
				$qry = "UPDATE #__limeticket_ticket_ticket SET ticket_status_id = '".LIMETICKETJ3Helper::getEscaped($db, $newstatus)."', closed = NULL WHERE id = '".LIMETICKETJ3Helper::getEscaped($db, $ticketid)."'";
			} else {
				$qry = "UPDATE #__limeticket_ticket_ticket SET closed = NULL WHERE id = '".LIMETICKETJ3Helper::getEscaped($db, $ticketid)."'";
			}
		...
		$qry = "UPDATE #__limeticket_ticket_ticket SET lastupdate = '{$now}' WHERE id = '".LIMETICKETJ3Helper::getEscaped($db, $ticketid)."'";
		$db->setQuery($qry);
		$db->Query(); 
		...
		if ($isadmin)
		{
			LIMETICKET_EMail::Admin_Reply($ticket, $subject, $body, array(), null);
		} else {
			LIMETICKET_EMail::User_Reply($ticket, $subject, $body, array(), null);			
		}
	}
	...
	function &getTicket($ticketid)
	{
		...
		$query .= " FROM #__limeticket_ticket_ticket as t ";
		$query .= " LEFT JOIN #__users as u ON t.user_id = u.id ";
		$query .= " LEFT JOIN #__limeticket_prod as p ON t.prod_id = p.id ";
		$query .= " LEFT JOIN #__limeticket_ticket_dept as d ON t.ticket_dept_id = d.id ";
		$query .= " LEFT JOIN #__limeticket_ticket_cat as c ON t.ticket_cat_id = c.id ";
		$query .= " LEFT JOIN #__limeticket_ticket_status as s ON t.ticket_status_id = s.id ";
		$query .= " LEFT JOIN #__limeticket_ticket_pri as pr ON t.ticket_pri_id = pr.id ";
		$query .= " LEFT JOIN #__users as au ON au.id = t.admin_id ";
		$query .= " WHERE t.id = '".LIMETICKETJ3Helper::getEscaped($db, $ticketid)."' ";
	...
	function TrimMessage()
	{
		...
		require_once (JPATH_SITE.DS.'components'.DS.'com_limeticket'.DS.'helper'.DS.'xml.php');
		
		$path = JPATH_ROOT.DS.'components'.DS.'com_limeticket'.DS.'plugins'.DS.'emailcheck'.DS.'trim'.DS;
	...
	function ValidateFromAddress()
	{
		...
		if (LIMETICKET_Settings::get('support_email_from_address') != "")
			$address = LIMETICKET_Settings::get('support_email_from_address');
	...
	function GetMessage($i)
	{
		...
		{
			require_once(JPATH_SITE.DS.'components'.DS.'com_limeticket'.DS.'helper'.DS.'third'.DS.'html2text.php');
			
			$h2t = new html2text($this->htmlmsg);
			$this->plainmsg = $h2t->getText();
		}

		if (isset($this->params['import_html']) && $this->params['import_html'] && LIMETICKET_Settings::get('allow_raw_html_messages') && strlen($this->htmlmsg) > 10)
		{
			$this->plainmsg = "{RAWHTML}".LIMETICKET_Helper::filterText($this->htmlmsg);	
		}
	}
	...
	function ParseSubject($subject, $from)
	{
		...
		if (preg_match_all("/\[(.*?)\]/", $subject, $matches))
		{
			foreach ($matches[1] as $ref)
			{
				...
				$qry = "SELECT t.id, t.reference, t.title, t.user_id, u.name, u.username, u.email FROM #__limeticket_ticket_ticket as t LEFT JOIN #__users as u ON t.user_id = u.id WHERE t.reference = '".LIMETICKETJ3Helper::getEscaped($db, $ref)."'";
				$db->setQuery($qry);
				$row = $db->loadObject();
				if ($row)
				{
		...
		$qry = "SELECT id, name, username, email FROM #__users WHERE email = '".LIMETICKETJ3Helper::getEscaped($db, $from)."'";
		$db->setQuery($qry);
		$row = $db->loadObject();
		if ($row)
		{
	...
	function &getStatuss()
	{
		if (empty($this->_statuss))
		{
			...
			$query = "SELECT * FROM #__limeticket_ticket_status ORDER BY id ASC";
	...
	function AddTicketAuditNote($ticketid,$note,$userid)
	{
		...
		$qry .= LIMETICKETJ3Helper::getEscaped($db, $ticketid)."','Audit Message','".LIMETICKETJ3Helper::getEscaped($db, $note)."','".LIMETICKETJ3Helper::getEscaped($db, $userid)."',3, '{$now}')";
...
class TrimMatch
{
	...
	{	
		...
		if (count($this->starts) > 0)
		{
			...
			{
				if (LIMETICKET_Helper::stringStartsWith($line, $starts))
				{
					$match = true;
				}
			}
