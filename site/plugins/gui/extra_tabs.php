...
class LIMETICKET_GUIPlugin_Extra_Tabs extends LIMETICKET_Plugin_GUI
{
	...
	function getProductOutput($location)
	{
		...
		foreach ($products as $prod)
		{
			...
			$output[] = '<a href="' . JRoute::_( 'index.php?option=com_limeticket&view=admin_support&what=search&searchtype=advanced&showbasic=1&product=' . $prod->id . $status ) . '">';
			$output[] = $prod->title;
			$output[] = '	</a>';
			$output[] = '</li> ';
		}
	...
	function getDepartmentOutput($location)
	{
		...
		foreach ($departments as $dept)
		{
			...
			$output[] = '<li class="' . $class . '">';
			$output[] = '<a href="' . JRoute::_( 'index.php?option=com_limeticket&view=admin_support&what=search&searchtype=advanced&showbasic=1&department=' . $dept->id . $status ) . '">';
			$output[] = $dept->title;
			$output[] = '	</a>';
			$output[] = '</li> ';
		}
