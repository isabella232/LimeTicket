...
class LIMETICKET_Plugin_OpenSearch_KBArts extends LIMETICKET_Plugin_OpenSearch
{
	var $title = "KB";
	...
	function search($search)
	{
		...
		$base_sql .= " FROM #__limeticket_kb_art as a ";

		// per product kb search
		if ($prodid > 0)
		{
			$where1[] = " a.id IN (SELECT kb_art_id FROM #__limeticket_kb_art_prod WHERE prod_id = " . LIMETICKETJ3Helper::getEscaped($db, $prodid) . ") ";
			$query1 = $base_sql;
		...
		foreach ($results as $result)
		{
			...
			$output->link = LIMETICKETRoute::_( 'index.php?option=com_limeticket&view=kb&tmpl=component&kbartid=' . $result->id );
