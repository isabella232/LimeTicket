...
class Ticket_Source_EMail extends Ticket_Source
{
	...
	function getTabs()
	{
		...
		if (!self::$tab_result)
		{
			$qry = "SELECT count(*) as cnt FROM #__limeticket_ticket_ticket WHERE source = 'email'";
			$db->setQuery($qry);
			$cnt = $db->loadObject();

			if ($cnt->cnt > 0 || LIMETICKET_Input::getCmd('layout') == "emails")
			{		
				$tab = new Ticket_Source_Data();
				$tab->tabname = JText::sprintf("SUP_SRC_EMAILS", $cnt->cnt);
				$tab->name = JText::_("SUP_SRC_EMAIL");
				$tab->count = $cnt->cnt;
				$tab->link = "index.php?option=com_limeticket&view=admin_support&layout=emails";
				
				if (LIMETICKET_Input::getCmd('layout') == "emails")
					$tab->active = true;
