...
class SupportActionsLimitOpen extends SupportActionsPlugin
{
	...
	function Tickets_openNew($params)
	{
		...
		$qry->from("#__limeticket_ticket_ticket");
		...
		if (isset($params['type']) && $params['type'] == 'without')
		{
			...
			{
				$sql = "SELECT group_id FROM #__limeticket_ticket_group_members WHERE user_id = '" . $db->escape($user->id) . "'";
				$db->setQuery($sql);
				...
				if (count($groups) > 0)
				{
					$sql = "SELECT user_id FROM #__limeticket_ticket_group_members WHERE group_id IN (" . implode(", ", $groups) . ")";
					$db->setQuery($sql);
					$users = $db->loadColumn();
