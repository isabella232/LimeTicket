...
class SupportActionsDomainGroup extends SupportActionsPlugin
{
	...
	function User_Open($ticket, $params)
	{
		...
		$db = JFactory::getDBO();
		$qry = "SELECT * FROM #__limeticket_ticket_group WHERE groupname = '" . $db->escape($domain) . "'";
		$db->setQuery($qry);
		$group = $db->loadObject();
		...
		if (!$group)
		{
			$qry = "INSERT INTO #__limeticket_ticket_group (groupname, allsee, allemail, allprods, ccexclude) VALUES ('" . $db->escape($domain) . "', 0, 0, 1, 0)";
			$db->setQuery($qry);
			$db->query();
		...
		$qry = "SELECT * FROM #__limeticket_ticket_group_members WHERE group_id = " . (int)$group_id . " AND user_id = " . (int)$ticket->user_id;
		$db->setQuery($qry);
		$entry = $db->loadObject();
		...
		if (!$entry)
		{
			$qry = "INSERT INTO #__limeticket_ticket_group_members (group_id, user_id) VALUES (" . (int)$group_id . ", " . (int)$ticket->user_id . ")";
			$db->setQuery($qry);
			$db->query();
		}
	}
