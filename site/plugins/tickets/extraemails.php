...
class SupportActionsExtraEMails extends SupportActionsPlugin
{
	...
	{
		$field = LIMETICKETCF::GetField($params['field_id']);
		...
		LIMETICKET_EMail::EMail_To_Ticket_Handler($tmpl, $ticket, "", "", array(), $vars, JFactory::getUser()->id);
	}	
	...
	function Ticket_updateCustomField_User($ticket, $params)
	{
		...
		LIMETICKET_EMail::EMail_To_Ticket_User($tmpl, $ticket, "", "", array(), $vars, JFactory::getUser()->id);
	}
	...
	function User_Reply($ticket, $params)
	{
		...
		{	
			...
			LIMETICKET_EMail::EMail_To_Ticket_User($tmpl, $ticket, $params['subject'], $params['user_message'], array(), array(), JFactory::getUser()->id);
		}
	}
