...
class SQLPlugin extends LIMETICKETCustFieldPlugin
{
	var $name = "SQL Query";
	var $force_display = 1;
	...
	function DisplaySettings($params)
	{
		...
		ob_start();
		include JPATH_SITE.DS.'components'.DS.'com_limeticket'.DS.'plugins'.DS.'custfield'.DS.'sql'.DS.'form.php';
		$result = ob_get_clean();
	...
	function SaveSettings() // return object with settings in
	{
		return $this->encodeParams( array ( 
			...
			'display'		=> LIMETICKET_Input::GetHTML('sql_display')
			));
	}
	...
	function Input($current, $params, $context, $id) // output the field for editing
	{
		...
		if (strpos($result, 'limeticket_sql_custom_error_message') === false)
	...
	function Display($value, $params, $context, $id) // output the field for display
	{
		...
		if (trim($params->command) != "")
		{
			...
			{
				return "<div class='alert alert-error limeticket_sql_custom_error_message' style='margin-bottom: 0'>" . $e->getMessage() . "</div>";
			}
