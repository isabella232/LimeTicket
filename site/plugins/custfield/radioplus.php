...
class RadioPlusPlugin extends LIMETICKETCustFieldPlugin
{
	var $name = "Radio Plus";
	...
	function DisplaySettings($params)
	{
		...
		ob_start();
		include JPATH_SITE.DS.'components'.DS.'com_limeticket'.DS.'plugins'.DS.'custfield'.DS.'radioplus'.DS.'form.php';
		$result = ob_get_clean();
	...
	{
		$values = explode("\n", LIMETICKET_Input::getString('radioplus_items'));
		$res = array();
		...
		return $this->encodeParams( array ( 
			'items'		=> $res,
			'other_label'		=> LIMETICKET_Input::getString('radioplus_other_label')
			));
	}
	...
	function Save($id, $params, $value = "")
	{
		...
		if ($value == "xxxotherxxx") $value = LIMETICKET_Input::getString("custom_{$id}_other");
