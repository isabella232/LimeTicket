...
class DualComboPlugin extends LIMETICKETCustFieldPlugin
{
	...
	function DisplaySettings($params)
	{
		...
		ob_start();
		include JPATH_SITE.DS.'components'.DS.'com_limeticket'.DS.'plugins'.DS.'custfield'.DS.'dualcombo'.DS.'form.php';
		$result = ob_get_clean();
	...
	{
		$p =  array ( 
			'items'		=> LIMETICKET_Input::getString('dualcombo_items'),
			'separator'		=> $_POST['dualcombo_separator'], //JRequest::getVar('dualcombo_separator'),
			'header1'		=> LIMETICKET_Input::getString('dualcombo_header1'),
			'header2'		=> LIMETICKET_Input::getString('dualcombo_header2'),
			'hidegroup'		=> LIMETICKET_Input::getInt('dualcombo_hidegroup'),
			'boxsep'		=> LIMETICKET_Input::getHTML('dualcombo_boxsep')
		);
	...
	function Input($current, $params, $context, $id) // output the field for editing
	{
		...
		$document->addScript(JURI::root().'components/com_limeticket/plugins/custfield/dualcombo/dualcombo.js'); 
	...
	{
		$values = LIMETICKET_Input::getString("custom_$id");
		return $values;
	}
}