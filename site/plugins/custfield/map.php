...
class MapPlugin extends LIMETICKETCustFieldPlugin
{
	...
	function DisplaySettings($params)
	{
		...
		ob_start();
		include JPATH_SITE.DS.'components'.DS.'com_limeticket'.DS.'plugins'.DS.'custfield'.DS.'map'.DS.'form.php';
		$result = ob_get_clean();
	...
	function SaveSettings() // return object with settings in
	{
		return $this->encodeParams( array ( 
			'lat'		=> LIMETICKET_Input::GetFloat('map_lat'),
			'lng'		=> LIMETICKET_Input::GetFloat('map_lng'),
			'zoom'		=> LIMETICKET_Input::getInt('map_zoom'),
			'width'		=> LIMETICKET_Input::getString('map_width'),
			'height'    => LIMETICKET_Input::getString('map_height'),
			));
	}
	...
	function Save($id, $params, $value = "")
	{
		...
		$data->address = LIMETICKET_Input::GetString("custom_$id");
		$data->lng = LIMETICKET_Input::GetString("custom_{$id}_lng");
		$data->lat = LIMETICKET_Input::GetString("custom_{$id}_lat");
		$data->zoom = LIMETICKET_Input::GetString("custom_{$id}_zoom");
	...
	function addJS()
	{
		...
		$document->addScript(JURI::root().'components/com_limeticket/plugins/custfield/map/map_cf.js'); 
		$document->addStyleSheet(JURI::root().'components/com_limeticket/plugins/custfield/map/map_cf.css'); 
	}
}