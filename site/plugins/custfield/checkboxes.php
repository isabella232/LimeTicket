...
class CheckboxesPlugin extends LIMETICKETCustFieldPlugin
{
	...
	function DisplaySettings($params)
	{
		...
		ob_start();
		include JPATH_SITE.DS.'components'.DS.'com_limeticket'.DS.'plugins'.DS.'custfield'.DS.'checkboxes'.DS.'form.php';
		$result = ob_get_clean();
	...
	{
		$data = LIMETICKET_Input::getString('checkboxes_entries');
	...
	function Input($current, $params, $context, $id)
	{
		...
		$posted = LIMETICKET_Input::getInt("custom_{$id}_count");
		if ($posted)
		{
			for ($i = 1 ; $i <= $posted ; $i++)
			{
				$val = LIMETICKET_Input::getString("custom_{$id}_{$i}");	
				if ($val)
				{
					$checked[$val] = $val;	
				}
		...
		foreach ($params->entries as $entry)
		{
			...
			$output[] = '<input type="checkbox" id="custom_$id_' . $i . '" name="custom_' . $id . '_' . $i . '" value="' . LIMETICKET_Helper::encode($entry) . '"  ';
