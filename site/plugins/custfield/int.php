...
class IntPlugin extends LIMETICKETCustFieldPlugin
{
	var $name = "Number";
	...
	function DisplaySettings($params)
	{
		...
		ob_start();
		include JPATH_SITE.DS.'components'.DS.'com_limeticket'.DS.'plugins'.DS.'custfield'.DS.'int'.DS.'form.php';
		$result = ob_get_clean();
	...
	function SaveSettings() // return object with settings in
	{
		return $this->encodeParams( array ( 
			'min'		=> LIMETICKET_Input::getInt('int_min'),
			'max'		=> LIMETICKET_Input::getInt('int_max')
			));
	}
	...
	function Save($id, $params, $value = "")
	{
		...
		$value = LIMETICKET_Input::getInt("custom_$id");
