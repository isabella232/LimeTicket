...
class SQLComboPlugin extends LIMETICKETCustFieldPlugin
{
	...
	function DisplaySettings($params)
	{
		...
		ob_start();
		include JPATH_SITE.DS.'components'.DS.'com_limeticket'.DS.'plugins'.DS.'custfield'.DS.'sqlcombo'.DS.'form.php';
		$result = ob_get_clean();
	...
	function SaveSettings() // return object with settings in
	{
		return $this->encodeParams( array ( 
			'command'		=> LIMETICKET_Input::getString('sql_command'),
			'display'		=> LIMETICKET_Input::getString('sql_display'),
			'field'		=> LIMETICKET_Input::getString('sql_field'),
			'group'		=> LIMETICKET_Input::getString('sql_group'),
			'multi'		=> LIMETICKET_Input::getInt('sql_multi'),
			'class_select'		=> LIMETICKET_Input::getString('sql_class_select'),
			'class_label'		=> LIMETICKET_Input::getString('sql_class_label')
		));
	}
	...
	function getData($id, $params, $context)
	{
		if (empty(self::$values[$id]))
		{
			...
			{
				self::$values[$id] = "<div class='alert alert-error limeticket_sql_custom_error_message' style='margin-bottom: 0'>" . $e->getMessage() . "</div>";
			}
		}
	...
	{
		//if (LIMETICKETJ3Helper::IsJ3()) JHtml::_('formbehavior.chosen', 'select');
	...
	{
		$params = $this->parseParams($params);
		$values = LIMETICKET_Input::getArray("custom_$id");
