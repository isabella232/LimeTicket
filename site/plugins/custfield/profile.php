...
class ProfilePlugin extends LIMETICKETCustFieldPlugin
{
	...
	{
		$path = JPATH_SITE.DS.'components'.DS.'com_limeticket'.DS.'plugins'.DS.'custfield'.DS.'profile'.DS.'sources';
	...
	function DisplaySettings($params) // passed object with settings in
	{
		...
		ob_start();
		include JPATH_SITE.DS.'components'.DS.'com_limeticket'.DS.'plugins'.DS.'custfield'.DS.'profile'.DS.'form.php';
		$result = ob_get_clean();
	...
	function SaveSettings() // return object with settings in
	{
		return $this->encodeParams( array ( 
			'field'		=> LIMETICKET_Input::getCmd('profile_field'),
	...
	function Display($value, $params, $context, $id) // output the field for display
	{
		...
		if (array_key_exists($field, $presets)) 
			return LIMETICKET_Helper::escape_sequence_decode($this->displayTemplate($user, $presets[$field], true, $plugin));

		$values = $source->getValues($user);

		if (array_key_exists($field, $values))
			return LIMETICKET_Helper::escape_sequence_decode($values[$field]);
	...
	function Search($params, $search, $peruser)
	{
		...
		if (count($result) > 0)
		{
			...
			$query->select("id as ticket_id")
				->from("#__limeticket_ticket_ticket")
