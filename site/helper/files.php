...
class LIMETICKET_File_Helper
{
	static function Thumbnail($image_file, $thumb_file,$width = 48, $height = 48)
	{
		$ctype = LIMETICKET_Helper::datei_mime("png");
		...
		if (!$im->image)
		{
			...
			$im->load(JPATH_SITE.DS.'components'.DS.'com_limeticket'.DS.'assets'.DS.'images'.DS.'blank_16.png');
		}
	...
	static function DownloadFile($filename, $display_name, $ext = "")
	{
		...
		$ctype = LIMETICKET_Helper::datei_mime($ext);		
	...
	{
		$base = JPATH_SITE.DS.LIMETICKET_Helper::getAttachLocation().DS.$subfolder.DS;
		...
		if ($ticket != null)
		{
			switch (LIMETICKET_Settings::get('attach_storage_filename'))
			{
				case 1:
					$path .= $ticket->id . DS;
					break;
		...
		$filename = str_replace("{UID}", LIMETICKET_File_Helper::shortUID($file_template), $file_template);
	...
	{
		$config = new JConfig();
		return substr(md5("limeticket_incoming".$token.$config->secret),0,8);
	}
