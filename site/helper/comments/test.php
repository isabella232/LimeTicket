...
class LIMETICKET_Comments_Handler_Test extends LIMETICKET_Comments_Handler
{
	var $ident = 5;	
	...
	function __construct($parent) 
	{
		...
		$this->comments->use_email = LIMETICKET_Settings::get('test_use_email');//FSJ_Settings::GetComponentSetting( fsj_get_com(), 'comments_email', 1 );
		$this->comments->use_website = LIMETICKET_Settings::get('test_use_website'); //FSJ_Settings::GetComponentSetting( fsj_get_com(), 'comments_website', 1 );
		...
		$this->article_link = "index.php?option=com_limeticket&view=test&prodid={id}";
		...
		if (LIMETICKET_Settings::get('test_who_can_add') == "anyone")
		{
			$this->comments->can_add = 1;
		} else if (LIMETICKET_Settings::get('test_who_can_add') == "moderators")
		{
			if (!LIMETICKET_Permission::CanModerate())
			{
				$this->comments->can_add = 0;	
			}
		} else if (LIMETICKET_Settings::get('test_who_can_add') == "registered")
		{
			...
			//echo "Testimonials ACL : " . LIMETICKET_Settings::get('test_who_can_add') . "<br>";	
			$user = JFactory::getUser();
			$authed = $user->getAuthorisedViewLevels();
			
			if (!in_array(LIMETICKET_Settings::get('test_who_can_add'), $authed))
			{
				$this->comments->can_add = 0;	
			} else {
				$this->comments->can_add = 1;
		...
		// set up moderation
		$commod = LIMETICKET_Settings::get('test_moderate');
		...
		$this->comments->dest_email = LIMETICKET_Settings::get( 'test_email_on_submit' );
		$this->table = "#__limeticket_prod";
	...
	function GetItemData($itemids = null)
	{
		...
		LIMETICKET_Translate_Helper::Tr($this->itemdata);
	}
}