...
class LIMETICKET_Comments_Handler
{
	var $ident = 0;	
	var $name;
	...
	function GetName()
	{
		if (empty($this->name))
		{
			...
			$classname = str_ireplace("LIMETICKET_Comments_Handler_","",$classname);
			if ($classname == " LIMETICKET_Comments_Handler")
				$classname = "general";
	...
	function EMail_AddFields(&$comment)
	{
		...
		$link = LIMETICKET_Helper::GetBaseURL() . LIMETICKETRoute::_(str_replace("{id}",$itemid,$this->article_link));
	...
	{
		$db = JFactory::getDBO();
		$qry = "SELECT {$this->field_id}, {$this->field_title} FROM {$this->table} WHERE {$this->field_id} = '".LIMETICKETJ3Helper::getEscaped($db, $itemid)."'";
		$db->setQuery($qry);
		return $db->loadAssoc();	
	}
	...
	{
		return LIMETICKETRoute::_(str_replace("{id}",$itemid,$this->article_link));
	}
