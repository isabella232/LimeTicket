...
class LIMETICKETMailer
{
	...
	function addFiles($files)
	{
		...
		{
			$filename = JPATH_SITE.DS.LIMETICKET_Helper::getAttachLocation().DS."support".DS.$file->diskfile;
	...
	function Send()
	{
		...
		{
			LIMETICKET_Settings::set('email_send_multiple', "multi");
		}

		if (LIMETICKET_Settings::Get('email_send_multiple') == "to" || LIMETICKET_Settings::Get('email_send_multiple') == "bcc")
		{
			...
			foreach ($emails as $email => $name)
			{
				...
				if (LIMETICKET_Settings::Get('email_send_multiple') == "bcc")
				{
			...
			foreach ($emails as $email => $name)
			{
				...
				if (strpos($body, "{login_code}") !== false)
				{
					...
					if ($user_id > 1)
					{
						$body = str_replace("{login_code}", LIMETICKET_Helper::AutoLoginCreate($user_id), $body);
					} else {
	...
	function doLog()
	{
		...
		if (array_key_exists("ticket", $this->debug_data) && $this->debug_data["ticket"])
		{
			...
			$output .= "Ticket: ";
			$output .= "<a href='" . LIMETICKETRoute::_('index.php?option=com_limeticket&view=admin_support&layout=ticket&ticketid=' . $ticket->id).  "'>";
		...
		{
			$output .= "Send config: " . LIMETICKET_Settings::Get('email_send_multiple') . "<br />";
		}
		...
		$db = JFactory::getDBO();
		$qry = "INSERT INTO #__limeticket_cron_log (cron, log, `when`, `type`, title, data) VALUES ('" . $db->escape("EMail Sent") . "', '" . $db->escape($output) . "', '{$now}', 'emailsend', '" . $db->escape($title) . "', '" . $db->escape($data) . "')";
		$db->setQuery($qry);
		$db->Query();
	}
	...
	static function Get_Sender()
	{
		...
		{		
			...
			if (!LIMETICKET_Helper::isValidEmail($address) || !LIMETICKET_Helper::isValidName($name))
			{
		...
		if (LIMETICKET_Helper::isValidName(LIMETICKET_Settings::get('support_email_from_name')))
			$name = trim(LIMETICKET_Settings::get('support_email_from_name'));

		if (LIMETICKET_Helper::isValidEmail(LIMETICKET_Settings::get('support_email_from_address')))
			$address = trim(LIMETICKET_Settings::get('support_email_from_address'));

		if (!LIMETICKET_Helper::isValidEmail($address))
		{
			$address = "noreply@none.com";
		}
			
		if (!LIMETICKET_Helper::isValidName($name))
		{
			$name = "Unknown";		
		}
	...
	function getMailer()
	{
		if (!LIMETICKET_Settings::Get('email_send_override'))
		{
		...
		$smtpauth = (LIMETICKET_Settings::Get('email_send_smtp_auth') == 0) ? null : 1;
		$smtpuser = LIMETICKET_Settings::Get('email_send_smtp_username'); // $conf->get('smtpuser');
		$smtppass = LIMETICKET_Settings::Get('email_send_smtp_password'); // $conf->get('smtppass');
		$smtphost = LIMETICKET_Settings::Get('email_send_smtp_host'); // $conf->get('smtphost');
		$smtpsecure = LIMETICKET_Settings::Get('email_send_smtp_security'); // $conf->get('smtpsecure');
		$smtpport = LIMETICKET_Settings::Get('email_send_smtp_port'); // $conf->get('smtpport');
		$mailfrom = LIMETICKET_Settings::Get('email_send_from_email'); // $conf->get('mailfrom');
		$fromname = LIMETICKET_Settings::Get('email_send_from_name'); // $conf->get('fromname');
		$mailer = LIMETICKET_Settings::Get('email_send_mailer'); // $conf->get('mailer');
