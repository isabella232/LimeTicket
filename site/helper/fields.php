...
class LIMETICKETCustFieldPlugin
{
...
{	
	...
	static function &GetCustomFields($ticketid,$prod_id,$ticket_dept_id,$maxpermission = 3,$isopen = false,$foruser = false)
	{
		...
		$qry = "SELECT * FROM #__limeticket_field as f WHERE f.published = 1 AND f.ident = 0 AND ";
		$qry .= " (allprods = 1 OR '".LIMETICKETJ3Helper::getEscaped($db, $prod_id)."' IN (SELECT prod_id FROM #__limeticket_field_prod WHERE field_id = f.id)) AND ";
		$qry .= " (alldepts = 1 OR '".LIMETICKETJ3Helper::getEscaped($db, $ticket_dept_id)."' IN (SELECT ticket_dept_id FROM #__limeticket_field_dept WHERE field_id = f.id)) ";
		...
		/*if ($isopen)
		{
			$qry .= " 1 ";//(f.permissions <= '".LIMETICKETJ3Helper::getEscaped($db, $maxpermission)."' OR f.permissions = 4 OR f.permissions = 5) ";
		} else {
			$qry .= " (f.permissions <= '".LIMETICKETJ3Helper::getEscaped($db, $maxpermission)."' OR f.permissions = 5)  ";
		}*/
		...
		if (count($rows) > 0)
		{
			...
			{
				$indexes[] = LIMETICKETJ3Helper::getEscaped($db, $index);
			} 
		}
		...
		$qry = "SELECT * FROM #__limeticket_field_values WHERE field_id IN ($indexlist)";
		$db->setQuery($qry);
	...
	static function &GetAllCustomFields($values = true, $ident = 0)
	{
		...
		if (empty(LIMETICKETCF::$allfields[$key]))
		{
			// get a list of all available fields
			$qry = "SELECT * FROM #__limeticket_field as f WHERE f.published = 1";
			...
			LIMETICKETCF::$allfields[$key] = $rows;
		}
		return LIMETICKETCF::$allfields[$key];
	}
	
	static function GetField($fieldid)
	{
		$db = JFactory::getDBO();
		$qry = "SELECT * FROM #__limeticket_field WHERE id = '".LIMETICKETJ3Helper::getEscaped($db, $fieldid)."'";
		$db->setQuery($qry);
	...
	static function FieldHeader(&$field, $showreq = false, $showperuser = true)
	{
		...
		if ($field['peruser'] && $showperuser)
			echo " <i class='icon-user limeticketTip' title='User Field'></i>";
	}
	...
	static function FieldInput(&$field,&$errors,$errortype="ticket",$context = array(), $showhelp = false)
	{
		...
		{
			$uservalues = LIMETICKETCF::GetUserValues($userid, $ticketid);
		...
		try {
			$current = LIMETICKET_Input::getString("custom_$id",$current);
		...
		if ($field['type'] == "text")
		{
			...
			$output = "<input type='text' class='input-large custom_".$field['alias'] . "' name='custom_$id' id='custom_$id' value=\"".LIMETICKET_Helper::escape($current)."\" maxlength='$text_max' size='$text_size' ";
			if ($field['required'])
				$output .= " required ";
		...
		if ($field['type'] == "radio")
		{
			$values = LIMETICKETCF::GetValues($field);
			$output = "";
			if (count($values) > 0)
			{
				foreach ($values as $value)
				{
					$output .= "<input class='custom_".$field['alias'] . "' type='radio' id='custom_$id' name='custom_$id' value=\"".LIMETICKET_Helper::escape($value)."\"";
		...
		if ($field['type'] == "combo")
		{
			...
			if (count($values) > 0)
			{
				foreach ($values as $value)
				{
					$output .= "<option value=\"".LIMETICKET_Helper::escape($value)."\"";
		...
		if (array_key_exists($id,$errors))
		{
			if ($errortype == "ticket")
			{
				...
				$output .= '</td><td class="limeticket_must_have_field">' . $errors[$id];
			}
		} else if ($showhelp)
		{
	...
	static function get_plugin_from_row(&$row)
	{
		...
		$query = "SELECT value FROM #__limeticket_field_values WHERE field_id = " . LIMETICKETJ3Helper::getEscaped($db, $row->id);
		$db->setQuery($query);
		$values = LIMETICKETJ3Helper::loadResultArray($db);
		...
		return LIMETICKETCF::get_plugin($plugin_name);
	}	
		
	static function get_plugin($name)
	{
		if ($name == "")
			return new LIMETICKETCustFieldPlugin();
		
		$name = preg_replace("/[^a-zA-Z0-9\s]/", "", $name);
		$name = strtolower($name);
		$filename = JPATH_SITE.DS.'components'.DS.'com_limeticket'.DS.'plugins'.DS.'custfield'.DS.$name.".php";
	...
	static function get_plugins()
	{
		...
		$path = JPATH_SITE.DS.'components'.DS.'com_limeticket'.DS.'plugins'.DS.'custfield'.DS;
	...
	static function get_filenames($source_dir, $include_path = FALSE, $_recursion = FALSE)
	{
		...
		if ($fp = @opendir($source_dir))
		{
			...
			while (FALSE !== ($file = readdir($fp)))
			{
				...
				{
					LIMETICKETCF::get_filenames($source_dir.$file.DS, $include_path, TRUE);
				}
	...
	static function ValidateFields(&$fields, &$errors)
	{
		...
		foreach ($fields as &$field)
		{
			if ($field['required'] > 0)
			{
				$value = LIMETICKET_Input::getString("custom_" . $field['id'],"XXXYYYXXX");
	...
	static function StoreFields(&$fields, $ticketid)
	{
		...
		$allfields = LIMETICKETCF::GetAllCustomFields(false);
		$db = JFactory::getDBO();
		...
		if (count($fields) > 0)
		{
			foreach ($fields as &$field)
			{
				...
				$value = LIMETICKET_Input::getString("custom_" . $field['id'],"XX--XX--XX");
				...
				if ($value != "XX--XX--XX")
				{
					...
					{
						$qry = "REPLACE INTO #__limeticket_ticket_user_field (user_id, field_id, value) VALUES ('".LIMETICKETJ3Helper::getEscaped($db, $userid)."','";
						$qry .= LIMETICKETJ3Helper::getEscaped($db, $field['id']) . "','";
						$qry .= LIMETICKETJ3Helper::getEscaped($db, $value) . "')";
						$db->setQuery($qry);
						$db->Query();
					} else {
						$qry = "REPLACE INTO #__limeticket_ticket_field (ticket_id, field_id, value) VALUES ('".LIMETICKETJ3Helper::getEscaped($db, $ticketid)."','";
						$qry .= LIMETICKETJ3Helper::getEscaped($db, $field['id']) . "','";
						$qry .= LIMETICKETJ3Helper::getEscaped($db, $value) . "')";
						$db->setQuery($qry);
						$db->Query();
	...
	{
		$allfields = LIMETICKETCF::GetAllCustomFields(true);
		//echo "V1 : $value<br>";
		...
		if (array_key_exists($fieldid, $allfields) && $allfields[$fieldid]['peruser'] && $userid > 0)
		{
			...
			$qry = "SELECT value FROM #__limeticket_ticket_user_field WHERE user_id = '".LIMETICKETJ3Helper::getEscaped($db, $userid)."' AND field_id = '".LIMETICKETJ3Helper::getEscaped($db, $fieldid)."'";
			//echo $qry . "<br>";
			$db->setQuery($qry);
			$row = $db->loadObject();
			$qry = "REPLACE INTO #__limeticket_ticket_user_field (user_id, field_id, value) VALUES ('".LIMETICKETJ3Helper::getEscaped($db, $userid)."','";
			$qry .= LIMETICKETJ3Helper::getEscaped($db, $fieldid). "','";
			$qry .= LIMETICKETJ3Helper::getEscaped($db, $value) . "')";
			$db->setQuery($qry);
			//echo $qry . "<br>";
			$db->Query();
			...
			$qry = "SELECT value FROM #__limeticket_ticket_field WHERE ticket_id = '".LIMETICKETJ3Helper::getEscaped($db, $ticketid)."' AND field_id = '".LIMETICKETJ3Helper::getEscaped($db, $fieldid)."'";
			//echo $qry . "<br>";
			$db->setQuery($qry);
			$row = $db->loadObject();
			$qry = "REPLACE INTO #__limeticket_ticket_field (ticket_id, field_id, value) VALUES ('".LIMETICKETJ3Helper::getEscaped($db, $ticketid)."','";
	...
	static function GetUserValues($userid = 0,$ticketid = 0)
	{
		...
		if (empty(LIMETICKETCF::$user_values))
		{
			$db = JFactory::getDBO();
			if ($userid < 1)
			{
				if (empty(LIMETICKETCF::$ticket_user_id))
				{
					$qry = "SELECT user_id FROM #__limeticket_ticket_ticket WHERE id = '".LIMETICKETJ3Helper::getEscaped($db, $ticketid)."'";
					$db->setQuery($qry);
					$row = $db->loadObject();
					if ($row)
						LIMETICKETCF::$ticket_user_id = $row->user_id;	
				}
				
				$userid = LIMETICKETCF::$ticket_user_id;
			}
			
			if ($userid < 1)
				return array();
			
			$qry = "SELECT * FROM #__limeticket_ticket_user_field WHERE user_id ='".LIMETICKETJ3Helper::getEscaped($db, $userid)."'";
			$db->setQuery($qry);
			LIMETICKETCF::$user_values = $db->loadAssocList('field_id');
		}
		
		return LIMETICKETCF::$user_values;
	}

	static function &GetTicketValues($ticketid,$ticket)
	{
		if (empty(LIMETICKETCF::$_ticketvalues))
			LIMETICKETCF::$_ticketvalues = array();
			
		if (!array_key_exists($ticketid,LIMETICKETCF::$_ticketvalues))
		{
			$allfields = LIMETICKETCF::GetAllCustomFields(true);
			
			$db = JFactory::getDBO();
			$qry = "SELECT * FROM #__limeticket_ticket_field WHERE ticket_id ='".LIMETICKETJ3Helper::getEscaped($db, $ticketid)."'";
			$db->setQuery($qry);
			$values = $db->loadAssocList('field_id');
		
			if (is_array($ticket))
			{
				$values2 = LIMETICKETCF::GetUserValues($ticket['user_id'], $ticket['id']);
			} else {
				$values2 = LIMETICKETCF::GetUserValues($ticket->user_id, $ticket->id);
			}
			...
			LIMETICKETCF::$_ticketvalues[$ticketid] = $values;
		}
		return LIMETICKETCF::$_ticketvalues[$ticketid];	
	}
	...
	static function &Comm_GetCustomFields($ident)
	{
		...
		if ($ident != -1)
		{
			$qry = "SELECT * FROM #__limeticket_field as f WHERE f.published = 1 AND (f.ident = 999 OR f.ident = '".LIMETICKETJ3Helper::getEscaped($db, $ident)."') ";
		} else {
			$qry = "SELECT * FROM #__limeticket_field as f WHERE f.published = 1 ";
		}
