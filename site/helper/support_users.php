...
class SupportUsers
{
	...
	static function getUser($id = null)
	{
		...
		if (empty(self::$users[$id]))
		{
			...
			$qry .= " LEFT JOIN #__limeticket_users as f ON u.id = f.user_id ";
	...
	static function BlankSettings()
	{
		if (empty(self::$user_defaults))
		{
			...
			$db = JFactory::getDBO();
			$sql = "SELECT * FROM #__limeticket_plugins WHERE type = 'gui' AND name = 'default_prefs' AND enabled = 1";
			$db->setQuery($sql);
	...
	static function updateUserSettings($settings, $id = null)
	{
		...
		if ($user->user_id > 0)
		{
			...
			$qry = "UPDATE #__limeticket_users SET settings = '" . $db->escape(json_encode($settings)) . "' WHERE user_id = " . (int)$user->id;
		} else {
			$qry = "REPLACE INTO #__limeticket_users (user_id, settings) VALUES (" . (int)$user->id . ", '" . $db->escape(json_encode($settings)) . "')";
		}
	...
	{	
		...
		{
			// we have a row existing, so update it
			$qry = "UPDATE #__limeticket_users SET rules = '" . $db->escape(json_encode($permissions)) . "' WHERE user_id = " . (int)$user->id;
		} else {
			$qry = "REPLACE INTO #__limeticket_users (user_id, rules) VALUES (" . (int)$user->id . ", '" . $db->escape(json_encode($permissions)) . "')";
		}
	...
	{
		$set = str_replace("com_limeticket.", "", $set);
		...
		if (empty(self::$misc_perms[$key]))
		{
			...
			$db = JFactory::getDBO();
			$qry = "SELECT lft, rgt FROM #__assets WHERE name = 'com_limeticket.{$set}'";
			$db->setQuery($qry);
			$asset = $db->loadObject();
			...
			$qry = "SELECT user_id, rules FROM #__limeticket_users WHERE rules != ''";
			$db->setQuery($qry);
	...
	static function getHandlers($explicit = false, $use_out_of_office = true)
	{
		...
		if (!array_key_exists($key, self::$all_handlers))
		{
			...
			{
				$users = self::usersWithPerm("support_admin", "limeticket.handler", $explicit);
				...
				self::$all_handlers[$key] = self::usersWithPerm("support_admin", "limeticket.handler", $explicit);
			}
		}
	...
	static function getAdminWhere()
	{
		...
		$where .= " t.id IN (SELECT ticket_id FROM #__limeticket_ticket_cc WHERE user_id = {$user->id} AND isadmin = 1) OR ";
		$where .= " (";
		...
		// users
		$can_see_unassigned = LIMETICKET_Permission::auth("limeticket.handler.seeunassigned", "com_limeticket.support_admin");
		$can_see_others = LIMETICKET_Permission::auth("limeticket.handler.seeothers", "com_limeticket.support_admin");
		...
		// products
		if (!LIMETICKET_Permission::auth("limeticket.handler.view.products", "com_limeticket.support_admin"))
		{
			$prod_ids = array();
			$prod_ids[0] = "0";
			...
			{
				if (LIMETICKET_Permission::auth("limeticket.handler.view.product." . $product->id, "com_limeticket.support_admin"))
				{
		...
		// departments
		if (!LIMETICKET_Permission::auth("limeticket.handler.view.departments", "com_limeticket.support_admin"))
		{
			$dept_ids = array();
			$dept_ids[0] = "0";
			...
			{
				if (LIMETICKET_Permission::auth("limeticket.handler.view.department." . $dept->id, "com_limeticket.support_admin"))
				{
		...
		// categories	
		if (!LIMETICKET_Permission::auth("limeticket.handler.view.categories", "com_limeticket.support_admin"))
		{
			$cat_ids = array();
			$cat_ids[0] = "0";
			...
			foreach ($cats as $cat)
			{
				if (LIMETICKET_Permission::auth("limeticket.handler.view.category." . $cat->id, "com_limeticket.support_admin"))
				{
	...
	static function getUsersWhere($userid = null)
	{
		...
		if ($userid > 0)
		{
			...
			if (LIMETICKET_Settings::get('support_restrict_prod_view'))
			{
	...
	static function getHandlersTicket($prodid, $deptid, $catid, $allownoauto = false, $assign_ticket = true, $explicit = false, $use_out_of_office = true) 
	{
		...
		foreach ($users as $user)
		{
			$perm_set = "limeticket.handler.view";
			if (LIMETICKET_Permission::auth("limeticket.handler.assign.separate", "com_limeticket.support_admin", $user->id) && $assign_ticket)
				$perm_set = "limeticket.handler.assign";
			
			if (LIMETICKET_Permission::auth("limeticket.handler.dontassign", "com_limeticket.support_admin", $user->id) && !$allownoauto)
				continue;	
			
			if ($prodid > 0 && !LIMETICKET_Permission::auth($perm_set.".products", "com_limeticket.support_admin", $user->id))
				if (!LIMETICKET_Permission::auth($perm_set.".product.". $prodid, "com_limeticket.support_admin", $user->id))
					continue;	
			
			if ($deptid > 0 && !LIMETICKET_Permission::auth($perm_set.".departments", "com_limeticket.support_admin", $user->id))
				if (!LIMETICKET_Permission::auth($perm_set.".department.". $deptid, "com_limeticket.support_admin", $user->id))
					continue;
					//echo "Skip : Not department<br>";
			
			if ($catid > 0 && !LIMETICKET_Permission::auth($perm_set.".categories", "com_limeticket.support_admin", $user->id))
				if (!LIMETICKET_Permission::auth($perm_set.".category.". $catid, "com_limeticket.support_admin", $user->id))
					continue;
