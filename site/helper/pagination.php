...
if (!class_exists('JPaginationEx'))
{
	class JPaginationEx extends JPagination
	{
		...
		{
			$this->skinstyle = LIMETICKET_Settings::get('skin_style');
		...
		function _list_footer($list)
		{
			...
			//$html .= "\n<div class=\"limeticket_pagination\">&nbsp;".$list['pageslinks']."</div>";
			$html .= "<div class=\"pagination wtf\">\n";
			$html .= "\n<ul>".$list['pageslinks']."</ul>";
			$html .= "</div>";
			//$html .= "\n<div class=\"limeticket_counter\">&nbsp;".$list['pagescounter']."</div>";
...
if (!class_exists("JPaginationJS"))
{
	class JPaginationJS extends JPaginationEx
	{
		...
		{
			if($item->base>0)
				return "<li><a href='' title=\"".$item->text."\" onclick=\"javascript: document.limeticketForm.limitstart.value=".$item->base.";jQuery('#limeticketForm').submit();jQuery('#limeticketFormTS').submit();return false;\" class=\"pagenav\">".$item->text."</a></li>";

			return "<li><a href='' title=\"".$item->text."\" onclick=\"javascript: document.limeticketForm.limitstart.value=0;jQuery('#limeticketForm').submit();jQuery('#limeticketFormTS').submit();return false;\" class=\"pagenav\">".$item->text."</a></li>";	
		}
	}
}
...
if (!class_exists('JPaginationAjax'))
{
	class JPaginationAjax extends JPaginationEx
	{
		function _buildDataObject()
		{
			...
			if (!$this->_viewall) {
				$data->all->base	= '0';
				$data->all->link	= LIMETICKETRoute::x("&limitstart=");
			}
