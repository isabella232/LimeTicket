	...
	static function runCron($test = null)
	{
		...
		while ($max_runs-- > 0 || $test > 0)
		{
			if ($test > 0)
			{
				...
				$qry = "SELECT * FROM #__limeticket_cron WHERE id = ".$db->escape($test);
			} else {		
				$qry = "SELECT * FROM #__limeticket_cron WHERE published = 1 AND ((UNIX_TIMESTAMP() - lastrun) - (`interval` * 60)) > 0 LIMIT 1";
			}
			...
			foreach ($rows as $row)
			{
				$db->setQuery("UPDATE #__limeticket_cron SET lastrun=UNIX_TIMESTAMP() WHERE id='{$row->id}' LIMIT 1");
				$db->query();

				$class = "LIMETICKETCron" . $row->class;
				if (substr($row->class, 0, 6) == "Plugin")
				{
					$file = strtolower(str_ireplace("plugin", "", $row->class) . ".php");
					$path = JPATH_SITE.DS.'components'.DS.'com_limeticket'.DS.'plugins'.DS.'cron'.DS;
				} else {
