...
class LIMETICKETParser
{
	...
	function loadTemplate($template, $tpltype)
	{
		...
		{
			$db = JFactory::getDBO();
			$query = "SELECT * FROM #__limeticket_templates WHERE template = '" . LIMETICKETJ3Helper::getEscaped($db, $template) . "'";
			$db->SetQuery($query);
	...
	function loadEmail($template_id)
	{
		if (is_array($template_id))
		{
			...
			{
				$db = JFactory::getDBO();
				$qry = 	"SELECT body, subject, ishtml, translation, tmpl FROM #__limeticket_emails WHERE tmpl = '".$db->escape($template_id)."'";
				$db->setQuery($qry);
	...
	function parseMessage()
	{
		...
		{
			$body = LIMETICKET_Helper::ParseBBCode($this->message_body,null,false,false, ($this->target == "user"));	
	...
	function getBody()
	{
		...
		if ($this->data->ishtml)
		{
			$body = LIMETICKET_Helper::MaxLineLength($body);
		} else {	
			$body = str_replace("<br />","\n",$body);
			$body = html_entity_decode($body);
			$body = preg_replace_callback("/(&#[0-9]+;)/", array("LIMETICKET_Helper", "email_decode_utf8"), $body); 
			$body = strip_tags($body);
		}
		
		$body = str_replace("XXXLTXXX", "<", $body);
		$body = str_replace("XXXGTXXX", ">", $body);
		$body = str_replace("c6f2686c6759870d330fb0089f3b02c4", "", $body);
	...
	function init()
	{
		...
		$this->vars["current_date"] = LIMETICKET_Helper::Date(time(), LIMETICKET_DATE_LONG);
		$this->vars["current_time"] = LIMETICKET_Helper::Date(time(), LIMETICKET_TIME_SHORT);
		$this->vars["current_datetime"] = LIMETICKET_Helper::Date(time(), LIMETICKET_DATETIME_LONG);
			...
			} else if ($bits[0] == "set")
			{
				...
				if (array_key_exists($bits[0],$this->vars))
				{
					if (isset($bits[1]) && $bits[1] > 0)
					{
						...
						$is_trimmed = false;
						$o .= LIMETICKET_Helper::truncate($this->vars[$bits[0]], $bits[1], $is_trimmed, $ending);
					} else {
						$o .= $this->vars[$bits[0]];
					}
				}	
	...
	function processSortTags()
	{
		...
		while (strpos($text, "{order ") !== FALSE)
		{
			...
			$replace = "<a href='#' onclick='limeticketAdminOrder(\"$field\");return false;'>" . $display . "</a>";
