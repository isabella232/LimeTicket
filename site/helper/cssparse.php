...
class LIMETICKET_CSSParse
{
	...
	static function OutputCSS($file, $use_less17 = false)
	{
		...
		if (!file_exists($jpc.DS.'limeticket'.DS.'css'))
			mkdir($jpc.DS.'limeticket'.DS.'css',0777,true);
		...
		if (LIMETICKET_Settings::get('css_indirect'))
			$out_filename = str_replace(".","_", $out_filename);
				
		$out_file = $jpc.DS.'limeticket'.DS.'css'.DS.$out_filename;
		
		if (!is_file($out_file) || filemtime($in_file) > filemtime($out_file)) {

			if ($use_less17)
			{
				if (!class_exists("Less_Parser")) require_once(JPATH_SITE.DS.'components'.DS.'com_limeticket'.DS.'helper'.DS.'third'.DS.'lessc170.php');
				$less = new Less_Parser;
				$less->parseFile($in_file);
				$output = $less->getCss();
			} else {
				require_once(JPATH_SITE.DS.'components'.DS.'com_limeticket'.DS.'helper'.DS.'third'.DS.'lessc.php');
				$less = new limeticket_lessc;
		...
		if (LIMETICKET_Settings::get('css_indirect'))
		{
			$document->addStyleSheet(JRoute::_("index.php?option=com_limeticket&view=css&file=" . $out_filename, false));
		} else {
			$document->addStyleSheet(JURI::root(true) . "/cache/limeticket/css/" . $out_filename);
		}
	...
	static function OutputCSS_New($file)
	{
		...
		if (!is_file($out_file) || filemtime($in_file) > filemtime($out_file)) {
			require_once(JPATH_SITE.DS.'components'.DS.'com_limeticket'.DS.'helper'.DS.'third'.DS.'lessc170.php');

			//jimport('fsj_core.less.lessc');
			$less = new Less_Parser;
			//$less = new \limeticket_lessc170\Less_Parser;
			$less->parseFile($in_file);
			$output = $less->getCss();
	...
	static function ParseStaticFiles()
	{
		...
		if (!self::ParseStaticFile(JPATH_SITE.'/components/com_limeticket/assets/css/bootstrap/bootstrap_missing.less', JPATH_SITE.'/components/com_limeticket/assets/css/bootstrap/bootstrap_missing.parsed.less'))
		{
			$ok = false;
			self::$failed[] = JPATH_SITE.'/components/com_limeticket/assets/css/bootstrap/bootstrap_missing.parsed.less';
		}
		
		if (!self::ParseStaticFile(JPATH_SITE.'/components/com_limeticket/assets/css/variables.less', JPATH_SITE.'/components/com_limeticket/assets/css/variables.parsed.less'))
		{
			$ok = false;
			self::$failed[] = JPATH_SITE.'/components/com_limeticket/assets/css/variables.parsed.less';
		}
		
		if (LIMETICKET_Settings::get('bootstrap_variables') != "")
		{
			if (!self::ParseStaticFile(JPATH_SITE.'/components/com_limeticket/assets/css/bootstrap/variables.override.less', JPATH_SITE.'/components/com_limeticket/assets/css/bootstrap/variables.parsed.less'))
			{
				$ok = false;
				self::$failed[] = JPATH_SITE.'/components/com_limeticket/assets/css/bootstrap/variables.parsed.less';
			}
		} else {
			file_put_contents(JPATH_SITE.'/components/com_limeticket/assets/css/bootstrap/variables.parsed.less', "");
		}
	
		if (!$ok)
			return false;	
		
		// tag the limeticket.less file so the parser knows it needs to be updated
		@touch(JPATH_SITE.'/components/com_limeticket/assets/css/limeticket.less');
		@touch(JPATH_SITE.'/components/com_limeticket/assets/css/bootstrap/bootstrap.less');
		@touch(JPATH_SITE.'/components/com_limeticket/assets/css/bootstrap/bootstrap_limeticketonly.less');
		...
		foreach($css_arr[0] as $key=>$val) {
			...
			if(!preg_match('/('.implode('|', array_map('preg_quote', $escaped)).')/i', $css_arr[1][$key])) {
				if(preg_match('/left|right/i', $css_arr[2][$key])) {
					if($rules = LIMETICKET_CSSParse::rtlCSSRule($css_arr[2][$key])) {
