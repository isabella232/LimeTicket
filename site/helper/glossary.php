...
class LIMETICKET_Glossary
{
	...
	static function isWordCS($data)
	{
		...
		if (LIMETICKET_Settings::get('glossary_case_sensitive') == 2)
			$iscs = 1;
		
		if (LIMETICKET_Settings::get('glossary_case_sensitive') == 1 && $data->word == strtoupper($data->word))
			$iscs = 1;
	...
	{
		if (empty(LIMETICKET_Glossary::$glossary))
		{
			$db = JFactory::getDBO();
			...
			// add javascript
			LIMETICKET_Helper::StylesAndJS(array('tooltip', 'glossary'));
		}
	}
	...
	static function ReplaceGlossary($text)
	{
		...
		LIMETICKET_Settings::GetAllViewSettings();
		...
		{
			$data->limit = 999;
			if (LIMETICKET_Settings::get('glossary_word_limit') > 0)
				$data->limit = LIMETICKET_Settings::get('glossary_word_limit');
			...
			if (LIMETICKET_Settings::get('glossary_link'))
			{
				if (LIMETICKET_Settings::$limeticket_view_settings['glossary_long_desc'] == 1 && $data->longdesc) // if long description is set to be shown an a separate page and we have a long description available
				{
					// link directly to the item
					$data->href = LIMETICKETRoute::_("index.php?option=com_limeticket&view=glossary&layout=word&word=" . $linkref, false);
				} else {
					$data->href = LIMETICKETRoute::_( 'index.php?option=com_limeticket&view=glossary&letter='.strtolower(substr($data->linkword,0,1)).'#' . $linkanc, false );
				}
			}
		...
		$domx = "f1f819543e6ffd61e17bf27aa460ce0a";
		
		// get list of html tags to ignore
		$tag_ignore = LIMETICKET_Settings::get('glossary_exclude');
		...
		LIMETICKET_Glossary::preg_replace_dom($dom->documentElement, $tags);
	...
	static function preg_replace_dom(DOMNode $dom, array $exclude = array()) {
		...
		foreach ($dom->childNodes as $node) {
			...
			if ($node instanceof DOMText) // only process text elements
			{
				...
				if ($totalcount)
				{
					...
					while ($pos !== FALSE && $runs++ < 50) // while we have found an instance of 
					{
						...
						// build link element
						$link_elem = LIMETICKET_Glossary::$cdom->createElement('a', $text);					
						$link_elem->setAttribute("href", $info->href);
						$link_elem->setAttribute("class", 'limeticket_glossary_word');
						$link_elem->setAttribute("ref", $info->ref);
						$link_elem->setAttribute("context", LIMETICKET_Glossary::$context);
			...
			{
				LIMETICKET_Glossary::preg_replace_dom($node, $exclude);
			}
		}
	}
	...
	{
		LIMETICKET_Glossary::GetGlossary();
		
		if (count(LIMETICKET_Glossary::$glossary) == 0)
			return "";
		...
		foreach(LIMETICKET_Glossary::$glossary as $data)
		{
			if (empty($data->inuse) || !$data->inuse)
				continue;
			...
			$count++;
			$footer = "";
			if ($data->longdesc && LIMETICKET_Settings::get('glossary_show_read_more'))
				$footer = "<p class='right limeticket_glossary_read_more' style='text-align: right'>" . JText::_(LIMETICKET_Settings::get('glossary_read_more_text')) . "</p>";
			
			if (LIMETICKET_Settings::get('glossary_title'))
			{
