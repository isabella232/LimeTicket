...
class SupportCanned
{
	...
	static function GetCannedReplies()
	{
		...
		{
			$db = JFactory::getDBO();
			$qry = "SELECT * FROM #__limeticket_ticket_fragments WHERE type = 0";
			$db->setQuery($qry);
	...
	static function CannedDropdown($editorid, $hascontainer = true, $ticket = null)
	{
		...
		{
			if (LIMETICKET_Settings::get('support_insertpopup') || LIMETICKET_Settings::get('bootstrap_v3'))
			{
		...
		$gui_plugin = LIMETICKET_GUIPlugins::output("adminCannedDropdown", array('ticket'=> $ticket, 'editor' => $editorid));
		if ($gui_plugin)
		{
			$html[] = $gui_plugin;
			$html[] = '<li class="divider"></li>';
		...
		{
			$html[] = '<li><a class="show_modal_iframe" href="' . LIMETICKETRoute::_("index.php?option=com_limeticket&view=admin_insert&tmpl=component&type=".$plugin->name."&editor=".$editorid) . '" data_modal_width="800">'.JText::_($plugin->title) . '</a></li>';
		}
		$html[] = '</ul></li>';
		$html[] = '<li class="divider"></li>';
		$html[] = '<li><a class="show_modal_iframe" href="' . LIMETICKETRoute::_("index.php?option=com_limeticket&view=admin_support&tmpl=component&layout=canned" ) . '"><i class="icon-edit"></i> '.JText::_('EDIT') . '</a></li>';
	...
	{
		$path = JPATH_SITE.DS.'components'.DS.'com_limeticket'.DS.'plugins'.DS.'picktable'.DS;
	...
	static function GetAllSigs($ticket)
	{
		if (empty(self::$all_sigs))
		{
			...
			$db = JFactory::getDBO();
			$query = 'SELECT * FROM #__limeticket_ticket_fragments WHERE type = 1';
			$db->setQuery($query);
			...
			$qry = "SELECT * FROM #__limeticket_users WHERE user_id = $userid";
			$db->setQuery($qry);
			$user = $db->loadObject();
			if ($user)
	...
	static function ParseSig($text, $ticket, $bbcode = true)
	{
		...
		//LIMETICKETParserTicket::core($parser, $ticket);
		...
		if ($bbcode)
			return LIMETICKET_Helper::ParseBBCode($text);
	...
	static function AppendSig($sigid, $ticket)
	{
		...
		$db = JFactory::getDBO();
		$qry = "SELECT * FROM #__limeticket_ticket_fragments WHERE id = " . LIMETICKETJ3Helper::getEscaped($db, $sigid);
		$db->setQuery($qry);
