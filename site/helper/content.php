...
class LIMETICKET_ContentEdit extends LIMETICKETView
{
	var $has_added = 0;
	var $has_ordering = 0;
	var $has_modified = 0;
	...
	function Init()
	{
		...
		{
			$this->tmplpath = JPATH_SITE.DS.'components'.DS.'com_limeticket'.DS.'tmpl'.DS.'content';	
	...
	{
		if (!LIMETICKET_Permission::auth("core.edit", $this->getAsset()) && !LIMETICKET_Permission::auth("core.edit.own", $this->getAsset()))
			return LIMETICKET_Admin_Helper::NoPerm();
		
		$this->Init();

		$db = JFactory::getDBO();
		$this->what = LIMETICKET_Input::getCmd('what','');
		...
		$return = LIMETICKET_Input::getString('return','');
		if ($return == 1)
		{
		...
		if ($this->what == "cancel")
		{
			...
			$link = LIMETICKETRoute::_('index.php?option=com_limeticket&view=admin_content&type=' . $this->id,false);
			$return = LIMETICKET_Input::getString('return','');
			if ($return && $return != 1)
				$link = $return;
		...
		if ($this->what == "edit")
		{
			...
			if (LIMETICKET_Permission::auth("core.edit", $this->getAsset()))
	...
	function Create()
	{
		if (!LIMETICKET_Permission::auth("core.create", $this->getAsset()))
			return LIMETICKET_Admin_Helper::NoPerm();
		...
		if (LIMETICKET_Permission::auth("core.edit.state", $this->getAsset()))
		{
		...
		if (LIMETICKET_Input::getString("title"))
			$this->item['title'] = LIMETICKET_Input::getString("title");
		
		if (LIMETICKET_Input::getString("body"))
			$this->item['body'] = LIMETICKET_Input::getHTML('body', '');
		
		if (LIMETICKET_Input::getString("question"))
			$this->item['question'] = LIMETICKET_Input::getString("question");
		
		if (LIMETICKET_Input::getString("answer"))
			$this->item['answer'] = LIMETICKET_Input::getHTML('answer', '');
	...
	function Save()
	{
		$db = JFactory::getDBO();
		$this->item = array();
		$this->item['id'] = LIMETICKET_Input::getInt('id',0);
		$user = JFactory::getUser();
		$userid = $user->get('id');
		...
		{	
			...
			$this->item[$field->field] = LIMETICKET_Input::getString($field->input_name,'');
			if ($field->type == "text")
				$this->item[$field->field] = LIMETICKET_Input::getHTML($field->input_name, '');	
		...
		if ($ok)
		{
			...
			if ($this->item['id'])
			{
				...
				foreach ($this->edit as $edit)
				{
					...
					if ($field->type != "related" && $field->type != "tags")
						$sets[] = $field->field . " = '" . LIMETICKETJ3Helper::getEscaped($db, $this->item[$field->field]) . "'";
					if ($field->more)
						$sets[] = "`".$field->more . "` = '" . LIMETICKETJ3Helper::getEscaped($db, $this->item[$field->more]) . "'";
				}

				if (LIMETICKET_Permission::auth("core.edit.state", $this->getAsset()))
				{
					$sets[] = "published = " . LIMETICKET_Input::getInt('published',0);	
					if ($this->has_author)
						$sets[] = "author = " . LIMETICKET_Input::getInt('author',0);
				}
				...
				$qry .= " WHERE id = '". LIMETICKETJ3Helper::getEscaped($db, $this->item['id']) . "'";
			} else {
				$fieldlist = array();
				if ($this->has_author)
					$fieldlist[] = "author";
				...
				foreach($this->edit as $edit)
				{
					...
					$setlist[] = "'" . LIMETICKETJ3Helper::getEscaped($db, $this->item[$field->field]) . "'";
					if ($field->more)
					{
						$fieldlist[] = "`".$field->more."`";	
						$setlist[] = "'" . LIMETICKETJ3Helper::getEscaped($db, $this->item[$field->more]) . "'";
					}
				...
				if (LIMETICKET_Permission::auth("core.edit.state", $this->getAsset()))
				{
					$setlist[] = LIMETICKET_Input::getInt('published',0);	
				} else {
					$setlist[] = "0";	
				}
			...
			foreach($this->edit as $edit)
			{
				...
				if ($field->type == "products")
				{
					...
					$qry = "DELETE FROM {$field->prod_table} WHERE {$field->prod_artid} = '".LIMETICKETJ3Helper::getEscaped($db, $this->item['id'])."'";
					$db->setQuery($qry);
					//echo $qry."<br>";
					$db->query($qry);
					...
					if (!$this->item[$field->field])
					{
						foreach ($this->products as &$product)
						{
							...
							$val = LIMETICKET_Input::getString($name);
							if ($val == "on")
							{
								$qry = "INSERT INTO {$field->prod_table} ({$field->prod_prodid}, {$field->prod_artid}) VALUES
									($pid, '".LIMETICKETJ3Helper::getEscaped($db, $this->item['id'])."')";
								$db->setQuery($qry);
								//echo $qry."<br>";
								$db->query($qry);
							}
				...
				} elseif ($field->type == "related")
				{
					...
					$qry1 = "DELETE FROM {$field->rel_table} WHERE {$field->rel_id} = '". LIMETICKETJ3Helper::getEscaped($db, $this->item['id']) . "'";
					$db->setQuery($qry1);
					//echo $qry1."<br>";
					$db->query();
					...
					{
						$id = LIMETICKETJ3Helper::getEscaped($db, $id);
						$qry1 = "REPLACE INTO {$field->rel_table} ({$field->rel_id}, {$field->rel_relid}) VALUES ('". LIMETICKETJ3Helper::getEscaped($db, $this->item['id']) . "', '$id')";
						$db->setQuery($qry1);
						//echo $qry1."<br>";
						$db->query();
					}
				...
				} else if ($field->type == "tags")
				{
					...
					$qry1 = "DELETE FROM {$field->tags_table} WHERE {$field->tags_key} = '". LIMETICKETJ3Helper::getEscaped($db, $this->item['id']) . "'";
					//echo $qry1 . "<br>";
					$db->setQuery($qry1);
					$db->query();
					...
					foreach ($tags as $tag)
					{
						$tag = trim($tag);
						if (!$tag) continue;
						$qry1 = "REPLACE INTO {$field->tags_table} ({$field->tags_key}, tag, language) VALUES (	'". LIMETICKETJ3Helper::getEscaped($db, $this->item['id']) . "', ";
						$qry1 .= "'". LIMETICKETJ3Helper::getEscaped($db, $tag) . "', '". LIMETICKETJ3Helper::getEscaped($db, $this->item['language']) . "')";
						//echo $qry1 . "<br>";
						$db->setQuery($qry1);
						$db->query();
			...
			if ($this->what == "apply")
			{
				$link = LIMETICKETRoute::_("index.php?option=com_limeticket&view=admin_content&type={$this->id}&what=edit&id={$this->articleid}",false);
			} elseif ($this->what == "savenew")
			{
				$link = LIMETICKETRoute::_("index.php?option=com_limeticket&view=admin_content&type={$this->id}&what=new",false);
			} else {
				$link = LIMETICKETRoute::_('index.php?option=com_limeticket&view=admin_content&type=' . $this->id,false);
				$return = LIMETICKET_Input::getString('return','');
				if ($return && $return != 1)
			...
			foreach($this->edit as $edit)
			{
				...
				if ($field->type == "related")
				{
					...
					$relids = LIMETICKET_Input::getString($field->field);
					...
					if (count($field->rel_ids) > 0)
					{
						...
						foreach ($field->rel_ids as $id)
							$ids[] = LIMETICKETJ3Helper::getEscaped($db, $id);
	...
	///					$qry = "SELECT {$field->rel_lookup_id}, {$field->rel_lookup_display} FROM {$field->rel_lookup_table} WHERE {$field->rel_lookup_id} IN (" . implode(", ", $field->rel_ids) . ")";
				...
				} else if ($field->type == "products")
				{
					...
					foreach($this->products as $product)
					{
						$prodform = LIMETICKET_Input::getString($field->field . "_prod_" . $product->id);
						if ($prodform == "on")
						{
							$prodcheck .= '<label class="checkbox">';
			...
			if (LIMETICKET_Permission::auth("core.edit.state", $this->getAsset()))
			{
				$this->item['published'] = LIMETICKET_Input::getInt('published',0);
				$this->item['author'] = LIMETICKET_Input::getInt('author',0);
			}
	...
	function Header()
	{
		...
		echo LIMETICKET_Helper::PageStyle();
		echo LIMETICKET_Helper::PageTitle("CONTENT_MANAGEMENT");

		include $this->snippet(JPATH_SITE.DS.'components'.DS.'com_limeticket'.DS.'views'.DS.'admin'.DS.'snippet'.DS.'_tabbar.php');
		include $this->snippet(JPATH_SITE.DS.'components'.DS.'com_limeticket'.DS.'views'.DS.'admin_content'.DS.'snippet'.DS.'_contentbar.php');
	}
	
	function Footer()
	{
		include JPATH_SITE.DS.'components'.DS.'com_limeticket'.DS.'_powered.php';
		echo LIMETICKET_Helper::PageStyleEnd();	
	}
	...
	function GetListFilter()
	{
		...
		if ($this->has_author)
		{
			...
			if (LIMETICKET_Permission::auth("core.edit.state", $this->getAsset()))
			{
				...
				$this->filter_html['userid'] = JHTML::_('select.genericlist',  $users, 'userid', 'class="input-medium" onchange="document.limeticketForm.submit( );"', 'id', 'name', $this->filter_values['userid']);
			}
		}
		// published
		...
		$this->filter_html['published'] = JHTML::_('select.genericlist',  $published, 'ispublished', 'class="input-medium" onchange="document.limeticketForm.submit( );"', 'id', 'title', $this->filter_values['published']);
		...
		foreach ($this->filters as $filter)
		{
			...
			if ($filter->source_table)
			{
				...
				if ($filter->nested)
				{
					require_once (JPATH_SITE.DS.'components'.DS.'com_limeticket'.DS.'helper'.DS.'nested.php');
					$filtervalues = LIMETICKET_Nested_Helper::BuildNest($filtervalues, "id", "parcatid", "ordering");
			...
			$this->filter_html[$filter->field] = JHTML::_('select.genericlist',  $filtervalues, $filter->input_name, 'class="input-medium" onchange="document.limeticketForm.submit( );"', $filter->source_id, $filter->source_display, $this->filter_values[$filter->field]);
		}
	}
	...
	function getList()
	{
		...
		if ($this->has_author && !LIMETICKET_Permission::auth("core.edit", $this->getAsset()))
			$where[] = "a.author = {$this->userid}";
		
		if ($this->filter_values['published'] > 0)
			$where[] = "a.published = ".LIMETICKETJ3Helper::getEscaped($db, $this->filter_values['published']-1);
		
		if ($this->has_author && $this->filter_values['userid'] > 0)
			$where[] = "a.author = ".LIMETICKETJ3Helper::getEscaped($db, $this->filter_values['userid']);
		...
		if ($this->filter_values['search'] != "")
		{
			...
			foreach($this->fields as $field)
			{
				if ($field->search)
				{
					$search[] = "{$field->field} LIKE '%".LIMETICKETJ3Helper::getEscaped($db, $this->filter_values['search'])."%'";	
				}	
			}	
		...
		foreach ($this->filters as $filter)
		{
			...
			if ($value)
			{
				$where[] = "a.{$filter->field} = '".LIMETICKETJ3Helper::getEscaped($db, $value) . "'";
			}
		}
		...
		{
			$qry .= " ORDER BY " . LIMETICKETJ3Helper::getEscaped($db, $this->filter_values['order']) . " " . LIMETICKETJ3Helper::getEscaped($db, $this->filter_values['order_dir']);
		} else {
	...
	function getSingle()
	{
		...
		if (!LIMETICKET_Permission::auth("core.edit", $this->getAsset()) && $this->userid != $row['userid'])
		{
			return null;	
		}
		...
		foreach($this->edit as $edit)
		{
			...
			if ($field->type == "products")
			{
				$qry = "SELECT {$field->prod_prodid} FROM {$field->prod_table} WHERE {$field->prod_artid} = '".LIMETICKETJ3Helper::getEscaped($db, $id)."'";
				$db->setQuery($qry);
			...
			{
				$qry = "SELECT {$field->rel_relid} FROM {$field->rel_table} WHERE {$field->rel_id} = '".LIMETICKETJ3Helper::getEscaped($db, $id)."'";
				$db->setQuery($qry);
				$field->rel_ids = array();
			...
			{
				$qry = "SELECT * FROM {$field->tags_table} WHERE {$field->tags_key} = '".LIMETICKETJ3Helper::getEscaped($db, $id)."'"; 
				$db->setQuery($qry);
				$row['tags'] = array();
	...
	function getArtLink()
	{
		...
		return LIMETICKETRoute::_($link, false); // OK
	}
	...
	function GetProducts()
	{
		...
		{
			$db = JFactory::getDBO();
			$query = "SELECT * FROM #__limeticket_prod WHERE published = 1 ORDER BY title";
			$db->setQuery($query);
	...
	function LookupInput($field, $item)
	{
		if (property_exists($field, "lookup_table") && $field->lookup_table)
		{
			...
			{
				require_once (JPATH_SITE.DS.'components'.DS.'com_limeticket'.DS.'helper'.DS.'nested.php');
				$sections = LIMETICKET_Nested_Helper::BuildNest($sections, "id", "parcatid", "ordering");
	...
	function GetLookupValues($field)
	{
		...
		{
			$ids[$item[$field->field]] = LIMETICKETJ3Helper::getEscaped($db, $item[$field->field]);	
		}
	...
	function HandlePick()
	{
		...
		$f = LIMETICKET_Input::getString('field');
		...
		if ($this->filter_values['search'] != "")
		{
			...
			{
				$search[] = "{$searchfield} LIKE '%".LIMETICKETJ3Helper::getEscaped($db, $this->filter_values['search'])."%'";	
			}	
		...
		foreach ($this->filters as $filter)
		{
			$value = LIMETICKET_Input::getString($filter->field);
			if ($value > 0)
			{
				$where[] = "a.{$filter->field} = ".LIMETICKETJ3Helper::getEscaped($db, $value);
			}
	...
	function HandlePickFilter($field)
	{
		...
		if ($this->has_author)
		{
			...
			if (LIMETICKET_Permission::auth("core.edit.state", $this->getAsset()))
			{
				...
				$users[] = JHTML::_('select.option', '0', JText::_("SELECT_USER"), 'id', 'name');
				$users = array_merge($users, $db->loadObjectList());
				$this->filter_html['userid'] = JHTML::_('select.genericlist',  $users, 'userid', 'class="input-medium" size="1" onchange="document.limeticketForm.submit( );"', 'id', 'name', $this->filter_values['userid']);
			}
		}
		// published
		...
		$published[] = JHTML::_('select.option', '2', JText::_("PUBLISHED"), 'id', 'title');
		$published[] = JHTML::_('select.option', '1', JText::_("UNPUBLISHED"), 'id', 'title');
		$this->filter_html['published'] = JHTML::_('select.genericlist',  $published, 'ispublished', 'class="input-medium" size="1" onchange="document.limeticketForm.submit( );"', 'id', 'title', $this->filter_values['published']);
		...
		foreach ($field->filters as $filter)
		{
			...
			$this->filter_html[$filter->field] = JHTML::_('select.genericlist',  $filtervalues, $filter->field, 'class="input-medium" size="1" onchange="document.limeticketForm.submit( );"', $filter->source_id, $filter->source_display, $this->filter_values[$filter->field]);
		}
	}
	...
	function HandlePublish()
	{
		...
		if (LIMETICKET_Permission::auth("core.edit.state", $this->getAsset()))
		{
			...
			$qry = "UPDATE {$this->table} SET published = $pub WHERE id = '".LIMETICKETJ3Helper::getEscaped($db, $id)."'";
	...
	function EditPanel($item,$changeclass = true)
	{
		...
		if (LIMETICKET_Permission::auth("core.edit", $this->getAsset()) || (LIMETICKET_Permission::auth("core.edit.own", $this->getAsset()) && $item['author'] == $this->userid))
		{
			$label_type = "success";
			if (!$item['published'])
				$label_type = "warning";
			...
			$output .= "<a href='".LIMETICKETRoute::_("index.php?option=com_limeticket&view=admin_content&type={$this->id}&what=edit&id={$item['id']}&option=com_limeticket&return=1")."' class='limeticketTip' title='".JText::_('EDIT_ARTICLE')."'><img src='" . JURI::root( true ). "/components/com_limeticket/assets/images/edit.png' alt='Edit' /></a>";	
			...
			if (LIMETICKET_Permission::auth("core.edit.state", $this->getAsset())) 
			{
				$output .= '<a href="#" id="publish_' .  $item['id'] . '" class="limeticket_publish_button limeticketTip" state="' . $item['published'] .'" title="' .  $tip . '">';
				$output .= LIMETICKET_Helper::GetPublishedText($item['published'],true);
				$output .= '</a>';
			} else {
				$output .= str_replace("_16.png","_g_16.png",LIMETICKET_Helper::GetPublishedText($item['published']));
			}
	...
	static function getArticleCounts()
	{
		if (empty(self::$artcounts))
		{
			...
			{
				require_once (JPATH_SITE.DS.'components'.DS.'com_limeticket'.DS.'helper'.DS.'content'.DS.$type.'.php');
				$class = "LIMETICKET_ContentEdit_$type";
				$content = new $class();
	...
	function auth($item)
	{
		return LIMETICKET_Permission::auth($item, $this->getAsset());	
	}
}

class LIMETICKET_Content_Field
{
	var $field = "";
	var $type = "string";
	var $desc = "";
...
class LIMETICKET_Content_Filter
{
