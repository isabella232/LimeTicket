 ...
 * 
 * REPLACES the old TicketHelper (LIMETICKET_Ticket_Helper)
**/
...
class SupportHelper
{
	...
	static function getDataList($sort, $key, $query, $translate = false)
	{
		...
		if ($sort)
		{
			if ($translate) LIMETICKET_Translate_Helper::Tr(self::$data_lists[$key]['s']);
			return self::$data_lists[$key]['s'];		
		}

		if ($translate) LIMETICKET_Translate_Helper::Tr(self::$data_lists[$key]['i']);
	...
	static function getPriorities($sort = true, $order = "ordering", $for_user = false, $user = null)
	{
		...
		return self::getDataList($sort, "priorities-" . $order, "SELECT * FROM #__limeticket_ticket_pri ORDER BY " . $order, true);
	}
	...
	{
		$data = self::getDataList($sort, "statuss-" . $order, "SELECT * FROM #__limeticket_ticket_status ORDER BY " . $order, true);
	...
	{
		return self::getDataList($sort, "categories-" . $order, "SELECT * FROM #__limeticket_ticket_cat WHERE published = 1 ORDER BY " . $order, true);
	}
	
	static function getProducts($sort = true, $order = "ordering")
	{
		return self::getDataList($sort, "products-" . $order, "SELECT * FROM #__limeticket_prod WHERE published = 1 ORDER BY " . $order, true);
	}
	...
	static function getProductsUserOpen($limitstart = 0, $limit = 50, $search = "", $user = null)
	{
		...
		{
			LIMETICKET_Translate_Helper::Tr(self::$user_open_products[$key]);
		...
		$query = "SELECT *, 0 as type FROM #__limeticket_prod";
		$where = array();
		$where[] = "insupport = 1";
		$order = array();
		$order[] = "ordering";
		...
		if (LIMETICKET_Settings::get('support_restrict_prod'))
			$where[] = "id = 0";
		...
		$qry = "SELECT *, 1 as type FROM #__limeticket_prod";
		$where = array();
		$where[] = "insupport = 1";
		$where[] = "published = 1";
		...
		if (!LIMETICKET_Settings::get('support_product_manual_category_order'))
		{
	...
	static function getDepartmentsUserOpen($prodid, $limitstart = 0, $limit = 50, $search = "", $user = null)
	{
		...
		{
			LIMETICKET_Translate_Helper::Tr(self::$user_open_departments[$key]);
			return self::$user_open_departments[$key];
		}

		$db = JFactory::getDBO();
	
		$search = LIMETICKET_Input::getString('deptsearch');  
		$swhere = "";
		if ($search != "__all__" && $search != '')
		...
		$query1 = "SELECT * FROM #__limeticket_ticket_dept";
		$where = array();
		...
		$query2 = "SELECT * FROM #__limeticket_ticket_dept";
		...
		$where[] = "id IN ( SELECT ticket_dept_id FROM #__limeticket_ticket_dept_prod WHERE prod_id = '".$db->escape($prodid)."' )";
	...
	static function getCategoriesUserOpen($prodid, $deptid, $user = null)
	{
		...
		{
			LIMETICKET_Translate_Helper::Tr(self::$user_open_categories[$key]);
		...
		$query = "SELECT * FROM #__limeticket_ticket_cat_prod WHERE prod_id = '".$db->escape($prodid)."'";
		$db->setQuery($query);
		$products = $db->loadAssocList('ticket_cat_id');
		
		$query = "SELECT * FROM #__limeticket_ticket_cat_dept WHERE ticket_dept_id = '".$db->escape($deptid)."'";
		$db->setQuery($query);
	...
	{
		return self::getDataList($sort, "departments-" . $order, "SELECT * FROM #__limeticket_ticket_dept WHERE published = 1 ORDER BY " . $order, true);
	}
	
	static function getTicketGroups($sort = true, $order = "groupname")
	{
		return self::getDataList($sort, "ticketgroups-" . $order, "SELECT * FROM #__limeticket_ticket_group ORDER BY " . $order);
	}
	
	static function getTags($sort = true, $order = "cnt DESC", $limit = 10)
	{
		return self::getDataList($sort, "tags-" . $order . "-" . $limit, "SELECT count(*) as cnt, tag, tag as id FROM #__limeticket_ticket_tags GROUP BY tag ORDER BY {$order} LIMIT {$limit}");
	}
	...
	static function getAllowedCategories($ticket, $sort = true, $order = "ordering, section, title")
	{
		...
		//return self::getDataList($sort, "allowed-categories-" . $order . "-" . $ticket->id, "SELECT * FROM #__limeticket_ticket_cat WHERE published = 1 ORDER BY " . $order, true);
	}
	...
	static function parseRedirectType($status, $type)
	{
		...
		if ($type == "" && $ticketid > 0)
			return LIMETICKETRoute::_('index.php?option=com_limeticket&view=admin_support&layout=ticket&ticketid=' . LIMETICKET_Input::getInt('ticketid'), false);
		...
		if (count($bits) != 2)
		{
			if ($ticketid > 0)
			{
				...
				return LIMETICKETRoute::_('index.php?option=com_limeticket&view=admin_support', false);
			}
		}
		
		if ($bits[0] == "list")
			return LIMETICKETRoute::_('index.php?option=com_limeticket&view=admin_support&tickets=' . $bits[1], false);
		...
		if ($bits[0] == "new" || $bits[0] == "old")
		{
			...
			if ($bits[0] == "new" && $newset_id > 0)
				return LIMETICKETRoute::_('index.php?option=com_limeticket&view=admin_support&layout=ticket&ticketid=' . $newset_id, false);
			
			if ($bits[0] == "old" && $oldest_id > 0)
				return LIMETICKETRoute::_('index.php?option=com_limeticket&view=admin_support&layout=ticket&ticketid=' . $oldest_id, false);
			
			return LIMETICKETRoute::_('index.php?option=com_limeticket&view=admin_support&tickets=' . $bits[1], false);
		}
		
		if ($ticketid > 0)
			return LIMETICKETRoute::_('index.php?option=com_limeticket&view=admin_support&layout=ticket&ticketid=' . LIMETICKET_Input::getInt('ticketid'));
	...
	static function getUserTicketCountsForAdmin($userid,$email)
	{
		...
		if ($userid)
		{
			$qry = "SELECT count(*) as cnt, ticket_status_id FROM #__limeticket_ticket_ticket WHERE user_id = '".LIMETICKETJ3Helper::getEscaped($db, $userid)."' GROUP BY ticket_status_id";
		} else {
			$qry = "SELECT count(*) as cnt, ticket_status_id FROM #__limeticket_ticket_ticket WHERE email = '".LIMETICKETJ3Helper::getEscaped($db, $email)."' GROUP BY ticket_status_id";
		}
	...
	static function getUserTicketCount($user = null)
	{
		...
		$db = JFactory::getDBO();
		$query = "SELECT count( * ) AS count, ticket_status_id FROM #__limeticket_ticket_ticket WHERE (user_id IN (".implode(", ",$uidlist) . ") OR id IN ( " . implode(", ",$tidlist) . ")) ";
		...
		$archived = LIMETICKET_Ticket_Helper::GetStatusID("def_archive");
		...
		$allopen = LIMETICKET_Ticket_Helper::GetStatusIDs("is_closed", true);
		$out['open'] = 0;
		...
		$allclosed = LIMETICKET_Ticket_Helper::GetClosedStatus();
		$out['closed'] = 0;
		...
		$all = LIMETICKET_Ticket_Helper::GetStatusIDs("def_archive");
		$out['all'] = 0;
		foreach ($rows as $row)
		{
	...
	static function getUserProdIDs($user = null)
	{
		...
		$qry = "SELECT * FROM #__limeticket_ticket_group_members WHERE user_id = '".LIMETICKETJ3Helper::getEscaped($db, $userid)."'";
		$db->setQuery($qry);
		...
		$qry = "SELECT prod_id FROM #__limeticket_ticket_group_prod WHERE group_id IN (" . implode(", ", $gids) . ")";
		$db->setQuery($qry);	
			...
			<span class="<?php if ($message->time < 0): ?> text-error <?php endif; ?>">
				...
				<?php 	
					...
					} else if ($message->timestart > 0 && $message->timeend > 0)
					{
						...
						echo "<i class='ticket_time_date'>" . LIMETICKET_Helper::Date($message->timestart, LIMETICKET_DATETIME_SHORT) . " - " . LIMETICKET_Helper::Date($message->timeend, LIMETICKET_DATETIME_SHORT) . "</i> (<b>";
					} else {
	...
	static function attachThumbnail($ticketid, $fileid, $for_user = false)
	{
		...
		if ($ticket->load($ticketid, $for_user))
		{
			...
			$thumb_file = JPATH_SITE.DS.LIMETICKET_Helper::getAttachLocation().DS."thumbnail".DS.$attach->diskfile.".thumb";
			...
			LIMETICKET_File_Helper::Thumbnail($image_file, $thumb_file);
		}
	...
	static function attachDownload($ticketid, $fileid, $for_user = false)
	{
		...
		if ($ticket->load($ticketid, $for_user))
		{
			...
			$file = JPATH_SITE.DS.LIMETICKET_Helper::getAttachLocation().DS."support".DS.$attach->diskfile;
					
			$display_filename = LIMETICKET_Helper::basename($attach->filename);
			
			if (!$for_user)
			{
				$user = JFactory::GetUser($attach->user_id);      
				$type = LIMETICKET_Settings::get('support_filename');
				switch ($type)
				{
					case 1:
			...
			require_once (JPATH_SITE.DS.'components'.DS.'com_limeticket'.DS.'helper'.DS.'files.php');
			LIMETICKET_File_Helper::DownloadFile($file, $display_filename);
		}
	...
	static function getTIDS($user_id)
	{
		...
		$qry = "SELECT ticket_id FROM #__limeticket_ticket_cc WHERE user_id = '".LIMETICKETJ3Helper::getEscaped($db,  $user_id)."' AND isadmin = 0";
		$db->setQuery($qry);
		$rows = $db->loadAssocList();
		$tidlist = array();
	...
	static function getProdIDS($user_id)
	{
		...
		$query = " SELECT * FROM #__limeticket_ticket_group_members as m LEFT JOIN #__limeticket_ticket_group AS g on m.group_id = g.id ";
		...
		$query = "SELECT prod_id FROM #__limeticket_ticket_group_prod WHERE group_id IN (" . implode($gids) . ")";		
		$db->setQuery($query);
	...
	static function getUIDS($user_id)
	{
		...
		// get groups
		$query = " SELECT * FROM #__limeticket_ticket_group_members ";
		$query .= " WHERE user_id = '".LIMETICKETJ3Helper::getEscaped($db, $user_id)."'";
		...
		{
			$gids[] = LIMETICKETJ3Helper::getEscaped($db, $group['group_id']);
		}
		
		$query = "SELECT * FROM #__limeticket_ticket_group WHERE id IN (" . implode(", ",$gids) .")";
		$db->setQuery($query);
		...
		foreach($grouplist as $group)
		{
			...
			{
				$gids[] = LIMETICKETJ3Helper::getEscaped($db, $group['id']);
			}
		}
		...
		$query = "SELECT user_id FROM #__limeticket_ticket_group_members WHERE group_id IN (" . implode(", ",$gids) .")";
		$db->setQuery($query);
	...
	static function getBaseSQL()
	{
		...
		$query .= " LEFT JOIN #__limeticket_ticket_dept as dept ON t.ticket_dept_id = dept.id ";
		$query .= " LEFT JOIN #__limeticket_ticket_cat as cat ON t.ticket_cat_id = cat.id ";
		$query .= " LEFT JOIN #__limeticket_prod as prod ON t.prod_id = prod.id ";
		$query .= " LEFT JOIN #__limeticket_ticket_pri as pri ON t.ticket_pri_id = pri.id ";
		$query .= " LEFT JOIN (SELECT group_id, user_id FROM #__limeticket_ticket_group_members GROUP BY user_id) as mem ON t.user_id = mem.user_id ";
		$query .= " LEFT JOIN #__limeticket_ticket_group as grp ON grp.id = mem.group_id ";
	...
	{
		$url = JRoute::_('index.php?option=com_limeticket&view=ticket&task=update.rating');
	...
	{
		$url = 'index.php?option=com_limeticket&view=ticket&task=update.ticketrating';
	...
	static function ratingChoose($rating, $ident, $url, $inline, $always_change, $text)
	{
		...
		if (!$inline) $output[] = "<div class='limeticket_ticket_rating' style='display: inline-block;' id='{$ident}' url='" . $url . "' wait='" . JText::_('PLEASE_WAIT') . "'>";
		if ($can_rate)
		{
			$output[] = "<a class='limeticketTip' href='#' onclick='return false;' title='". JText::_($text)."'>";
		} else {
		...
		JFactory::getDocument()->addScript(JURI::root(true).'/components/com_limeticket/assets/js/rating.js'); 
	...
	static function displayRating($rating, $showvalue = true)
	{
		...
		$output[] = "<div class='limeticket_ticket_rating' style='display: inline-block;'>";
