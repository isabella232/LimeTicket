...
class LIMETICKET_EMail_To
{
	...
	function addTicketAdmins($ticket)
	{
		...
		if ($ticket->admin_id > 0)
		{
			...
			$this->addAddress(LIMETICKET_Settings::get('support_email_unassigned'), 'Admin: Unassigned ticket setting');
		} 	

		// if email all admins is set
		if (LIMETICKET_Settings::get('support_email_all_admins') && !(LIMETICKET_Settings::get('support_email_all_admins_only_unassigned') && $ticket->admin_id > 0))
		{
			...
			$this->addUser(
				SupportUsers::getHandlersTicket(
						...
						LIMETICKET_Settings::get('support_email_all_admins_ignore_auto'), 
						LIMETICKET_Settings::get('support_email_all_admins_can_view'),
						false)
					, null, 'Admin: All admins setting');
		}
		
		// any cc emails need adding	
		$this->addAddress(LIMETICKET_Settings::get('support_email_admincc'), 'Admin: CC all setting');

		// any admins that are cc'd on the ticket
		$db = JFactory::getDBO();
		$qry = "SELECT user_id FROM #__limeticket_ticket_cc as c LEFT JOIN #__users as u ON c.user_id = u.id WHERE ticket_id = " . (int)$ticket->id . " AND isadmin = 1";
		$db->setQuery($qry);
	...
	function addTicketUsers($ticket)
	{
		...
		if ($ticket->id > 0)
		{
			$qry = "SELECT u.name, u.id, u.email, c.email as uremail FROM #__limeticket_ticket_cc as c LEFT JOIN #__users as u ON c.user_id = u.id WHERE c.ticket_id = {$ticket->id} AND isadmin = 0 ORDER BY name";
			$db->setQuery($qry);
		...
		if ($ticket->user_id > 0)
		{
			...
			$qry = "SELECT * FROM #__limeticket_ticket_group WHERE id IN (SELECT group_id FROM #__limeticket_ticket_group_members WHERE user_id = '".LIMETICKETJ3Helper::getEscaped($db, $ticket->user_id)."')";
			$db->setQuery($qry);
			...
			if (count($groups) > 0)
			{
				...
				$qry = "SELECT m.*, u.id as user_id, u.email, u.name FROM #__limeticket_ticket_group_members as m LEFT JOIN #__users as u ON m.user_id = u.id WHERE group_id IN (" . implode(", ",$gids) . ")";
				$db->setQuery($qry);
				$users = $db->loadObjectList();
