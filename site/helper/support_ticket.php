...
class SupportTicket
{
	...
	function create($data)
	{
		...
		$base_data = array(
			'ticket_status_id' => LIMETICKET_Ticket_Helper::GetStatusID('def_open'),
		...
		$qry = "INSERT INTO #__limeticket_ticket_ticket (" . implode(", ", $keys) . ") VALUES (" . implode(", ", $values) . ")";
		$db->setQuery($qry);
		$db->query();
		...
		$fields = LIMETICKETCF::GetCustomFields($this->id,$this->prod_id,$this->ticket_dept_id);
		LIMETICKETCF::StoreFields($fields,$this->id);
		
		$this->reference = LIMETICKET_Ticket_Helper::createRef($this->id);
		
		$qry = "UPDATE #__limeticket_ticket_ticket SET reference = '" . $db->escape($this->reference) . "' WHERE id = " . (int)$id;
		$db->setQuery($qry);
		$db->query();
	}
	...
	function setupUserPerimssions($userid = null)
	{
		...
		{
			$this->readonly = false;
			$this->canclose = LIMETICKET_Settings::get('support_user_can_close');
			return;
		}
		...
		$qry = "SELECT group_id FROM #__limeticket_ticket_group_members WHERE user_id = '" . $db->escape($this->user_id) . "'";
		$db->setQuery($qry);
		$owner_groups = $db->loadObjectList('group_id');
		
		// find a list of groups the user belongs to
		$qry = "SELECT * FROM #__limeticket_ticket_group_members WHERE user_id = '" . $db->escape($userid) . "'";
		$db->setQuery($qry);
		...
		$qry = "SELECT * FROM #__limeticket_ticket_group WHERE id IN (" . implode(", ", $gids) . ")";
		$db->setQuery($qry);
		...
		foreach($groups as $group_id => $group)
		{
			...
			if ($perm > 2) $this->canclose = LIMETICKET_Settings::get('support_user_can_close');
		}
	...
	{
		$db = JFactory::getDBO();
		$qry = "SELECT id FROM #__limeticket_ticket_ticket WHERE id = " . $db->escape($ticket_id);
		$db->setQuery($qry);
		$row = $db->loadObject();
		if ($row)
			return true;
	...
	function loadTags()
	{
		if (!$this->loaded_tags)
		{		
			$qry = "SELECT * FROM #__limeticket_ticket_tags WHERE ticket_id = {$this->id} ORDER BY tag";
			$db = JFactory::getDBO();
			$db->setQuery($qry);
	...
	function loadAttachments()
	{
		...
		{
			$db = JFactory::getDBO();
			$qry = "SELECT a.*, u.name FROM #__limeticket_ticket_attach as a LEFT JOIN #__users as u ON a.user_id = u.id WHERE ticket_ticket_id = {$this->id} ORDER BY added DESC";
			$db->setQuery($qry);
	...
	function loadGroups()
	{
		if (!$this->loaded_groups)
		{
			...
			{
				$db = JFactory::getDBO();
				$query = "SELECT g.* FROM #__limeticket_ticket_group_members as m LEFT JOIN #__limeticket_ticket_group as g ON m.group_id = g.id WHERE m.user_id = " . (int)$this->user_id;
				$db->setQuery($query);
	...
	function loadMessages($reverse = null, $types = array())
	{
		...
		{
			$query = "SELECT m.*, u.name, u.username FROM #__limeticket_ticket_messages as m LEFT JOIN #__users as u ON m.user_id = u.id WHERE ticket_ticket_id = {$this->id}";
	...
	{
		$cotime = LIMETICKET_Helper::GetDBTime() - strtotime($this->checked_out_time);
		if ($cotime < LIMETICKET_Settings::get('support_lock_time') && $this->checked_out != JFactory::getUser()->id)
		{
	...
	function loadCustomFields()
	{
		...
		{
			$this->customfields = LIMETICKETCF::GetCustomFields($this->id,$this->prod_id,$this->ticket_dept_id,3);
			$this->custom = LIMETICKETCF::GetTicketValues($this->id, $this);
	...
	{
		$db = JFactory::getDBO();
		$qry = "SELECT * FROM #__limeticket_ticket_related WHERE source_id = " . $this->id;
		$db->setQuery($qry);
	...
	function loadCC()
	{
		...
		{
			$db = JFactory::getDBO();
			$qry = "SELECT u.name, u.username, u.email, u.id, c.isadmin, c.readonly, c.email as uremail, c.user_id as urid FROM #__limeticket_ticket_cc as c LEFT JOIN #__users as u ON c.user_id = u.id WHERE c.ticket_id = '{$this->id}' ORDER BY name";
			$db->setQuery($qry);
...
/***************************************/
	...
	function updateLastUpdated()
	{
		...
		$qry = "UPDATE #__limeticket_ticket_ticket SET lastupdate = '{$now}' WHERE id = {$this->id}";
		$db = JFactory::getDBO();
		$db->setQuery($qry);
		$db->Query();	
	...
	function updatePriority($new_pri_id)
	{
		...
		$qry = "UPDATE #__limeticket_ticket_ticket SET ticket_pri_id = ".(int)$new_pri_id." WHERE id = {$this->id}";
		$db = JFactory::getDBO();
		$db->setQuery($qry);
		$db->Query();	
	...
	function updateStatus($new_status_id, $direct_change = false, $is_batch = false)
	{
		...
		$qry = "UPDATE #__limeticket_ticket_ticket SET ticket_status_id = {$new_status_id}, {$isclosed} WHERE id = {$this->id}";
		$db = JFactory::getDBO();
		$db->setQuery($qry);
		$db->Query();	
	...
	function updateCategory($new_cat_id)
	{
		...
		$qry = "UPDATE #__limeticket_ticket_ticket SET ticket_cat_id = ".(int)$new_cat_id." WHERE id = {$this->id}";
		$db = JFactory::getDBO();
		$db->setQuery($qry);
		$db->Query();	
	...
	function updateUser($new_user_id)
	{
		...
		$qry = "UPDATE #__limeticket_ticket_ticket SET user_id = ".(int)$new_user_id." WHERE id = {$this->id}";
		$db = JFactory::getDBO();
		$db->setQuery($qry);
		$db->Query();	
	...
	function updateProduct($new_product_id)
	{
		...
		$qry = "UPDATE #__limeticket_ticket_ticket SET prod_id = ".(int)$new_product_id." WHERE id = {$this->id}";
		$db = JFactory::getDBO();
		$db->setQuery($qry);
		$db->Query();	
	...
	function updateDepartment($new_department_id)
	{
		...
		$qry = "UPDATE #__limeticket_ticket_ticket SET ticket_dept_id = ".(int)$new_department_id." WHERE id = {$this->id}";
		$db = JFactory::getDBO();
		$db->setQuery($qry);
		$db->Query();	
	...
	function updateUnregEMail($new_email)
	{
		...
		$db = JFactory::getDBO();
		$qry = "UPDATE #__limeticket_ticket_ticket SET email = '".LIMETICKETJ3Helper::getEscaped($db,$new_email)."' WHERE id = {$this->id}";
		$db->setQuery($qry);
		$db->Query();	
	...
	function updateSubject($new_subject)
	{
		...
		$db = JFactory::getDBO();
		$qry = "UPDATE #__limeticket_ticket_ticket SET title = '".LIMETICKETJ3Helper::getEscaped($db,$new_subject)."' WHERE id = {$this->id}";
		$db->setQuery($qry);
		$db->Query();	
	...
	function updateSource($new_source)
	{
		...
		$db = JFactory::getDBO();
		$qry = "UPDATE #__limeticket_ticket_ticket SET source = '".LIMETICKETJ3Helper::getEscaped($db,$new_source)."' WHERE id = {$this->id}";
		$db->setQuery($qry);
		$db->Query();	
	...
	function updateLock()
	{
		$db = JFactory::getDBO();
		$now = LIMETICKET_Helper::CurDate();
		$qry = "UPDATE #__limeticket_ticket_ticket SET checked_out = '".(int)JFactory::getUser()->id."', checked_out_time = '{$now}' where id = {$this->id}";
		$db->setQuery($qry);
		$db->query();		
	...
	function updateCustomField($fieldid, $value, $max_permission = 3)
	{
		...
		if (empty($this->fields))
			$this->fields = LIMETICKETCF::GetCustomFields($this->id,$this->prod_id,$this->ticket_dept_id,$max_permission);
		
		list($old, $new) = LIMETICKETCF::StoreField($fieldid, $this->id, $this, $value);
					
		if ($old != $new)
		{
			$field = LIMETICKETCF::GetField($fieldid);
	...
	{	
		...
		$db = JFactory::getDBO();
		$qry = "REPLACE INTO #__limeticket_ticket_tags (ticket_id, tag) VALUES ({$this->id}, '".LIMETICKETJ3Helper::getEscaped($db, $tag)."')";
		$db->setQuery($qry);
		$db->query();		
	...
	function removeTag($tag)
	{
		...
		$db = JFactory::getDBO();
		$qry = "DELETE FROM #__limeticket_ticket_tags WHERE ticket_id = {$this->id} AND tag = '".LIMETICKETJ3Helper::getEscaped($db, $tag)."'";
		$db->setQuery($qry);
		$db->query();		
	...
	{
		$db = JFactory::getDBO();
		$qry = "UPDATE #__limeticket_ticket_ticket SET timetaken = timetaken + " . (int)$minutes . " WHERE id = {$this->id}";
		$db->setQuery($qry);
		$db->Query();
		
		// extra query to force the time to always be above 0
		$qry = "UPDATE #__limeticket_ticket_ticket SET timetaken = 0 WHERE id = {$this->id} AND timetaken < 0";
		$db->setQuery($qry);
		$db->Query();
	...
	function deleteAttach($file_id)
	{
		...
		{
			$db = JFactory::getDBO();
			$qry = "DELETE FROM #__limeticket_ticket_attach WHERE ticket_ticket_id = {$this->id} AND id = {$attach->id}";
			$db->setQuery($qry);
			$db->Query();
			
			$destpath = JPATH_SITE.DS.LIMETICKET_Helper::getAttachLocation().DS.'support'.DS;			
	...
	function addMessage($body, $subject = "", $user_id = -1, $type = TICKET_MESSAGE_USER, $time = 0, $timestart = 0, $timeend = 0, $source = '')
	{
		...
		$qry = "INSERT INTO #__limeticket_ticket_messages (ticket_ticket_id, subject, body, user_id, admin, posted, `time`, timestart, timeend, source) VALUES ('";
		$qry .= $db->escape($this->id) . "','".$db->escape($subject)."','".$db->escape($body)."',".(int)$user_id.", " . (int)$type . ", '".LIMETICKET_Helper::CurDate()."', '" . $db->escape($time) ."', '" . $db->escape($timestart) ."', '" . $db->escape($timeend) ."', '" . $db->escape($source) ."')";
	...
	function addMessageUnreg($body, $subject, $type, $name, $email, $source = '')
	{
		...
		$qry = "INSERT INTO #__limeticket_ticket_messages (ticket_ticket_id, subject, body, admin, posted, poster, email, source) VALUES ('";
		$qry .= $db->escape($this->id) . "','".$db->escape($subject)."','".$db->escape($body)."', " . (int)$type . ", '".LIMETICKET_Helper::CurDate()."', '" . $db->escape($name) ."', '" . $db->escape($email) ."', '" . $db->escape($source) ."')";
	...
	function addFilesFromPost($message_id, $user_id = -1, $hide_from_user = 0)
	{
		...
		for ($i = 0; $i < 10; $i ++)
		{
			...
			{
				$destpath = JPATH_SITE.DS.LIMETICKET_Helper::getAttachLocation().DS.'support'.DS;					
				$destname = LIMETICKET_File_Helper::makeAttachFilename("support", $file['name'], date("Y-m-d"), $this, $user_id);
		 
				if (JFile::upload($file['tmp_name'], $destpath . $destname))
				{
					$qry = "INSERT INTO #__limeticket_ticket_attach (ticket_ticket_id, filename, diskfile, size, user_id, added, message_id, hidefromuser) VALUES ('";
					$qry .= LIMETICKETJ3Helper::getEscaped($db, $this->id) . "',";
					$qry .= "'" . LIMETICKETJ3Helper::getEscaped($db, $file['name']) . "',";
					$qry .= "'" . LIMETICKETJ3Helper::getEscaped($db, $destname) . "',";
					$qry .= "'" . $file['size'] . "',";
					$qry .= "'" . LIMETICKETJ3Helper::getEscaped($db, $user_id) . "',";
					$qry .= "'".LIMETICKET_Helper::CurDate()."', $message_id, '".LIMETICKETJ3Helper::getEscaped($db, $hide_from_user)."' )";
		...
		$token = LIMETICKET_File_Helper::makeUploadSubdir(JRequest::getVar('upload_token'));
		if (is_array($post_files))
		{
			foreach ($post_files as $file)
			{
				$destpath = JPATH_SITE.DS.LIMETICKET_Helper::getAttachLocation().DS.'support'.DS;					
				$destname = LIMETICKET_File_Helper::makeAttachFilename("support", $file, date("Y-m-d"), $this, $user_id);
				$source = JPATH_ROOT.'/tmp/limeticket/incoming/'.$token.'/'.$file;

				require_once (JPATH_SITE.DS.'components'.DS.'com_limeticket'.DS.'helper'.DS.'bigfiletools.php');
				...
				if (JFile::move($source, $dest))
				{
					$qry = "INSERT INTO #__limeticket_ticket_attach (ticket_ticket_id, filename, diskfile, size, user_id, added, message_id, hidefromuser) VALUES ('";
					$qry .= LIMETICKETJ3Helper::getEscaped($db, $this->id) . "',";
					$qry .= "'" . LIMETICKETJ3Helper::getEscaped($db, $file) . "',";
					$qry .= "'" . LIMETICKETJ3Helper::getEscaped($db, $destname) . "',";
					$qry .= "'" . $size . "',";
					$qry .= "'" . LIMETICKETJ3Helper::getEscaped($db, $user_id) . "',";
					$qry .= "'".LIMETICKET_Helper::CurDate()."', $message_id, '".LIMETICKETJ3Helper::getEscaped($db, $hide_from_user)."' )";
		...
		if (is_dir(JPATH_ROOT.'/tmp/limeticket/incoming/'.$token))
			@rmdir(JPATH_ROOT.'/tmp/limeticket/incoming/'.$token);

		LIMETICKET_File_Helper::CleanupIncoming();
	...
	function updateMessage($message_id, $subject, $body, $time = 0, $timestart = 0, $timeend = 0)
	{
		...
		$qry = "UPDATE #__limeticket_ticket_messages SET subject = '".$db->escape($subject)."', body = '".$db->escape($body)."'";
		...
		$qry .= " WHERE id = " . LIMETICKETJ3Helper::getEscaped($db, $message_id);
		$db->setQuery($qry);
		$db->Query($qry);
	...
	function deleteMessage($message_id, $subject, $body)
	{
		...
		$qry = "DELETE FROM #__limeticket_ticket_messages WHERE id = " . LIMETICKETJ3Helper::getEscaped($db, $message_id);
		$db->setQuery($qry);
		$db->Query($qry);
	...
	function addRelated($ticketid)
	{
		...
		$qry = "REPLACE INTO #__limeticket_ticket_related (source_id, dest_id) VALUES ";
	...
	function removeRelated($ticketid)
	{
		...
		$qry = "DELETE FROM #__limeticket_ticket_related WHERE source_id = " . (int)$this->id . " AND dest_id = " .  (int)$ticketid;
		$db->setQuery($qry);
		$db->Query();
		
		$qry = "DELETE FROM #__limeticket_ticket_related WHERE source_id = " . (int)$ticketid . " AND dest_id = " .  (int)$this->id;
		$db->setQuery($qry);
		$db->Query();
	}
	...
	function addCC($ids, $is_admin, $is_readonly)
	{
		...
		foreach ($ids as $id)
		{
			if ($id > 0)
			{
				$qry = "REPLACE INTO #__limeticket_ticket_cc (ticket_id, user_id, isadmin, readonly) VALUES (" . $db->escape((int)$this->id) . ", ";
	...
	function addEMailCC($email)
	{
		...
		$qry = "REPLACE INTO #__limeticket_ticket_cc (ticket_id, user_id, isadmin, email) VALUES (" . $db->escape((int)$this->id) . ", ";
	...
	function removeCC($ids, $is_admin)
	{
		...
		foreach ($ids as $id)
		{
			$qry = "DELETE FROM #__limeticket_ticket_cc WHERE ticket_id = " . $db->escape($this->id);
...
/************************************/
	...
	{
		$statuss = SupportHelper::getStatuss(false);
		LIMETICKET_Translate_Helper::Tr($statuss);
	...
	static function parseTitle($title, $ticketid)
	{
		...
		if (trim($title) != "")
		{
			...
			$db = JFactory::getDBO();
			$qry = "SELECT * FROM #__limeticket_ticket_messages WHERE ticket_ticket_id = " . $db->escape($ticketid) . " ORDER BY posted ASC LIMIT 1";
			$db->setQuery($qry);
			$message = $db->loadObject();
			$msg = $message->body;
			$msg = LIMETICKET_Helper::ParseBBCode($msg);
			$msg = strip_tags($msg);
			if (trim($msg) != "")
			{
	...
	function isLocked()
	{
		if (empty($this->locked))
		{
			$cotime = LIMETICKET_Helper::GetDBTime(); - strtotime($this->checked_out_time);
			$this->locked = false;
			if ($cotime < LIMETICKET_Settings::get('support_lock_time') && $this->checked_out != JFactory::getUser()->id && $this->checked_out != 0)
			{
				$this->locked = true;
			}
		}
	 ...
	 * 
	 * handler_id = the LIMETICKET User ID of the ticket handler
	 * 
	 * Type:
	 * 
	 * define("TICKET_ASSIGN_FORWARD", 0);
	...
	function assignHandler($handler_id, $type = TICKET_ASSIGN_FORWARD)
	{
		...
		$qry = "UPDATE #__limeticket_ticket_ticket SET admin_id = {$handler_id} WHERE id = {$this->id}";
		$db = JFactory::getDBO();
		$db->setQuery($qry);
		$db->Query();	
	...
	function addAuditNote($note)
	{
		...
		$qry .= LIMETICKETJ3Helper::getEscaped($db, $this->id)."','Audit Message','".LIMETICKETJ3Helper::getEscaped($db, $note)."','".LIMETICKETJ3Helper::getEscaped($db, $this->current_user)."',3, '{$now}')";
	...
	function delete()
	{
		if (LIMETICKET_Settings::get('support_delete'))
		{
			$this->loadAttachments();
			
			foreach ($this->attach as $attach)
			{
				$image_file = JPATH_SITE.DS."components/com_limeticket/files/support/" . $attach->diskfile;
				$thumb_file = JPATH_SITE.DS."components/com_limeticket/files/support/" . $attach->diskfile . ".thumb";
			...
			$qry = "DELETE FROM #__limeticket_ticket_messages WHERE ticket_ticket_id = {$this->id}";
			$db->setQuery($qry);
			$db->query();
			
			$qry = "DELETE FROM #__limeticket_ticket_attach WHERE ticket_ticket_id = {$this->id}";
			$db->setQuery($qry);
			$db->query();
			
			$qry = "DELETE FROM #__limeticket_ticket_cc WHERE ticket_id = {$this->id}";
			$db->setQuery($qry);
			$db->query();
			
			$qry = "DELETE FROM #__limeticket_ticket_field WHERE ticket_id = {$this->id}";
			$db->setQuery($qry);
			$db->query();
			
			$qry = "DELETE FROM #__limeticket_ticket_tags WHERE ticket_id = {$this->id}";
			$db->setQuery($qry);
			$db->query();
			
			$qry = "DELETE FROM #__limeticket_ticket_ticket WHERE id = {$this->id}";
			$db->setQuery($qry);
			$db->query();
		}
	}
	...
	function stripImagesFromMessage($message_id)
	{
		...
		$qry = "SELECT * FROM #__limeticket_ticket_messages WHERE id = " . $db->escape($message_id);
		$db->setQuery($qry);
		$message = $db->loadObject();
		...
		{		
			...
			$destpath = JPATH_SITE.DS.LIMETICKET_Helper::getAttachLocation().DS.'support'.DS;					
			$destname = LIMETICKET_File_Helper::makeAttachFilename("support", $filename, date("Y-m-d", strtotime($message->posted)), $this, $message->user_id);

			if (file_put_contents($destpath.$destname, $image_data))
			{
				$size = filesize($destpath.$destname);

				$qry = "INSERT INTO #__limeticket_ticket_attach (ticket_ticket_id, filename, diskfile, size, user_id, added, message_id, inline) VALUES ('";
				$qry .= LIMETICKETJ3Helper::getEscaped($db, $this->id) . "',";
				$qry .= "'" . LIMETICKETJ3Helper::getEscaped($db, $filename) . "',";
				$qry .= "'" . LIMETICKETJ3Helper::getEscaped($db, $destname) . "',";
				$qry .= "'" . $size . "',";
				$qry .= "'" . LIMETICKETJ3Helper::getEscaped($db, $message->user_id) . "',";
			...
			$key = LIMETICKET_Helper::base64url_encode(LIMETICKET_Helper::encrypt($attach_id, LIMETICKET_Helper::getEncKey("file")));
			$replace = "]" . JURI::base() . "index.php?option=com_limeticket&view=image&fileid={$attach_id}&key={$key}" . "[/img]";
		...
		if ($count > 0)
		{
			$qry = "UPDATE #__limeticket_ticket_messages SET body = \"" . $db->escape($body) . "\" WHERE id = " . $db->escape($message_id);
			$db->setQuery($qry);
			$db->Query();
		}
	}
	...
	function forParser(&$vars, $foruser, $display = true, $template = "")
	{
		...
		if ($display)
		{
			if ($foruser)
			{
				$vars['subject'] = "<a href='".LIMETICKETRoute::_( 'index.php?option=com_limeticket&view=ticket&layout=view&ticketid=' . $this->id )."'>" . $title . "</a>";
			} else {
				$vars['subject'] = "<input type='checkbox' class='ticket_cb' style='display: none' id='ticket_cb_{$this->id}' name='ticket_{$this->id}'><a href='".LIMETICKETRoute::_( 'index.php?option=com_limeticket&view=admin_support&layout=ticket&ticketid=' . $this->id )."'>" . $title . "</a>";				
				$vars['checkbox'] = "<input type='checkbox' class='ticket_cb' style='display: none' id='ticket_cb_{$this->id}' name='ticket_{$this->id}'>";
			}	
		}
		
		$uri = JURI::getInstance();	
		$baseUrl = LIMETICKET_Settings::get('support_email_no_domain') ? "" : $uri->toString( array('scheme', 'host', 'port'));

		// choose which user link to generate
		if ($this->user_id < 1)
		{
			// unregistered user
			$url = 'index.php?option=com_limeticket&t=' . $this->id . "&p=" . $this->password;
			if (LIMETICKET_Settings::get('support_email_link_unreg') > 0) $url .= "&Itemid=" . LIMETICKET_Settings::get('support_email_link_unreg'); // add fixed item id if needed
			$vars['ticket_link'] = $baseUrl . JRoute::_($url, false);
		} else {
			// registered user
			$url = 'index.php?option=com_limeticket&view=ticket&layout=view&ticketid=' . $this->id;

			if ((new JVersion())->RELEASE < "3.7" && LIMETICKET_Settings::get('support_email_include_autologin')) $url .= "&login={login_code}";
			if (LIMETICKET_Settings::get('support_email_link_reg') > 0) $url .= "&Itemid=" . LIMETICKET_Settings::get('support_email_link_reg'); // add fixed item id if needed
			$vars['ticket_link'] = $baseUrl . JRoute::_($url, false);
		}
		
		// ticket admin link
		$url = 'index.php?option=com_limeticket&view=admin_support&layout=ticket&ticketid=' . $this->id;
		if ((new JVersion())->RELEASE < "3.7" && LIMETICKET_Settings::get('support_email_link_admin') > 0) $url .= "&Itemid=" . LIMETICKET_Settings::get('support_email_link_admin'); // add fixed item id if needed
		if (LIMETICKET_Settings::get('support_email_include_autologin_handler')) $url .= "&login={login_code}";
		$vars['admin_link'] = $baseUrl . JRoute::_($url, false);

		// email pending link
		$url = 'index.php?option=com_limeticket&view=admin_support&layout=emails';
		if (LIMETICKET_Settings::get('support_email_link_pending') > 0) $url .= "&Itemid=" . LIMETICKET_Settings::get('support_email_link_pending'); // add fixed item id if needed
		...
		// dates
		$vars["lastactivity"] = LIMETICKET_Helper::TicketTime($this->lastupdate,LIMETICKET_DATETIME_SHORT);
		$vars["opened"] = LIMETICKET_Helper::TicketTime($this->opened,LIMETICKET_DATETIME_SHORT);
		$vars["closed"] = LIMETICKET_Helper::TicketTime($this->closed,LIMETICKET_DATETIME_SHORT);
		...
		// website title
		$vars["websitetitle"] = LIMETICKET_Helper::getSiteName();
		
		// password
		$vars["password"] = $this->password;
		$vars['haspassword'] = in_array(LIMETICKET_Settings::get('support_unreg_type'), array(0,1)) ? 1 : 0; // if the system is using passwords for tickets or not

		if ($this->prod_img) $vars['product_img'] = JURI::root( true ) . "/images/limeticket/products/" . $this->prod_img;
		if ($this->dept_img) $vars['department_img'] = JURI::root( true ) . "/images/limeticket/departments/" . $this->dept_img;
			...
			strpos($template, "last_poster_username") !== false)
		{
			...
			$db = JFactory::getDBO();
			$qry = "SELECT user_id, posted FROM #__limeticket_ticket_messages WHERE ticket_ticket_id = " . $db->escape($this->id) . " AND admin IN (0, 1) ORDER BY posted DESC LIMIT 1";
			$db->setQuery($qry);
			$rows = $db->loadObjectList();
		...
		if (!$foruser && $display)
		{
			...
			if ($cotime < LIMETICKET_Settings::get('support_lock_time') && $this->checked_out != JFactory::getUser()->id && $this->checked_out > 0)
			{
				...
				$vars['lock'] = "<img class='limeticketTip' title='<b>" . JText::_("TICKET_LOCKED") . "</b><br />".htmlentities($html,ENT_QUOTES,"utf-8")."' src='". JURI::root( true ) . "/components/com_limeticket/assets/images/lock.png'>";
			}
		}
		...
		{
			if (!LIMETICKET_Settings::get('support_hide_tags'))
			{
				$this->loadTags();
				if (count($this->tags) > 0)
				{
					if ($display)
					{
						...
						$vars['tags'] = "<img class='limeticketTip' title='".htmlentities($html,ENT_QUOTES,"utf-8")."' src='". JURI::root( true ) . "/components/com_limeticket/assets/images/tag.png'>";
					} else {
		...
		if (!$template || strpos($template, "attach") !== false)
		{
			...
			if ($vars['attach_cnt'] > 0)
			{
				if ($display)
				{
					...
					foreach($this->attach as $attach)
					{
						$html .= "<div>" . $attach->filename ." (" . LIMETICKET_Helper::display_filesize($attach->size) . ")</div>";
					}
						
					$vars['attach'] = "<img class='limeticketTip' title='".htmlentities($html,ENT_QUOTES,"utf-8")."' src='". JURI::root( true ) . "/components/com_limeticket/assets/images/attach.png'>";
				} else {
		...
		// message counts
		if (LIMETICKET_Settings::get('support_show_msg_counts') || $foruser)
		{
			if (!$template || strpos($template, "msgcount") !== false || strpos($template, "msgcnt") !== false)
			{
				...
				if ($display)
				{
					...
					$vars['msgcnt'] = "<span class='limeticketTip label label-default' title='".htmlentities($tip,ENT_QUOTES,"utf-8")."'>".$this->msgcount['total']."</span>";
				}
			}
		}
		...
		if ($display && !$foruser)
		{
			$delete = "<a class='pull-right btn btn-default btn-mini' href='" . LIMETICKETRoute::_( 'index.php?option=com_limeticket&view=admin_support&task=archive.delete&tickets='.LIMETICKET_Input::getCmd('tickets').'&ticketid=' . $this->id) . "'>";
			$delete .= JText::_("DELETE") . "</a>";
			$vars['deletebutton'] = $delete;

			$archive = "<a class='pull-right btn btn-default btn-mini' href='" . LIMETICKETRoute::_( 'index.php?option=com_limeticket&view=admin_support&task=archive.archive&tickets='.LIMETICKET_Input::getCmd('tickets').'&ticketid=' . $this->id) . "'>";
		...
		if ($display)
		{
			$handler_highlight = '<span class="limeticketTip label label-warning pull-right" title="'. JText::_('UNASSIGNED_TICKET') . '">' . $vars['handlername'] . '</span>';
			
			if ($this->admin_id == JFactory::getUser()->id)
			$handler_highlight = '<span class="limeticketTip label label-success pull-right" title="'. JText::_('MY_TICKET') . '">' . $vars['handlername'] . '</span>';
			else if ($this->admin_id > 0)
				$handler_highlight = '<span class="limeticketTip label label-info pull-right" title="' . JText::_('OTHER_HANDLERS_TICKET'). '">' . $vars['handlername'] . '</span>';
		...
		if ($display && !$foruser)
		{
			...
			if (LIMETICKET_Settings::get('support_entire_row'))
			{
				$style .= "cursor: pointer;";
				$trhl .= " onclick='window.location=\"".LIMETICKETRoute::_( 'index.php?option=com_limeticket&view=admin_support&layout=ticket&ticketid=' . $this->id )."\"' ";
			}
		...
		if ($display)
		{
			$vars['hidehandler'] = (LIMETICKET_Settings::get('support_hide_handler') == 1);
			$vars["candelete"] = LIMETICKET_Settings::get('support_delete');
		}
	...
	{
		$old_lang = LIMETICKET_Translate_Helper::setLanguage($lang);
		...
		$another = '0ac73669723cc43646b9a36f90c2073f';
		
		$this->product = LIMETICKET_Translate_Helper::TrF('title', $this->product, $this->prtr);
		$this->proddesc = LIMETICKET_Translate_Helper::TrF('description', $this->proddesc, $this->prtr);
		
		$this->department = LIMETICKET_Translate_Helper::TrF('title', $this->department, $this->dtr);
		$this->deptdesc = LIMETICKET_Translate_Helper::TrF('description', $this->deptdesc, $this->dtr);
		
		$this->category = LIMETICKET_Translate_Helper::TrF('title', $this->category, $this->ctr);
		$this->priority = LIMETICKET_Translate_Helper::TrF('title', $this->priority, $this->pritr);
		
		$this->status = LIMETICKET_Translate_Helper::TrF('title', $this->status, $this->str);
		...
		LIMETICKET_Translate_Helper::setLanguage($old_lang);
	}
	...
	function parseMessageHistory($ishtml, $foruser = false)
	{
		...
		foreach ($marray as &$message)
		{
			...
			$parser->setVar('posted',LIMETICKET_Helper::Date($message->posted));
			
			$body = LIMETICKET_Helper::ParseBBCode($message->body,null,false,false,$foruser);
	...
	{
		$db = JFactory::getDBO();
		$sql = "SELECT ticket_ticket_id FROM #__limeticket_ticket_messages WHERE id = '" . $db->escape($messageid) . "'";
		$db->setQuery($sql);
	...
	{
		$db = JFactory::GetDBO();
		$sql = "UPDATE #__limeticket_ticket_messages SET rating = '" . $db->escape($rating) . "' WHERE id = '" . $db->escape($messageid) . "'";
		$db->setQuery($sql);
		$db->query();
	...
	{
		$db = JFactory::GetDBO();
		$sql = "UPDATE #__limeticket_ticket_ticket SET rating = '" . $db->escape($rating) . "' WHERE id = '" . $db->escape($this->id) . "'";
		$db->setQuery($sql);
		$db->query();
