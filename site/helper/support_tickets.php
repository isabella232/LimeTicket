...
class SupportTickets
{
	...
	function loadTags()
	{
		...
		$db = JFactory::getDBO();
		$qry = "SELECT * FROM #__limeticket_ticket_tags WHERE ticket_id IN (" . implode(", ",$this->ids) . ")";
		$db->setQuery($qry);
	...
	function loadAttachments()
	{
		...
		$db = JFactory::getDBO();
		$qry = "SELECT * FROM #__limeticket_ticket_attach WHERE ticket_ticket_id IN (" . implode(", ",$this->ids) . ")";
		$db->setQuery($qry);
		$items = $db->loadObjectList();
		foreach($items as $item)
	...
	function loadGroups()
	{
		...
		$qry = "SELECT m.user_id, g.* FROM #__limeticket_ticket_group_members as m LEFT JOIN #__limeticket_ticket_group as g ON m.group_id = g.id WHERE m.user_id IN (" . implode(",",$user_ids) . ")";
		$db->setQuery($qry);
		$items = $db->loadObjectList();
	...
	function loadMessageCounts()
	{
		...
		*/
		...
		$db = JFactory::getDBO();
		$qry = "SELECT ticket_ticket_id, admin, count(*) as msgcnt FROM #__limeticket_ticket_messages WHERE ticket_ticket_id IN (" . implode(", ",$this->ids) . ") GROUP BY ticket_ticket_id, admin";
		$db->setQuery($qry);
		$items = $db->loadObjectList();
	...
	function loadMessages($reverse = null, $types = array())
	{
		...
		$query = "SELECT m.*, u.name FROM #__limeticket_ticket_messages as m LEFT JOIN #__users as u ON m.user_id = u.id WHERE ticket_ticket_id IN (" . implode(", ",$this->ids) . ") ";
	...
	function loadLockedUsers()
	{
		...
		foreach($this->tickets as $ticket)
		{
			$cotime = LIMETICKET_Helper::GetDBTime() - strtotime($ticket->checked_out_time);
			if ($cotime < LIMETICKET_Settings::get('support_lock_time') && $ticket->checked_out != $user_id)
			{
	...
	function loadCustomFields()
	{
		...
		$qry = "SELECT * FROM #__limeticket_ticket_field WHERE ticket_id IN (" . implode(", ",$this->ids) . ")";
		$db->setQuery($qry);
		...
		if (count($user_ids) > 0)
		{
			$qry = "SELECT * FROM #__limeticket_ticket_user_field WHERE user_id IN (" . implode(", ", $user_ids) . ")";
			$db->setQuery($qry);
			$rows = $db->loadObjectList();
	...
	function loadTicketsByStatus($tickets, $for_user = false)
	{
		...
		foreach (LIMETICKETCF::GetAllCustomFields() as $field)
		{
			...
			if ($field['peruser'])
			{
				$query .= " LEFT JOIN #__limeticket_ticket_user_field as cf{$id} ON cf{$id}.user_id = t.user_id AND cf{$id}.field_id = {$id} \n";
			} else {
				$query .= " LEFT JOIN #__limeticket_ticket_field as cf{$id} ON cf{$id}.ticket_id = t.id AND cf{$id}.field_id = {$id} \n";
			}
		}
		...
		if ($tickets == "open")
		{
			$open = LIMETICKET_Ticket_Helper::GetStatusIDs("def_open");
		...
		{
			$allopen = LIMETICKET_Ticket_Helper::GetStatusIDs("is_closed");
		...
		elseif ($tickets == 'all')
		{
			...
			$allopen = LIMETICKET_Ticket_Helper::GetStatusIDs("def_archive", true);
			$query .= " WHERE ticket_status_id IN ( " . implode(", ", $allopen) . " ) \n";
		}
		elseif ($tickets == 'archived')
		{
			// need all tickets that arent archived
			$allopen = LIMETICKET_Ticket_Helper::GetStatusIDs("def_archive");
			$query .= " WHERE ticket_status_id IN ( " . implode(", ", $allopen) . " ) \n";
		}
		else
		{
			$query .= " WHERE ticket_status_id = " . (int)LIMETICKETJ3Helper::getEscaped($db, $tickets) . "\n";
		}
		...
		$ordering = JFactory::getApplication()->getUserStateFromRequest("limeticket_admin.ordering","ordering","");
	...
	private function getTagFilter()
	{
		...
		if ($tags)
		{
			...
			if (count($tags) > 0)
			{
				foreach($tags as $tag)
				{
					$qry = "SELECT ticket_id FROM #__limeticket_ticket_tags WHERE tag = '".LIMETICKETJ3Helper::getEscaped($db, $tag)."'";
					$db->setQuery($qry);
	...
	function loadTicketsBySearch($for_user = false)
	{
		...
		$searchtype = LIMETICKET_Input::getCmd('searchtype','basic');
		$wherebits = array();
		...
		if ($searchtype == "basic")
		{
			...
			$search_x = LIMETICKET_Helper::strForLike($search);

			if ($search != "")
			{
				$mode = "";
				if (LIMETICKET_Helper::contains($search, array('*', '+', '-', '<', '>', '(', ')', '~', '"')))
					$mode = "IN BOOLEAN MODE";

				//$wherebits[] = " t.title LIKE '%".LIMETICKETJ3Helper::getEscaped($db, $search)."%' ";
				$wherebits[] = " MATCH (t.title) AGAINST ('" . $db->escape($search) . "' $mode) /* Title */ ";
				$wherebits[] = " t.reference LIKE '".$search_x."' /* Reference */ ";
			
				if (LIMETICKET_Settings::get('search_extra_like') || strlen($search) < 4)
				{
				...
				if (LIMETICKET_Settings::get('support_basic_name'))
				{
					$wherebits[] = " u.name LIKE '".$search_x."' /* Name */ ";
					$wherebits[] = " unregname LIKE '".$search_x."' /* UnReg Name */ ";
				}

				if (LIMETICKET_Settings::get('support_basic_username'))
				{
					$wherebits[] = " u.username LIKE '".$search_x."' /* Username */ ";
				}

				if (LIMETICKET_Settings::get('support_basic_email'))
				{
					$wherebits[] = " u.email LIKE '".$search_x."' /* User email */ ";
					$wherebits[] = " t.email LIKE '".$search_x."' /* Unreg Email */ ";
				}

				if (LIMETICKET_Settings::get('support_basic_messages'))
				{
					$qry = "SELECT ticket_ticket_id FROM #__limeticket_ticket_messages WHERE subject LIKE '%" . LIMETICKETJ3Helper::getEscaped($db, $search) . "%' OR ";
					$qry .= " MATCH (body) AGAINST ('" . $db->escape($search) . "' $mode) ";
					if (LIMETICKET_Settings::get('search_extra_like')) $qry .= " OR body LIKE '" . $db->escape($search_x) . "'";
		...
		} else if ($searchtype == "advanced")
		{
			...
			if ($subject) $wherebits[] = " t.title LIKE '".LIMETICKET_Helper::strForLike($subject)."' /* Title */ ";	
			
			$reference = LIMETICKET_Input::getString('reference','');
			if ($reference) $wherebits[] = " t.reference LIKE '".LIMETICKET_Helper::strForLike($reference)."' /* Reference */ ";
			
			$username = LIMETICKET_Input::getString('username','');
			if ($username) $wherebits[] = " u.username LIKE '".LIMETICKET_Helper::strForLike($username)."' /* Username */ ";
			
			$useremail = LIMETICKET_Input::getString('useremail','');
			if ($useremail)
				$wherebits[] = " ( u.email LIKE '".LIMETICKET_Helper::strForLike($useremail)."' OR t.email LIKE '".LIMETICKET_Helper::strForLike($useremail)."' ) /* EMail */";
			
			$userfullname = LIMETICKET_Input::getString('userfullname','');
			if ($userfullname)
				$wherebits[] = " ( u.name LIKE '".LIMETICKET_Helper::strForLike($userfullname)."' OR unregname LIKE '".LIMETICKET_Helper::strForLike($userfullname)."' ) /* Name */";
			
			$content = LIMETICKET_Input::getString('content','');
			if ($content)
			{
				$mode = "";
				if (LIMETICKET_Helper::contains($search, array('*', '+', '-', '<', '>', '(', ')', '~', '"')))
				...
				$qry = "SELECT ticket_ticket_id FROM #__limeticket_ticket_messages WHERE ( subject LIKE '" . LIMETICKET_Helper::strForLike($content) . "' OR ";
				$qry .= " MATCH (body) AGAINST ('" . $db->escape($content) . "' $mode)";
				if (LIMETICKET_Settings::get('search_extra_like')) $qry .= " OR body LIKE '" . LIMETICKET_Helper::strForLike($search) . "'";
				$qry .= ") AND admin IN (0, 1, 2, 4) ";
				$db->setQuery($qry);	
				$wherebits[] = $this->TicketIDsToWhere($db->loadObjectList(), "ticket_ticket_id") . " /* Messages */ ";			
			}
			
			$handlers = LIMETICKET_Input::getIntArray('handler');
			...
			if ($handlers)
			{
				...
				foreach ($handlers as $handler)
				{
					...
					}  elseif ($handler == -4) // mine and cc'd
					{
						...
						$qry = "SELECT ticket_id FROM #__limeticket_ticket_cc WHERE isadmin = 1 AND user_id = " . $db->escape($user->id);
						...
						$qry = "SELECT * FROM #__limeticket_ticket_cc WHERE isadmin = 1 AND user_id = " . $db->escape($handler);
						$db->setQuery($qry);
						...
						//$wherebits[] = " t.admin_id = '".LIMETICKETJ3Helper::getEscaped($db, $handler)."' ";
					}
				}
			...
			$status = LIMETICKET_Input::getCmdArray('status','');
			if ($status)
			{
				...
				if (in_array("open", $status)) $ids = array_merge($ids, LIMETICKET_Ticket_Helper::GetStatusIDs("def_open"));
				if (in_array("allopen", $status)) $ids = array_merge($ids, LIMETICKET_Ticket_Helper::GetStatusIDs("is_closed", true));
				if (in_array("all", $status)) $ids = array_merge($ids, LIMETICKET_Ticket_Helper::GetStatusIDs("def_archive", true));
				if (in_array("archived", $status)) $ids = array_merge($ids, LIMETICKET_Ticket_Helper::GetStatusIDs("def_archive"));
			...
			$products = LIMETICKET_Input::getIntArray('product');
			if ($products) $wherebits[] = " t.prod_id IN (".implode(", ", $products).") /* Product */";
			
			$departments = LIMETICKET_Input::getIntArray('department');
			if ($departments) $wherebits[] = " t.ticket_dept_id IN (".implode(", ", $departments).") /* Department */ ";
			
			$cat = LIMETICKET_Input::getIntArray('cat','');
			if ($cat) $wherebits[] = " t.ticket_cat_id IN (".implode(", ", $cat).") /* Category */";
			 
			$pri = LIMETICKET_Input::getIntArray('priority','');
			if ($pri) $wherebits[] = " t.ticket_pri_id IN (".implode(", ", $pri).") /* Priority */";
				
			$group = LIMETICKET_Input::getIntArray('group','');
			if ($group) $wherebits[] = " t.user_id IN (SELECT user_id FROM #__limeticket_ticket_group_members WHERE group_id IN (".implode(", ", $group).") GROUP BY user_id) /* Ticket Group */";

		
			$date_from = LIMETICKET_Helper::DateValidate(LIMETICKET_Input::getString('date_from',''));
			$date_to = LIMETICKET_Helper::DateValidate(LIMETICKET_Input::getString('date_to',''));

			if ($date_from)
			$wherebits[] = " t.lastupdate > DATE_SUB('".LIMETICKETJ3Helper::getEscaped($db, $date_from)."',INTERVAL 1 DAY) /* Date From */";
			
			if ($date_to)
			$wherebits[] = " t.opened < DATE_ADD('".LIMETICKETJ3Helper::getEscaped($db, $date_to)."',INTERVAL 1 DAY) /* Date To */";
	...
	private function searchFields(&$wherebits, $type)
	{
		...
		{			
			...
			$search = LIMETICKET_Input::getString('custom_' . $fieldid,"");
			...
			if ($field['peruser'])
			{
				...
				$qry = "SELECT user_id FROM #__limeticket_ticket_user_field WHERE field_id = '" . LIMETICKETJ3Helper::getEscaped($db, $fieldid) . "' AND value LIKE '%" . LIMETICKETJ3Helper::getEscaped($db, $search) . "%'";	
				$db->setQuery($qry);
	...
	static function &getTicketCount($foradmin = true, $current_handler_only = false)
	{
		...
		{
			$db = JFactory::getDBO();
			$query = "SELECT count( * ) AS count, ticket_status_id FROM #__limeticket_ticket_ticket as t WHERE 1 ";
			...
			$all = LIMETICKET_Ticket_Helper::GetStatusIDs("def_archive", true);
			$out['all'] = 0;
			foreach ($all as $id)
			{
				if (array_key_exists($id, $out))
