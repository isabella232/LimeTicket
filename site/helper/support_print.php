...
class Support_Print
{
	static function getPrintList($for_admin, $ticket = null, $isbatch = false)
	{
		...
		foreach ($files as $file)
		{
			...
			if (!LIMETICKET_Helper::IsPluignEnabled("ticketprint", $id))
				continue;
	...
	static function loadPrint($name)
	{
		...
		$path = JPATH_SITE.DS.'components'.DS.'com_limeticket'.DS.'plugins'.DS.'ticketprint'.DS.$name.".xml";
	...
	static function outputPrint($name, $print)
	{
		...
		$path = JPATH_SITE.DS.'components'.DS.'com_limeticket'.DS.'plugins'.DS.'ticketprint'.DS.$name.DS.$file;
