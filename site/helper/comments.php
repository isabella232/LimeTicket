...
require_once (JPATH_SITE.DS.'components'.DS.'com_limeticket'.DS.'helper'.DS.'comments'.DS.'_handler.php');
...
class LIMETICKET_Comments
{
	var $table = "#__limeticket_comments";
	var $customfields;
	var $tmplpath;
	var $_data;
	var $_counts;
	...
	function __construct($identifier, $itemid = -1, &$itemlist = null)
	{
		...
		if (LIMETICKET_Input::getInt('uid') > 0)
			$this->uid = LIMETICKET_Input::getInt('uid');
		/*$this->use_comments = LIMETICKET_Settings::get('announce_comments_allow');
		if (!$this->use_comments)
			return;*/

		$this->identifier = $identifier;
		
		$this->use_email = LIMETICKET_Settings::get('commnents_use_email');//FSJ_Settings::GetComponentSetting( fsj_get_com(), 'comments_email', 1 );
		$this->use_website = LIMETICKET_Settings::get('commnents_use_website'); //FSJ_Settings::GetComponentSetting( fsj_get_com(), 'comments_website', 1 );
		
		$this->tmplpath = JPATH_SITE.DS.'components'.DS.'com_limeticket'.DS.'tmpl'.DS.'comments';	
		$this->dest_email = LIMETICKET_Settings::get( 'email_on_comment' );
		...
		$this->comments_hide_add = LIMETICKET_Settings::get('comments_hide_add');
		
		if (LIMETICKET_Settings::get('comments_who_can_add') == "registered")
		{
		...
		// set up moderation
		$commod = LIMETICKET_Settings::get('comments_moderate');
		...
		if (LIMETICKET_Settings::get('comments_general_use_custom'))
		...
		if ($this->handler)
		{
			...
			$this->template_type = 3;
			if (LIMETICKET_Settings::get('comments_' . $this->handler->GetName() . '_use_custom'))
				$this->template_type = 2;
		}
	...
	function IncludeTemplates()
	{
		...
		{
			require_once (JPATH_SITE.DS.'components'.DS.'com_limeticket'.DS.'helper'.DS.'comments'.DS. $this->identifier.'.php');
			$classname = "LIMETICKET_Comments_Handler_" . $this->identifier;
			...
			$this->customfields = LIMETICKETCF::Comm_GetCustomFields($this->ident);
			//exit;
		} else {
			$dir = JPATH_SITE.DS.'components'.DS.'com_limeticket'.DS.'helper'.DS.'comments';
			$dh = opendir($dir);
			while (($file = readdir($dh)) !== false) {
				...
				$classname = "LIMETICKET_Comments_Handler_" . str_ireplace(".php","",$file);
			...
			$this->customfields = LIMETICKETCF::Comm_GetCustomFields(-1);
			//print_p($this);
			//exit;
		}
	}
	...
	function GetComment($commentid)
	{
		...
		$where[] = "id = ".LIMETICKETJ3Helper::getEscaped($db, $commentid);
	...
	function GetComments($since = 0, $count = -1, $offset = 0)
	{
		...
		if ($this->identifier == "test")
			$qry .= " LEFT JOIN #__limeticket_prod AS p ON c.itemid = p.id ";
		...
		if (is_array($this->itemid))
		{
			if (count($this->itemid) > 0)
			{
				...
				foreach ($this->itemid as $item)
					$items[] = LIMETICKETJ3Helper::getEscaped($db, $item);
			...
			if ($this->itemid > -1)
				$where[] = "c.itemid='".LIMETICKETJ3Helper::getEscaped($db, $this->itemid)."'";
			if ($this->itemid == -2)
				$where[] = "c.itemid=0";
		}
	...
	function GetCommentCounts()
	{
		...
		if (empty($this->_counts))
		{
			...
			foreach ($this->itemlist as $item)
			{
				if (is_array($item))
				{
					$ids[] = LIMETICKETJ3Helper::getEscaped($db, $item['id']);	
				} else {
					$ids[] = LIMETICKETJ3Helper::getEscaped($db, $item->id);	
				}
			}	
	...
	function Process()
	{
		...
		$task = LIMETICKET_Input::getCmd('task');
		if ($task == "commentpost")
			return $this->DoPost();
	...
	{
		$commentid = LIMETICKET_Input::getInt('commentid',0);   
	...
	function DoEditComment()
	{
		...
		$this->customfields = LIMETICKETCF::Comm_GetCustomFields($ident);
	...
	function DoDeleteComment()
	{
		...
		$db = JFactory::getDBO();
		$qry = "DELETE FROM #__limeticket_comments WHERE id = '".LIMETICKETJ3Helper::getEscaped($db, $commentid)."'";
		$db->SetQuery($qry);
		$db->Query();
	...
	function DoPost()
	{	
		$allfields = LIMETICKETCF::GetAllCustomFields(false, -1);

		$this->post['name'] = LIMETICKET_Input::getString('name','');   
		$this->post['email'] = LIMETICKET_Input::getEMail('email','');   
		$this->post['website'] = LIMETICKET_Input::getURL('website','');   
		$this->post['body'] = LIMETICKET_Input::getString('body','');   
		$this->post['itemid'] = LIMETICKET_Input::getInt('itemid',0);   
		$this->post['ident'] = LIMETICKET_Input::getInt('ident',0);   
		...
		if (!LIMETICKETCF::ValidateFields($this->customfields, $this->errors))
		{
			$this->valid = 0;	
		}
		...
		{			
			...
			$custom = LIMETICKETCF::Comm_StoreFields($this->customfields);
			...
			$qry .= " , '" . LIMETICKETJ3Helper::getEscaped($db, $this->post['name']);
			$qry .= "' , '" . LIMETICKETJ3Helper::getEscaped($db, $this->post['email']);
			$qry .= "' , '" . LIMETICKETJ3Helper::getEscaped($db, $this->post['website']);
			$qry .= "' , '" . LIMETICKETJ3Helper::getEscaped($db, $this->post['body']);
			$qry .= "' , $published, '{$now}', '".LIMETICKETJ3Helper::getEscaped($db, $userid)."', '".LIMETICKETJ3Helper::getEscaped($db, $custom)."' )";
			
			$db->SetQuery($qry);
			$db->Query();

			// load comment using main method to make sure it displays the same
			$sql = "SELECT * FROM #__limeticket_comments WHERE id = " . $db->insertid();
			$db->setQuery($sql);
			$this->_data = $db->loadAssocList();
			$this->PopulateCustomFields();
			...
			{
				$value = LIMETICKET_Input::getString("custom_$id","");
				...
				$this->comment['custom_'.$id] = LIMETICKETCF::FieldOutput($allfields[$id],$values, array());	
				//$this->comment["custom_$id"] = LIMETICKET_Input::getString("custom_$id","");
			}

			print_p($this->comment);*/
			
			LIMETICKET_EMail::Send_Comment($this);
	...
	{	
		$this->post['name'] = LIMETICKET_Input::getString('name','');   
		$this->post['commentid'] = LIMETICKET_Input::getInt('commentid',0);   
		$this->post['email'] = LIMETICKET_Input::getEMail('email','');   
		$this->post['website'] = LIMETICKET_Input::getURL('website','');   
		$this->post['body'] = LIMETICKET_Input::getString('body','');   
		...
		$qry = "UPDATE {$this->table} SET name = '".LIMETICKETJ3Helper::getEscaped($db, $this->post['name'])."', email = '".LIMETICKETJ3Helper::getEscaped($db, $this->post['email'])."', ";
		$qry .= "website = '".LIMETICKETJ3Helper::getEscaped($db, $this->post['website'])."', body = '".LIMETICKETJ3Helper::getEscaped($db, $this->post['body'])."', ";
		$qry .= "custom = '".LIMETICKETJ3Helper::getEscaped($db, $custom)."' WHERE id = ".LIMETICKETJ3Helper::getEscaped($db, $this->post['commentid']);
	...
	function IncludeJS()
	{
		global $limeticket_comments_js;
		if (empty($limeticket_comments_js))
		{
			$document = JFactory::getDocument();
			$document->addScript( JURI::root().'components/com_limeticket/assets/js/comments.js' );
			
			$limeticket_comments_js = 1;
			
			echo "<div id='comments_urls' ";
			echo "url='" . htmlentities(LIMETICKETRoute::_('index.php?option=com_limeticket&option=com_limeticket&view=' . JRequest::getVar('view') . '&task=XXTASKXX&commentid=XXCIDXX&uid=XXUIDXX',false),ENT_QUOTES,"utf-8") . "' ";
			echo "refresh='" . htmlentities(LIMETICKETRoute::_('index.php?option=com_limeticket&option=com_limeticket&view=' . JRequest::getVar('view') . '&task=modinner&ident=XXXIDXX&published=XXPXX',false),ENT_QUOTES,"utf-8") . "' ";
			echo "wait='" . htmlentities(JText::_('PLEASE_WAIT'),ENT_QUOTES,"utf-8") . "' ";
			echo "ident='" . LIMETICKET_Input::getInt("ident",0) . "' ";
			echo "published='" . LIMETICKET_Input::getInt("published",0) . "' ";
	...
	function GetModLink()
	{
		return JRoute::_('index.php?option=com_limeticket&view=admin&layout=moderate');	
	}
	...
	function GetModerateComments()
	{
		...
		$this->ident = LIMETICKET_Input::getInt('ident',0);
		$this->published = LIMETICKET_Input::getInt('published',0);
		
		if ($this->ident > 0)
			$where[] = "ident='".LIMETICKETJ3Helper::getEscaped($db, $this->ident)."'";	
		
		$where[] = "published='".LIMETICKETJ3Helper::getEscaped($db, $this->published)."'";
		$qry .= implode(" AND ",$where);
	...
	{
		$allfields = LIMETICKETCF::GetAllCustomFields(false, $this->ident);
		...
		foreach($this->_data as &$item)
		{
			...
			foreach ($values as $fieldvalue)
			{
				...
				$item['custom_'.$id] = LIMETICKETCF::FieldOutput($allfields[$fid],$values, array('userid' => $item['userid'], 'inlist' => 1));
			}
		}	
	}
	...
	function DisplayModerate()
	{
		...
		$db	= JFactory::getDBO();
		$db->SetQuery("SELECT ident FROM #__limeticket_comments WHERE published = 0 GROUP BY ident");
		...
		$this->identselect = JHTML::_('select.genericlist',  $idents, 'ident', ' class="inputbox" size="1" onchange="limeticket_moderate_refresh()"', 'id', 'title', $this->ident);
		...
		$this->whatcomm = JHTML::_('select.genericlist',  $whatcomm, 'published', ' class="inputbox" size="1" onchange="limeticket_moderate_refresh()"', 'id', 'title', $this->published);
