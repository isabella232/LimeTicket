...
require_once (JPATH_SITE.DS.'components'.DS.'com_limeticket'.DS.'helper'.DS.'input.php');
require_once (JPATH_SITE.DS.'components'.DS.'com_limeticket'.DS.'helper'.DS.'guiplugins.php');
require_once (JPATH_SITE.DS.'components'.DS.'com_limeticket'.DS.'helper'.DS.'route.php');
require_once (JPATH_SITE.DS.'components'.DS.'com_limeticket'.DS.'helper'.DS.'view.php');
...
define('LIMETICKET_DATE_SHORT',0);
define('LIMETICKET_DATE_MID',1);
define('LIMETICKET_DATE_LONG',2);

define('LIMETICKET_TIME_SHORT',3);
define('LIMETICKET_TIME_LONG',4);

define('LIMETICKET_DATETIME_SHORT',5);
define('LIMETICKET_DATETIME_MID',6);
define('LIMETICKET_DATETIME_LONG',7);

define('LIMETICKET_DATETIME_MYSQL',8);

define('LIMETICKET_DATE_CUSTOM',9);

$LIMETICKETRoute_menus = array();
global $LIMETICKETRoute_menus;

require_once(JPATH_SITE.DS.'components'.DS.'com_limeticket'.DS.'helper'.DS.'cssparse.php');

class LIMETICKET_Helper
{
	...
	static function StylesAndJS($params = array(), $css = array(), $js = array())
	{
		...
		if (!self::$styles_incl)
		{
			...
			//$document->addStyleSheet(LIMETICKETRoute::_( "index.php?option=com_limeticket&view=css&layout=default&old=1" )); // Add old stylesheets to page

			LIMETICKET_CSSParse::OutputCSS('components/com_limeticket/assets/css/limeticket.less');
			
			if (!LIMETICKETJ3Helper::IsJ3())
				LIMETICKET_CSSParse::OutputCSS('components/com_limeticket/assets/css/limeticket_j25.less');
				
			if (LIMETICKET_Settings::get('hide_warnings'))
				$document->addScriptDeclaration('var limeticket_no_warn = true;');
				
			$document->addScript( JURI::root(true).'/components/com_limeticket/assets/js/main.js' );	
			
			if (LIMETICKET_Settings::get('bootstrap_v3') && !JFactory::getApplication()->isAdmin())
			{
				LIMETICKET_CSSParse::OutputCSS('components/com_limeticket/assets/css/bs3_fixes.less');
				$document->addScript( JURI::root(true).'/components/com_limeticket/assets/js/bs3_fixes.js' );	
			}
		...
		foreach ($params as $param)
		{
			...
			{
				case 'autoscroll':
					$document->addScript( JURI::root(true).'/components/com_limeticket/assets/js/jquery/jquery.autoscroll.js' );
					break;
				case 'tooltip':
					$document->addScript(JURI::root(true).'/components/com_limeticket/assets/js/limeticket_tooltip.js'); 
					break;
				case 'glossary':
					$document->addScript(JURI::root(true).'/components/com_limeticket/assets/js/glossary.js'); 
					break;
				case 'translate':
					$document->addScript( JURI::root(true).'/administrator/components/com_limeticket/assets/js/translate.js' );
					$document->addScript( JURI::root(true).'/administrator/components/com_limeticket/assets/js/popup.js' );
					$document->addStyleSheet(JURI::root(true).'/administrator/components/com_limeticket/assets/css/popup.css'); 
					break;
				case 'csstest':
					$document->addScript( JURI::root(true).'/administrator/components/com_limeticket/assets/js/csstest.js' );
					break;
				case 'admin_css':
					$document->addStyleSheet(JURI::root(true).'/administrator/components/com_limeticket/assets/css/main.css'); 
					break;
				case 'calendar':
					$document->addStyleSheet(JURI::root(true).'/components/com_limeticket/assets/css/calendar.css'); 
					//$document->addStyleSheet(JURI::root(true).'/components/com_limeticket/assets/css/calendar_omega.css'); 
					$document->addScript(JURI::root(true).'/components/com_limeticket/assets/js/calendar.js'); 
					break;
				case 'base64':
					$document->addScript(JURI::root(true).'/components/com_limeticket/assets/js/base64.js'); 	
					break;
				case 'ticket_list':
					LIMETICKET_Translate_Helper::CalenderLocale();
					$document->addScript(JURI::root(true).'/components/com_limeticket/assets/js/ticket_list.js'); 	
					break;
				case 'cookie':
					$document->addScript(JURI::root(true).'/components/com_limeticket/assets/js/jquery/jquery.cookie.js'); 
					break;
				case 'scrollsneak':
					$document->addScript(JURI::root(true).'/components/com_limeticket/assets/js/scrollsneak.js'); 
					break;
			}
		...
		if (!JFactory::getApplication()->isAdmin() && LIMETICKET_Settings::get('artisteer_fixes'))
		{
			$document->addScript(JURI::root(true).'/components/com_limeticket/assets/js/artisteer.js'); 
			LIMETICKET_CSSParse::OutputCSS('components/com_limeticket/assets/css/artisteer.less');
		}
	...
	{	
		...
		$option = LIMETICKET_Settings::get('bootstrap_css');
		...
		if (LIMETICKETJ3Helper::IsJ3())
		{
			...
			if (LIMETICKET_Settings::get('bootstrap_js') == "yes" || JFactory::getApplication()->isAdmin())
			{
				$document = JFactory::getDocument();
				$document->addScript(JURI::root(true)."/components/com_limeticket/assets/js/bootstrap/bootstrap.js"); 
				
				//$option = "limeticketonly";
			}
		}

		if ($option == "limeticketonly")
		{
			LIMETICKET_CSSParse::OutputCSS('components/com_limeticket/assets/css/bootstrap/bootstrap_limeticketonly.less');
		} else if ($option == "limeticketonlyv3")
		{
			LIMETICKET_CSSParse::OutputCSS('components/com_limeticket/assets/css/bootstrap3/bootstrap3_limeticketonly.less', true);
		} else if ($option == "partial")
		{		
			LIMETICKET_CSSParse::OutputCSS('components/com_limeticket/assets/css/bootstrap/bootstrap_missing.parsed.less');
		} else if ($option == "yes")
		{
			LIMETICKET_CSSParse::OutputCSS('components/com_limeticket/assets/css/bootstrap/bootstrap.less');
		}
	}
	...
	static function BootstrapAdminForce()
	{
		$document = JFactory::getDocument();
		$document->addScript(JURI::root(true)."/components/com_limeticket/assets/js/bootstrap/bootstrap.js"); 
		LIMETICKET_CSSParse::OutputCSS('components/com_limeticket/assets/css/bootstrap/bootstrap.j25-admin.less');
	}
	
	static $jquery_incl = false;
	static function IncludeJQuery($force = false)
	{		
		if (LIMETICKET_Helper::$jquery_incl)
			return;
		
		LIMETICKET_Helper::$jquery_incl = true;
		...
		$include = LIMETICKET_Settings::get('jquery_include');
		if ($include == "")
			$include = "auto";
		$url = JURI::root(true).'/components/com_limeticket/assets/js/jquery/jquery-1.10.2.min.js';
		$cpurl = JURI::root(true).'/components/com_limeticket/assets/js/jquery/jquery-migrate-1.2.1.min.js';
		$ncurl = JURI::root(true).'/components/com_limeticket/assets/js/jquery/jquery.noconflict.js';
	...
	static function GetRouteMenus()
	{
		...
		if (empty($LIMETICKETRoute_menus))
		{
			$LIMETICKETRoute_menus = array();
			$db = JFactory::getDBO();
			$qry = "SELECT id, link, access FROM #__menu WHERE link LIKE '%option=com_limeticket%' AND published = 1 AND type = 'component' ";			
			$qry .= ' AND language in (' . $db->Quote(JFactory::getLanguage()->getTag()) . ',' . $db->Quote('*') . ')';
			
			$db->setQuery($qry);
			$menus = $db->loadObjectList('id');

			$LIMETICKETRoute_access = array();
			
			foreach($menus as $menu)
			{
				$LIMETICKETRoute_access[$menu->id] = $menu->access;
				$LIMETICKETRoute_menus[$menu->id] = LIMETICKETRoute::SplitURL($menu->link);
			}
		}
	}
	...
	{
		echo "<style>\n";
		echo LIMETICKET_Settings::get('display_style');
		echo "</style>\n";
		echo LIMETICKET_Settings::get('display_head');
		$class = '';
		if (LIMETICKETJ3Helper::IsJ3())
			$class = "limeticket_main_j3";
		
		$view = LIMETICKET_Input::getCmd('view');
		if ($view)
			$class .= " limeticket_view_" . $view;
		
		$layout = LIMETICKET_Input::getCmd('layout');
		if ($layout)
			$class .= " limeticket_layout_" . $layout;
		
		$itemid = LIMETICKET_Input::getInt('Itemid');
		if ($itemid)
			$class .= " limeticket_itemid_" . $itemid;
		
		$output = "<div class='limeticket_main limeticket_comp $class'>\n";
		
		$msgList = LIMETICKET_Helper::getMessageQueue();
		if (count($msgList) > 0)
			include(JPATH_SITE.DS.'components'.DS.'com_limeticket'.DS.'tmpl'.DS.'messages.php');

		if (LIMETICKET_Settings::get('page_headingout'))
		{
	...
	{
		echo "</div>\n";
		echo LIMETICKET_Settings::get('display_foot');
		
		echo "<div id='limeticket_base_url' style='display: none;'>" . JURI::root(true) . "</div>";
	}
...
?>
<style>
<?php echo LIMETICKET_Settings::get('display_popup_style'); ?>
</style>
<?php
		if ((int)$iframe == 2) {
			// popup window
...
jQuery(document).ready(function () {
	...
	jQuery('body').addClass('limeticket_main');
	jQuery('body').addClass('limeticket_popup');
	...
	limeticket_resize_popup();
	
	setInterval("limeticket_resize_popup()", 250);
});

var limeticket_resize_height = 0;
function limeticket_resize_popup()
{
	...
	var diff = Math.abs(limeticket_resize_height - sheight);

	if (diff > 10)
	{
		limeticket_resize_height = sheight;
		var offset = 200;
		if (sheight + offset > window_height)
		{
			sheight = window_height - offset;	
		} else {
			parent.jQuery('#limeticket_modal iframe').attr("scrolling", "no");
		}
		
		if (sheight < 50)
			sheight = 200;
		
		parent.jQuery('#limeticket_modal iframe').removeAttr('style');
		parent.jQuery('#limeticket_modal iframe').css('height', sheight + 'px');	
	}
}
...
<?php		
		...
		$popup_js = LIMETICKET_Settings::get("popup_js");
		$popup_css = LIMETICKET_Settings::get("popup_css");
	...
	{
		$html[] = '</div>';
		$html[] = '<div class="modal-footer limeticket_main limeticket_popup">';
	...
	{		
		...
		$html[] = '<div class="modal-header limeticket_main limeticket_popup">';
		...
		$html[] = '</h3>';
		$html[] = '</div>';
		$html[] = '<div class="modal-body limeticket_main limeticket_popup">';
	...
	static function TitleString($title,$subtitle,$menutitle,$isbrowser)
	{
		if ($isbrowser)
		{
			$setting = LIMETICKET_Settings::get('browser_prefix');		
			if ($setting == -1)
				$setting = LIMETICKET_Settings::get('title_prefix');		
		} else {
			$setting = LIMETICKET_Settings::get('title_prefix');		
		}
		...
		switch ($setting)
		{
			...
			case 1:	// Title - Subtitle
				if ($subtitle && $title)
					return JText::sprintf('LIMETICKET_PAGE_HEAD', $title, $subtitle);
				if ($subtitle)
					return $subtitle;
				return $title;
				break;
			...
			case 4: // Menu Title - Title or Subtitle
				if ($subtitle)
					return JText::sprintf('LIMETICKET_PAGE_HEAD', $menutitle, $subtitle);
				
				if ($title)
					return JText::sprintf('LIMETICKET_PAGE_HEAD', $menutitle, $title);
			...
			case 5: // Menu Title - Title - Subtitle
				if ($subtitle)
					return JText::sprintf('LIMETICKET_PAGE_HEAD_TRIPLE', $menutitle, $title, $subtitle);
	...
	?>
		<style>
		<?php echo LIMETICKET_Settings::get('display_module_style'); ?>
		</style>
	<?php
		...
		echo "<div class='limeticket_main limeticket_module $class'>\n";		
	}
	...
	static function PageTitle($title = "",$subtitle = "",$template = 'display_h1')
	{
		...
		$pageparams = $mainframe->getPageParameters('com_limeticket');			
		...
		// setup browser title
		$title_browser = LIMETICKET_Helper::TitleString($title, $subtitle, $pageheading, true);
		...
		// setup page title
		$title_page = LIMETICKET_Helper::TitleString($title, $subtitle, $menutitle, false);

		// title set to none, just return
		if (LIMETICKET_Settings::get('title_prefix') == 99)
			return $post;
		
		// should the "Show Page Heading" option be used?
		if (LIMETICKET_Settings::get('use_joomla_page_title_setting'))
		{
			if ($pageparams->get('show_page_heading',1))
				return str_replace("$1",$title_page,LIMETICKET_Settings::get($template)) . $post;
			
			return $post;
		}
		
		// Normal, always show title
		$output = str_replace("$1",$title_page,LIMETICKET_Settings::get($template));
	...
	static function PageSubTitle($title,$usejtext = true)
	{
		...
		return str_replace("$1",$title,LIMETICKET_Settings::get('display_h2'));
	}
	...
	static function Date($date,$format = LIMETICKET_DATE_LONG, $format_custom = null)
	{
		//echo "In : $date<br>";
		//echo "Format : " . $format . "<br>";
		//echo "Offset : " . LIMETICKET_Settings::Get('timezone_offset') . "<br>";
		
		if ((int)$date > 10000)
			$date = date("Y-m-d H:i:s", $date);

		if ((int)LIMETICKET_Settings::Get('timezone_offset') != 0)
		{
			$time = strtotime($date);
			$time += 3600 * (int)LIMETICKET_Settings::Get('timezone_offset');
			$date = date("Y-m-d H:i:s", $time);
		}
		
		switch($format)
		{
			case LIMETICKET_DATE_SHORT:	
				$ft = JText::_('DATE_FORMAT_LC4');
				break;
			case LIMETICKET_DATE_MID:	
				$ft = JText::_('DATE_FORMAT_LC3');
				break;
			case LIMETICKET_DATE_LONG:	
				$ft = JText::_('DATE_FORMAT_LC1');
				break;
			case LIMETICKET_TIME_SHORT:	
				$ft = 'H:i';
				break;
			case LIMETICKET_TIME_LONG:	
				$ft = 'H:i:s';
				break;
			case LIMETICKET_DATETIME_SHORT:	
				$ft = JText::_('DATE_FORMAT_LC4') . ', H:i';
				break;
			case LIMETICKET_DATETIME_MID:	
				$ft = JText::_('DATE_FORMAT_LC3') . ', H:i';
				break;
			case LIMETICKET_DATETIME_LONG:	
				$ft = JText::_('DATE_FORMAT_LC1') . ', H:i';
				break;
			case LIMETICKET_DATETIME_MYSQL:	
				$ft = 'Y-m-d H:i:s';
				break;
			case LIMETICKET_DATE_CUSTOM:
				$ft = $format_custom;
				break;
			default:
		...
		if ($format == LIMETICKET_DATETIME_SHORT && LIMETICKET_Settings::Get('date_dt_short') != "")
			$ft = LIMETICKET_Settings::Get('date_dt_short');
		
		if ($format == LIMETICKET_DATETIME_MID && LIMETICKET_Settings::Get('date_dt_long') != "")
			$ft = LIMETICKET_Settings::Get('date_dt_long');
		
		if ($format == LIMETICKET_DATE_SHORT && LIMETICKET_Settings::Get('date_d_short') != "")
			$ft = LIMETICKET_Settings::Get('date_d_short');
		
		if ($format == LIMETICKET_DATE_MID && LIMETICKET_Settings::Get('date_d_long') != "")
			$ft = LIMETICKET_Settings::Get('date_d_long');
	...
	{
		if ($format == '')
			$format = LIMETICKET_Settings::get('date_dt_short');
	...
	{
		$format = LIMETICKET_Helper::getFormat($format);
	...
	static function escapeJavaScriptTextForAlert($string)
	{
		if (function_exists("mb_convert_encoding"))
			return mb_convert_encoding(LIMETICKET_Helper::escapeJavaScriptText($string), 'UTF-8', 'HTML-ENTITIES');
		
		return LIMETICKET_Helper::escapeJavaScriptText($string);
	}

	/*$LIMETICKETRoute_debug = array();
	...
	static function NeedBaseBreadcrumb($pathway, $aparams)
	{
		...
		$parts = LIMETICKETRoute::SplitURL($link);
		
		if (!array_key_exists('Itemid', $parts))
			return true;
			
		//print_p($parts);
		if (!array_key_exists($parts['Itemid'],$LIMETICKETRoute_menus))
		{
		...
		/*foreach($LIMETICKETRoute_menus[$parts['Itemid']] as $key => $value)
		{
			if ($value != "")
			{
		...
		foreach($aparams as $key => $value)
		{
			if ($value != "")
			{
				if (!array_key_exists($key,$LIMETICKETRoute_menus[$parts['Itemid']]))
				{
					$ok = false;
					break;
				}
			
				if ($LIMETICKETRoute_menus[$parts['Itemid']][$key] != $value)
				{
					$ok = false;
					break;		
				}
		...
		if ($ok)
			return false;
		/*print_p($aparams);
		print_p($LIMETICKETRoute_menus[$parts['Itemid']]);*/
	...
	static function GetPublishedText($ispub,$notip = false)
	{
		...
		if ($notip)
			return '<img src="components/com_limeticket/assets/images/' . $img . '" width="16" height="16" border="0" alt="' . $alt .'" />';	
			
		return '<img class="limeticketTip" src="components/com_limeticket/assets/images/' . $img . '" width="16" height="16" border="0" alt="' . $alt .'" title="'.$alt.'" />';	
	...
	static function GetYesNoText($ispub)
	{
		...
		$src = JURI::base() . "/components/com_limeticket/assets/images";
	...
	static function ShowError(&$errors, $key)
	{
		...
		return "<div class='limeticket_ticket_error'>" . $errors[$key] . "</div>";
	}
	...
	{
		if (!LIMETICKET_Helper::$sceditor)
		{
			if (LIMETICKET_Settings::Get('support_sceditor'))
			{
				$document = JFactory::getDocument();
				$document->addScript(JURI::root(true).'/components/com_limeticket/assets/js/sceditor/jquery.sceditor.bbcode.js'); 
				$document->addScript(JURI::root(true).'/components/com_limeticket/assets/js/sceditor/include.sceditor.js'); 
				$document->addScriptDeclaration("var sceditor_emoticons_root = '" . JURI::root( true ) . "/components/com_limeticket/assets/';");
				$document->addScriptDeclaration("var sceditor_style_root = '" . JURI::root( true ) . "/components/com_limeticket/assets/js/sceditor/';");
				$document->addScriptDeclaration("var sceditor_style_type = '" . LIMETICKET_Settings::get('sceditor_content') . "';");
				
				$button_exclude = array();
				if (!LIMETICKET_Settings::get('sceditor_emoticons'))
					$button_exclude[] = "emoticon";
				
				if (LIMETICKET_Settings::get('sceditor_buttonhide'))
					$button_exclude[] = str_replace(" ", "", LIMETICKET_Settings::get('sceditor_buttonhide'));
				
				$document->addScriptDeclaration("var sceditor_toolbar_exclude = '" . implode(",",$button_exclude) . "';");
				if ($for_user)
				{
					$document->addScriptDeclaration("var sceditor_paste = '" . LIMETICKET_Settings::get('sceditor_paste_user') . "';");
				} else {
					$document->addScriptDeclaration("var sceditor_paste = '" . LIMETICKET_Settings::get('sceditor_paste_admin') . "';");
				}
				
				$document->addStyleSheet(JURI::root(true).'/components/com_limeticket/assets/js/sceditor/themes/' . LIMETICKET_Settings::get('sceditor_theme') . '.css'); 
				//$document->addStyleSheet(JURI::root(true).'/components/com_limeticket/assets/js/sceditor/themes/square.css'); 
			}
			LIMETICKET_Helper::$sceditor = true;
		}
	}
	...
	{
		if (substr($text,0,9) == "{RAWHTML}" && LIMETICKET_Settings::get('allow_raw_html_messages'))
		{
			$tidy_config = array(
                     'clean' => true,
                     'output-xhtml' => true,
			...
			require_once(JPATH_SITE.DS.'components'.DS.'com_limeticket'.DS.'helper'.DS.'third'.DS.'htmlpurifier'.DS.'HTMLPurifier.standalone.php');    
			...
			{
				$text = str_replace("[quoted]", '<div class="quoted_text_cont"><span class="quoted_text_show small muted">' . \JText::_("LIMETICKET_SHOW_QUOTED_TEXT") . '</span><div class="quoted_text">', $text);
		...
		require_once(JPATH_SITE.DS.'components'.DS.'com_limeticket'.DS.'helper'.DS.'third'.DS.'sbbcodeparser'.DS.'sbbcodeparser.php');
		...
		if (LIMETICKET_Settings::get('sceditor_emoticons'))
		{
			$parser->add_emoticons(array(
                ":)" => JURI::root() . "components/com_limeticket/assets/emoticons/smile.png",
                ":angel:" => JURI::root() . "components/com_limeticket/assets/emoticons/angel.png",
                ":angry:" => JURI::root() . "components/com_limeticket/assets/emoticons/angry.png",
                "8-)" => JURI::root() . "components/com_limeticket/assets/emoticons/cool.png",
                ":'(" => JURI::root() . "components/com_limeticket/assets/emoticons/cwy.png",
                ":ermm:" => JURI::root() . "components/com_limeticket/assets/emoticons/ermm.png",
                ":D" => JURI::root() . "components/com_limeticket/assets/emoticons/grin.png",
                "<3" => JURI::root() . "components/com_limeticket/assets/emoticons/heart.png",
                ":(" => JURI::root() . "components/com_limeticket/assets/emoticons/sad.png",
                ":O" => JURI::root() . "components/com_limeticket/assets/emoticons/shocked.png",
                ":P" => JURI::root() . "components/com_limeticket/assets/emoticons/tongue.png",
                ";)" => JURI::root() . "components/com_limeticket/assets/emoticons/wink.png",
                ":alien:" => JURI::root() . "components/com_limeticket/assets/emoticons/alien.png",
                ":blink:" => JURI::root() . "components/com_limeticket/assets/emoticons/blink.png",
                ":blush:" => JURI::root() . "components/com_limeticket/assets/emoticons/blush.png",
                ":cheerful:" => JURI::root() . "components/com_limeticket/assets/emoticons/cheerful.png",
                ":devil:" => JURI::root() . "components/com_limeticket/assets/emoticons/devil.png",
                ":dizzy:" => JURI::root() . "components/com_limeticket/assets/emoticons/dizzy.png",
                ":getlost:" => JURI::root() . "components/com_limeticket/assets/emoticons/getlost.png",
                ":happy:" => JURI::root() . "components/com_limeticket/assets/emoticons/happy.png",
                ":kissing:" => JURI::root() . "components/com_limeticket/assets/emoticons/kissing.png",
                ":ninja:" => JURI::root() . "components/com_limeticket/assets/emoticons/ninja.png",
                ":pinch:" => JURI::root() . "components/com_limeticket/assets/emoticons/pinch.png",
                ":pouty:" => JURI::root() . "components/com_limeticket/assets/emoticons/pouty.png",
                ":sick:" => JURI::root() . "components/com_limeticket/assets/emoticons/sick.png",
                ":sideways:" => JURI::root() . "components/com_limeticket/assets/emoticons/sideways.png",
                ":silly:" => JURI::root() . "components/com_limeticket/assets/emoticons/silly.png",
                ":sleeping:" => JURI::root() . "components/com_limeticket/assets/emoticons/sleeping.png",
                ":unsure:" => JURI::root() . "components/com_limeticket/assets/emoticons/unsure.png",
                ":woot:" => JURI::root() . "components/com_limeticket/assets/emoticons/w00t.png",
                ":wassat:" => JURI::root() . "components/com_limeticket/assets/emoticons/wassat.png",
                ":whistling:" => JURI::root() . "components/com_limeticket/assets/emoticons/whistling.png",
                ":love:" => JURI::root() . "components/com_limeticket/assets/emoticons/wub.png"
            ));
		}
		...
		while (stripos($text, "[url=") !== false)
		{
			...
			if (stripos($rest, "[/url]") !== false)
			{
				...
				$target = LIMETICKET_Settings::get('ticket_link_target') ? '_blank' : '';
		...
		{		
			...
			{
				$link = JRoute::_('index.php?option=com_limeticket&view=ticket&ticketid=' . JRequest::getVar('ticketid') . '&fileid=' . $attachid, false);
			} else {
				$link = JRoute::_('index.php?option=com_limeticket&view=admin_support&task=attach.view&ticketid=' . JRequest::getVar('ticketid') . '&fileid=' . $attachid, false);
			}
	...
	{
		require_once(JPATH_SITE.DS.'components'.DS.'com_limeticket'.DS.'tmpl'.DS.'modal.php');
	}
	...
	static function NoPerm()
	{
		...
		require_once (JPATH_SITE.DS.'components'.DS.'com_limeticket'.DS.'tmpl'.DS.'noperm.php');		
	...
	static function TableExists($table)
	{
		...
		if (empty(self::$_tables))
		{
			...
			self::$_tables = LIMETICKETJ3Helper::loadResultArray($db);
		}
	...
	static function HelpText($ident, $return = false)
	{
		...
		{
			$db = JFactory::getDBO();
			$qry = "SELECT * FROM #__limeticket_help_text WHERE published = 1";
			$db->setquery($qry);
			self::$help_texts = $db->loadObjectList("identifier");
			
			if (self::$help_texts) LIMETICKET_Translate_Helper::Tr(self::$help_texts);
		}
	...
	{
		$ticket_id = LIMETICKET_Input::getInt('t');
		$ticket_pass = LIMETICKET_Input::getString('p');
		if ($ticket_pass && $ticket_id > 0)
		{
			$db = JFactory::getDBO();
			
			$qry = "SELECT * FROM #__limeticket_ticket_ticket WHERE id = " . $db->escape($ticket_id) . " AND password = '" . $db->escape($ticket_pass) . "'";
			$db->setQuery($qry);
			...
			if ($ticket)
			{
				...
				$link = LIMETICKETRoute::_("index.php?option=com_limeticket&view=ticket&layout=view&ticketid=" . $ticket_id, false);
	...
	static function getSiteName()
	{
		...
		if (LIMETICKET_Settings::get('support_email_site_name') != "")
			$sitename = LIMETICKET_Settings::get('support_email_site_name');
	...
	static function TicketTime($time, $format = LIMETICKET_DATETIME_SHORT) {
		
		$abs = LIMETICKET_Helper::Date($time, $format);
		$rel = LIMETICKET_Helper::RelativeTime(strtotime($time));

		if (LIMETICKET_Settings::get('absolute_last_open') == 3) { // relative with no tip
			return $abs;
		} elseif (LIMETICKET_Settings::get('absolute_last_open') == 2) {	// absolute without tip
			return $rel;
		} elseif (LIMETICKET_Settings::get('absolute_last_open') == 1) {	// absolute withtip
			return "<span class='limeticketTip' title='$rel'>$abs</span>";
		} else { // relative with tip
			return "<span class='limeticketTip' title='$abs'>$rel</span>";
		}
	}
	...
	{  
		$min = -1;
		if (substr(LIMETICKET_Input::GetCmd('view'), 0, 5) == "admin" || $for_admin)
		{
			$min = self::convertPHPSizeToBytes(LIMETICKET_Settings::get('support_attach_max_size_admins'));
		} else {
			$min = self::convertPHPSizeToBytes(LIMETICKET_Settings::get('support_attach_max_size'));
		}
	...
	static function IncludeFileUpload()
	{
		...
		$document = JFactory::getDocument();
		$document->addStyleSheet(JURI::root(true).'/components/com_limeticket/assets/css/fileupload/jquery.fileupload.css'); 
		$document->addStyleSheet(JURI::root(true).'/components/com_limeticket/assets/css/fileupload/jquery.fileupload-ui.css'); 

		$document->addScript( JURI::root(true).'/components/com_limeticket/assets/js/fileupload/tmpl.js' );
		$document->addScript( JURI::root(true).'/components/com_limeticket/assets/js/fileupload/jquery.iframe-transport.js' );
		$document->addScript( JURI::root(true).'/components/com_limeticket/assets/js/fileupload/jquery.fileupload.js' );
		$document->addScript( JURI::root(true).'/components/com_limeticket/assets/js/fileupload/jquery.fileupload-process.js' );
		$document->addScript( JURI::root(true).'/components/com_limeticket/assets/js/fileupload/jquery.fileupload-validate.js' );
		$document->addScript( JURI::root(true).'/components/com_limeticket/assets/js/fileupload/jquery.fileupload-ui.js' );
		$document->addScript( JURI::root(true).'/components/com_limeticket/assets/js/fileupload/fileupload.js' );

		$upload_token = substr(md5(time()), 0, 8);
		$post_url = JRoute::_('index.php?option=com_limeticket&view=attach&upload_token='.$upload_token.'&task=process',false);
		...
		$main_js = "
var limeticket_file_upload_post = '$post_url';
var limeticket_file_upload_max_size = '$max_size';
var limeticket_file_upload_file_types = $formats;
";
	...
	{
		if (substr(LIMETICKET_Input::GetCmd('view'), 0, 5) == "admin")
		{
			$formats = trim(LIMETICKET_Settings::get('support_attach_types_admins'));
		} else {
			$formats = trim(LIMETICKET_Settings::get('support_attach_types'));
		}
	...
	static function enqueueMessage($msg, $type = 'message')
	{
		...
		{
			$session = JFactory::getSession();
			$sessionQueue = $session->get('limeticket.queue');

			if (count($sessionQueue))
			{
				self::$_messageQueue = $sessionQueue;
				$session->set('limeticket.queue', null);
			}
		}
		...
		$session = JFactory::getSession();
		$session->set('limeticket.queue', self::$_messageQueue);
	}
	...
	static function getEncKey($salt = "")
	{
		...
		if ($salt == "") $salt = "limeticket_enc_salt";
	...
	static function AutoLoginCreate($userid)
	{
		...
		return LIMETICKET_Helper::base64url_encode($enc);
	}

	static function AutoLoginDecrypt($data)
	{
		$data = urldecode($data);
		$data = LIMETICKET_Helper::base64url_decode($data);
	...
	static function loadPlugins()
	{
		...
		{
			$db = JFactory::getDBO();
			$qry = "SELECT * FROM #__limeticket_plugins";
			$db->setQuery($qry);
	...
	static function cleanLogs()
	{
		...
		if (LIMETICKET_Settings::get('support_cronlog_keep') > 0)
		{
			$qry = "DELETE FROM #__limeticket_cron_log WHERE `when` < DATE_SUB(NOW(), INTERVAL ".(int)LIMETICKET_Settings::get('support_cronlog_keep')." DAY)";
			$db->SetQuery($qry);
			$db->Query();	
		}	
		
		if (LIMETICKET_Settings::get('support_emaillog_keep') > 0)
		{
			$db = JFactory::getDBO();	
			
			$now = LIMETICKET_Helper::CurDate();
			$qry = "DELETE FROM #__limeticket_ticket_email_log WHERE firstseen < DATE_SUB(NOW(), INTERVAL ".(int)LIMETICKET_Settings::get('support_emaillog_keep')." DAY)";
			$db->SetQuery($qry);
			$db->Query();	
		}	
	}
	...
	{
		$loc = LIMETICKET_Settings::get('attach_location');
	...
	static function getPassthroughVars()
	{
		...
		$fields = LIMETICKETCF::GetCustomFields(0, 0, 0);
	...
	static function openPassthrough()
	{
		...
		foreach ($vars as $var)
		{
			$value = LIMETICKET_Input::getString($var);
			if ($value)
			{
	...
	static function IncludeChosen($selector = 'select')
	{
		...
		$document->addScript( JURI::root(true).'/components/com_limeticket/assets/js/jquery/jquery.chosen.js' );	
		$document->addStyleSheet(JURI::root(true).'/components/com_limeticket/assets/css/chosen_j25.css'); 
	...
	static function getCurrentURL($raw = true, $remove = array('what' => 'find'))
	{
		...
		if (!$raw) $url = LIMETICKETRoute::_($url);
