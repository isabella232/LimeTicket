...
require_once (JPATH_SITE.DS.'components'.DS.'com_limeticket'.DS.'helper'.DS.'email_to.php');
require_once (JPATH_SITE.DS.'components'.DS.'com_limeticket'.DS.'helper'.DS.'parser.php');

class LIMETICKET_EMail
{
	/**
	 * Emails to users
	 */	
	...
	static function EMail_To_Ticket_User($template_id, $ticket, $subject, $body, $files, $extra_vars, $sender)
	{
		...
		$to = new LIMETICKET_EMail_To();
		$to->addTicketUsers($ticket);
		if ($cc_admins && LIMETICKET_Settings::get('support_email_bcc_handler')) $to->addTicketAdmins($ticket);
		...
		$parser->setSender(LIMETICKET_EMail_To::UserToRec($sender));
		...
		{	
			...
			if (LIMETICKET_Settings::get('support_email_file_user') && (!$is_create || $session->Get('admin_create') > 0)) $mailer->addFiles($files);
	...
	static function EMail_To_Ticket_Handler($template_id, $ticket, $subject, $body, $files, $extra_vars, $sender)
	{
		...
		{			
			...
			if (LIMETICKET_Settings::get('support_email_file_handler') == 1) $mailer->addFiles($files);
	...
	static function Admin_Forward($ticket, $subject, $body, $files = array(), $sender = null, $oldhandler = null)
	{
		...
		if ($oldhandler > 0) LIMETICKET_EMail_To::$extra_admin_to_id[$oldhandler] = "Admin: Old Handler";
	...
	{
		return LIMETICKET_Settings::get('support_' . $tag);
	}
	...
	static function MakePassList($tickets, $is_html)
	{
		if ($is_html)
		{
			...
			if (in_array(LIMETICKET_Settings::get('support_unreg_type'), array(0,1)))
			...
			foreach ($tickets as $ticket)
			{
				...
				$output[] = "<td>" . LIMETICKET_Helper::Date($ticket->lastupdate, LIMETICKET_DATETIME_MID) . "</td>";		
			...
			foreach ($tickets as $ticket)
			{
				...
				$output[] = JText::_('LAST_UPDATE') . " : " . LIMETICKET_Helper::Date($ticket->lastupdate, LIMETICKET_DATETIME_MID) . "\n";		
	...
	/*static function ParseGeneralTemplate($template, $data)
	{
		...
		if ($template['ishtml'])
			$email['body'] = LIMETICKET_Helper::MaxLineLength($email['body']);
	...
	static function ParseMessageRows(&$messages, $ishtml, $foruser = false)
	{
		...
		foreach ($messages as &$message)
		{
			...
			$vars[] = self::BuildVar('posted',LIMETICKET_Helper::Date($message['posted']));
			
			$message['body'] = LIMETICKET_Helper::ParseBBCode($message['body'],null,false,false,$foruser);
	...
	/*static function GetHandler($admin_id, $tmpl)
	{
		...
		$db = JFactory::getDBO();
		$query = " SELECT * FROM #__users WHERE id = '".LIMETICKETJ3Helper::getEscaped($db, $admin_id)."'";
		$db->setQuery($query);
	...
	/*static function GetUser($user_id)
	{
		...
		$db = JFactory::getDBO();
		$qry = "SELECT * FROM #__users WHERE id = '".LIMETICKETJ3Helper::getEscaped($db, $user_id)."'";
		$db->setQuery($qry);
		$row = $db->loadAssoc();
		return $row;
	}*/
	...
	{
		$db = JFactory::getDBO();
		$qry = "SELECT title FROM #__limeticket_kb_art WHERE id = '".LIMETICKETJ3Helper::getEscaped($db, $artid)."'";	
		$db->setQuery($qry);
		$row = $db->loadAssoc();
		return $row['title'];
	}*/
	...
	{
		$db = JFactory::getDBO();
		$qry = "SELECT title FROM #__limeticket_ticket_pri WHERE id = '".LIMETICKETJ3Helper::getEscaped($db, $pri_id)."'";	
		$db->setQuery($qry);
		$row = $db->loadAssoc();
		return $row['title'];
	}*/
	...
	{
		$db = JFactory::getDBO();
		$qry = "SELECT title FROM #__limeticket_ticket_cat WHERE id = '".LIMETICKETJ3Helper::getEscaped($db, $cat_id)."'";	
		$db->setQuery($qry);
		$row = $db->loadAssoc();
		return $row['title'];
	}*/
	...
	/*static function GetDepartment($dept_id, $field = 'title')
	{
		...
		{
			$db = JFactory::getDBO();
			$qry = "SELECT title, description FROM #__limeticket_ticket_dept WHERE id = '".LIMETICKETJ3Helper::getEscaped($db, $dept_id)."'";	
			$db->setQuery($qry);
	...
	{
		$db = JFactory::getDBO();
		$qry = "SELECT m.*, u.name, u.username, u.email FROM #__limeticket_ticket_messages as m";
		$qry .= " LEFT JOIN #__users as u ON m.user_id = u.id";
		$qry .= " WHERE ticket_ticket_id = '".LIMETICKETJ3Helper::getEscaped($db, $ticket_id)."'";	
	...
	{
		if (LIMETICKET_Settings::get('support_unreg_domain_restrict') == 0) return true;

		$lines = explode("\n", LIMETICKET_Settings::get('support_unreg_domain_list'));
		...
		foreach ($lines as $line)
		{
			...
			if (substr($line, 0, 1) == "/")
			{
				...
				// domain
				if (LIMETICKET_Helper::endsWith($email, "@" . $line))
				$match = true;
			}
		}

		if (LIMETICKET_Settings::get('support_unreg_domain_restrict') == 1) return $match;
		if (LIMETICKET_Settings::get('support_unreg_domain_restrict') == 2) return !$match;
	}	
}
